...
void cmovo  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  0);
}
void cmovno (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  1);
}
void cmovb  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  2);
}
...
void cmovnb (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  3);
}
...
void cmove  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  4);
}
void cmovz  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  4);
}
void cmovne (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  5);
}
void cmovnz (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  5);
}
void cmovbe (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  6);
}
void cmovna (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  6);
}
void cmovnbe(Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  7);
}
void cmova  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  7);
}
void cmovs  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  8);
}
void cmovns (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 |  9);
}
void cmovp  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 10);
}
void cmovpe (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 10);
}
void cmovnp (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 11);
}
void cmovpo (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 11);
}
void cmovl  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 12);
}
void cmovnge(Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 12);
}
void cmovnl (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 13);
}
void cmovge (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 13);
}
void cmovle (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 14);
}
void cmovng (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 14);
}
void cmovnle(Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 15);
}
void cmovg  (Reg reg, Operand op) {
	opModRM(reg, op, op.isREG(16 | i32e), op.isMEM, 0x0F, 0B01000000 | 15);
}

void jo  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x70, 0x80, 0x0F);
}
void jno (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x71, 0x81, 0x0F);
}
void jb  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x72, 0x82, 0x0F);
}
void jc  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x72, 0x82, 0x0F);
}
void jnae(T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x72, 0x82, 0x0F);
}
void jnb (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x73, 0x83, 0x0F);
}
void jae (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x73, 0x83, 0x0F);
}
void jnc (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x73, 0x83, 0x0F);
}
void je  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x74, 0x84, 0x0F);
}
void jz  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x74, 0x84, 0x0F);
}
void jne (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x75, 0x85, 0x0F);
}
void jnz (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x75, 0x85, 0x0F);
}
void jbe (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x76, 0x86, 0x0F);
}
void jna (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x76, 0x86, 0x0F);
}
void jnbe(T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x77, 0x87, 0x0F);
}
void ja  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x77, 0x87, 0x0F);
}
void js  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x78, 0x88, 0x0F);
}
void jns (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x79, 0x89, 0x0F);
}
void jp  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7A, 0x8A, 0x0F);
}
void jpe (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7A, 0x8A, 0x0F);
}
void jnp (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7B, 0x8B, 0x0F);
}
void jpo (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7B, 0x8B, 0x0F);
}
void jl  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7C, 0x8C, 0x0F);
}
void jnge(T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7C, 0x8C, 0x0F);
}
void jnl (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7D, 0x8D, 0x0F);
}
void jge (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7D, 0x8D, 0x0F);
}
void jle (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7E, 0x8E, 0x0F);
}
void jng (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7E, 0x8E, 0x0F);
}
void jnle(T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7F, 0x8F, 0x0F);
}
void jg  (T) (T label, LabelType type = T_AUTO) {
	opJmp(label, type, 0x7F, 0x8F, 0x0F);
}
...
{
	void jcxz (T) (T label) {
		db(0x67); opJmp(label, T_SHORT, 0xe3, 0, 0);
	}
...
void bt (Operand op, uint8 imm) {
	opR_ModM(op, 16 | 32 | 64, 4, 0x0f, 0xba, NONE, false, 1); db(imm);
}
...
void bts(Operand op, uint8 imm) {
	opR_ModM(op, 16 | 32 | 64, 5, 0x0f, 0xba, NONE, false, 1); db(imm);
}
...
void btr(Operand op, uint8 imm) {
	opR_ModM(op, 16 | 32 | 64, 6, 0x0f, 0xba, NONE, false, 1); db(imm);
}
...
void btc(Operand op, uint8 imm) {
	opR_ModM(op, 16 | 32 | 64, 7, 0x0f, 0xba, NONE, false, 1); db(imm);
}
...
void vblendpd  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x0D, true, 0, imm);
}
void vblendpd  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x0D, true, 0, imm);
}
void vblendps  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x0C, true, 0, imm);
}
void vblendps  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x0C, true, 0, imm);
}
void vdppd     (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x41, false, 0, imm);
}
void vdppd     (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x41, false, 0, imm);
}
void vdpps     (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x40, true, 0, imm);
}
void vdpps     (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x40, true, 0, imm);
}
void vmpsadbw  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x42, true, 0, imm);
}
void vmpsadbw  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x42, true, 0, imm);
}
void vpblendw  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x0E, true, 0, imm);
}
void vpblendw  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x0E, true, 0, imm);
}
void vpblendd  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x02, true, 0, imm);
}
void vpblendd  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x02, true, 0, imm);
}
void vroundsd  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x0B, false, 0, imm);
}
void vroundsd  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x0B, false, 0, imm);
}
void vroundss  (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x0A, false, 0, imm);
}
void vroundss  (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x0A, false, 0, imm);
}
void vpclmulqdq(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x44, false, 0, imm);
}
void vpclmulqdq(Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x44, false, 0, imm);
}
...
void vcmppd(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F | PP_66, 0xC2,  true, -1, imm);
}
void vcmppd(Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F | PP_66, 0xC2,  true, -1, imm);
}
void vcmpps(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F,         0xC2,  true, -1, imm);
}
void vcmpps(Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F,         0xC2,  true, -1, imm);
}
void vcmpsd(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F | PP_F2, 0xC2, false, -1, imm);
}
void vcmpsd(Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F | PP_F2, 0xC2, false, -1, imm);
}
void vcmpss(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F | PP_F3, 0xC2, false, -1, imm);
}
void vcmpss(Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F | PP_F3, 0xC2, false, -1, imm);
}

void vinsertps( Xmm x1,  Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F3A | PP_66, 0x21, false, 0, imm);
}
void vinsertps(Xmm xmm,          Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F3A | PP_66, 0x21, false, 0, imm);
}
...
void vpalignr (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,   x2, op, MM_0F3A | PP_66, 0x0F, true, -1, imm);
}
void vpalignr (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm,  xmm, op, MM_0F3A | PP_66, 0x0F, true, -1, imm);
}
...
void vshufpd (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F | PP_66, 0xC6, true, -1, imm);
}
void vshufpd (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F | PP_66, 0xC6, true, -1, imm);
}
void vshufps (Xmm x1, Xmm x2, Operand op, uint8 imm) {
	opAVX_X_X_XM( x1,  x2, op, MM_0F,         0xC6, true, -1, imm);
}
void vshufps (Xmm xmm,        Operand op, uint8 imm) {
	opAVX_X_X_XM(xmm, xmm, op, MM_0F,         0xC6, true, -1, imm);
}
...
void cmpeqpsb   (Xmm x, Operand op) {
	cmpps (x, op, 0);
}
void cmpltpsb   (Xmm x, Operand op) {
	cmpps (x, op, 1);
}
void cmplepsb   (Xmm x, Operand op) {
	cmpps (x, op, 2);
}
...
void vmovhpd(Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !op2.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x, op1, op2, MM_0F | PP_66, 0x16, false);
}
void vmovhps(Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !op2.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x, op1, op2, MM_0F,         0x16, false);
}
void vmovlpd(Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !op2.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x, op1, op2, MM_0F | PP_66, 0x12, false);
}
void vmovlps(Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !op2.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x, op1, op2, MM_0F,         0x12, false);
}
...
void vbroadcastsd(Ymm y, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(y, op, MM_0F38 | PP_66, 0x19, true, 0);
}
void vbroadcastss(Xmm x, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(x, op, MM_0F38 | PP_66, 0x18, true, 0);
}
void vpbroadcastb(Xmm x, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(x, op, MM_0F38 | PP_66, 0x78, true, 0);
}
void vpbroadcastw(Xmm x, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(x, op, MM_0F38 | PP_66, 0x79, true, 0);
}
void vpbroadcastd(Xmm x, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(x, op, MM_0F38 | PP_66, 0x58, true, 0);
}
void vpbroadcastq(Xmm x, Operand op) {
	if (!(op.isXMM || op.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_XM_IMM(x, op, MM_0F38 | PP_66, 0x59, true, 0);
}

void vextractf128(Operand op, Ymm y, uint8 imm) {
	opAVX_X_X_XMcvt(y, y.isXMM ? xm0 : ym0, op, op.isXMM, Kind.YMM, MM_0F3A | PP_66, 0x19, true, 0, imm);
}
void vextracti128(Operand op, Ymm y, uint8 imm) {
	opAVX_X_X_XMcvt(y, y.isXMM ? xm0 : ym0, op, op.isXMM, Kind.YMM, MM_0F3A | PP_66, 0x39, true, 0, imm);
}
...
void vinsertf128(Ymm y1, Ymm y2, Operand op, uint8 imm) {
	opAVX_X_X_XMcvt(y1, y2, op, op.isXMM, Kind.YMM, MM_0F3A | PP_66, 0x18, true, 0, imm);
}
void vinserti128(Ymm y1, Ymm y2, Operand op, uint8 imm) {
	opAVX_X_X_XMcvt(y1, y2, op, op.isXMM, Kind.YMM, MM_0F3A | PP_66, 0x38, true, 0, imm);
}
void vperm2f128 (Ymm y1, Ymm y2, Operand op, uint8 imm) {
	opAVX_X_X_XM(y1, y2, op, MM_0F3A | PP_66, 0x06, true, 0, imm);
}
void vperm2i128 (Ymm y1, Ymm y2, Operand op, uint8 imm) {
	opAVX_X_X_XM(y1, y2, op, MM_0F3A | PP_66, 0x46, true, 0, imm);
}
...
void vpextrw(    Reg r, Xmm x, uint8 imm) {
	opAVX_X_X_XM(XMM(r.getIdx), xm0,    x, MM_0F   | PP_66, 0xC5, false, r.isBit(64) ? 1 : 0, imm);
}
void vpextrw(Address addr, Xmm x, uint8 imm) {
	opAVX_X_X_XM(            x, xm0, addr, MM_0F3A | PP_66, 0x15, false,                  -1, imm);
}

void vpextrb(Operand op, Xmm x, uint8 imm) {
	if (!op.isREG(i32e) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, xm0, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x14, false, -1, imm);
}
void vpextrd(Operand op, Xmm x, uint8 imm) {
	if (!op.isREG(32)   && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, xm0, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x16, false,  0, imm);
}


void vpinsrb(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x1, x2, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x20, false, -1, imm);
}
void vpinsrb(Xmm x,          Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x,  x,  op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x20, false, -1, imm);
}
void vpinsrw(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x1, x2, op, !op.isMEM, Kind.XMM, MM_0F   | PP_66, 0xC4, false, -1, imm);
}
void vpinsrw(Xmm x,          Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x,  x,  op, !op.isMEM, Kind.XMM, MM_0F   | PP_66, 0xC4, false, -1, imm);
}
void vpinsrd(Xmm x1, Xmm x2, Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x1, x2, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x22, false,  0, imm);
}
void vpinsrd(Xmm x,          Operand op, uint8 imm) {
	if (!op.isREG(32) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x,  x,  op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x22, false,  0, imm);
}
...
void vpslldq(Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym7 : xm7, x1, x2, MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpslldq(Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym7 : xm7, x,  x,  MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsrldq(Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym3 : xm3, x1, x2, MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsrldq(Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym3 : xm3, x,  x,  MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsllw (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym6 : xm6, x1, x2, MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpsllw (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym6 : xm6, x,  x,  MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpslld (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym6 : xm6, x1, x2, MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpslld (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym6 : xm6, x,  x,  MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpsllq (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym6 : xm6, x1, x2, MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsllq (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym6 : xm6, x,  x,  MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsraw (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym4 : xm4, x1, x2, MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpsraw (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym4 : xm4, x,  x,  MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpsrad (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym4 : xm4, x1, x2, MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpsrad (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym4 : xm4, x,  x,  MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpsrlw (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym2 : xm2, x1, x2, MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpsrlw (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym2 : xm2, x,  x,  MM_0F | PP_66, 0x71, true, -1, imm);
}
void vpsrld (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym2 : xm2, x1, x2, MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpsrld (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym2 : xm2, x,  x,  MM_0F | PP_66, 0x72, true, -1, imm);
}
void vpsrlq (Xmm x1, Xmm x2, uint8 imm) {
	opAVX_X_X_XM(x1.isYMM ? ym2 : xm2, x1, x2, MM_0F | PP_66, 0x73, true, -1, imm);
}
void vpsrlq (Xmm x,          uint8 imm) {
	opAVX_X_X_XM( x.isYMM ? ym2 : xm2, x,  x,  MM_0F | PP_66, 0x73, true, -1, imm);
}

void vblendvpd(Xmm x1, Xmm x2, Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x2, op, MM_0F3A | PP_66, 0x4B, true, -1, x4.getIdx << 4);
}
void vblendvpd(Xmm x1,         Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x1, op, MM_0F3A | PP_66, 0x4B, true, -1, x4.getIdx << 4);
}
void vblendvps(Xmm x1, Xmm x2, Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x2, op, MM_0F3A | PP_66, 0x4A, true, -1, x4.getIdx << 4);
}
void vblendvps(Xmm x1,         Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x1, op, MM_0F3A | PP_66, 0x4A, true, -1, x4.getIdx << 4);
}
void vpblendvb(Xmm x1, Xmm x2, Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x2, op, MM_0F3A | PP_66, 0x4C, false, -1, x4.getIdx << 4);
}
void vpblendvb(Xmm x1,         Operand op, Xmm x4) {
	opAVX_X_X_XM(x1, x1, op, MM_0F3A | PP_66, 0x4C, false, -1, x4.getIdx << 4);
}
...
void vmovhlps(Xmm x1, Xmm x2, Operand op = OP()) {
	if (!op.isNone && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x1, x2, op, MM_0F, 0x12, false);
}
void vmovlhps(Xmm x1, Xmm x2, Operand op = OP()) {
	if (!op.isNone && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x1, x2, op, MM_0F, 0x16, false);
}
void vmovmskpd(Reg r, Xmm x) {
	if (!r.isBit(i32e)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x.isXMM ? XMM(r.getIdx) : YMM(r.getIdx), x.isXMM ? xm0 : ym0, x, MM_0F | PP_66, 0x50, true, 0);
}
void vmovmskps(Reg r, Xmm x) {
	if (!r.isBit(i32e)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x.isXMM ? XMM(r.getIdx) : YMM(r.getIdx), x.isXMM ? xm0 : ym0, x, MM_0F,         0x50, true, 0);
}
...
void vmovsd(Xmm x1, Xmm x2, Operand op = OP()) {
	if (!op.isNone && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x1, x2, op, MM_0F | PP_F2, 0x10, false);
}
...
void vmovss(Xmm x1, Xmm x2, Operand op = OP()) {
	if (!op.isNone && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(x1, x2, op, MM_0F | PP_F3, 0x10, false);
}
...
void vcvtsi2ss (Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !(op2.isREG(i32e) || op2.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, op1, op2, op2.isREG, Kind.XMM, MM_0F | PP_F3, 0x2A, false, (op1.isMEM || op2.isMEM) ? -1 : (op1.isREG(32) || op2.isREG(32)) ? 0 : 1);
}
void vcvtsi2sd (Xmm x, Operand op1, Operand op2 = OP()) {
	if (!op2.isNone && !(op2.isREG(i32e) || op2.isMEM)) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, op1, op2, op2.isREG, Kind.XMM, MM_0F | PP_F2, 0x2A, false, (op1.isMEM || op2.isMEM) ? -1 : (op1.isREG(32) || op2.isREG(32)) ? 0 : 1);
}
void vcvtps2pd (Xmm x, Operand op) {
	if (!op.isMEM && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, x.isXMM ? xm0 : ym0, op, !op.isMEM, x.isXMM ? Kind.XMM : Kind.YMM, MM_0F, 0x5A, true);
}
void vcvtdq2pd (Xmm x, Operand op) {
	if (!op.isMEM && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, x.isXMM ? xm0 : ym0, op, !op.isMEM, x.isXMM ? Kind.XMM : Kind.YMM, MM_0F | PP_F3, 0xE6, true);
}
void vcvtpd2ps (Xmm x, Operand op) {
	if (x.isYMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(op.isYMM ? YMM(x.getIdx) : x, op.isYMM ? ym0 : xm0, op, MM_0F | PP_66, 0x5A, true);
}
void vcvtpd2dq (Xmm x, Operand op) {
	if (x.isYMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(op.isYMM ? YMM(x.getIdx) : x, op.isYMM ? ym0 : xm0, op, MM_0F | PP_F2, 0xE6, true);
}
void vcvttpd2dq(Xmm x, Operand op) {
	if (x.isYMM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XM(op.isYMM ? YMM(x.getIdx) : x, op.isYMM ? ym0 : xm0, op, MM_0F | PP_66, 0xE6, true);
}
void vcvtph2ps (            Xmm x, Operand op) {
	if (!op.isMEM && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opVex(x, null, op, MM_0F38 | PP_66, 0x13, 0);
}
void vcvtps2ph (Operand op, Xmm x, uint8 imm)  {
	if (!op.isMEM && !op.isXMM) throw new XError(ERR.BAD_COMBINATION); opVex(x, null, op, MM_0F3A | PP_66, 0x1d, 0, imm);
}
...
version (XBYAK64)
{
	...
	void vpextrq(Operand op, Xmm x,              uint8 imm) {
		if (!op.isREG(64) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x, xm0, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x16, false, 1, imm);
	}
	void vpinsrq(Xmm x1, Xmm x2, Operand op, uint8 imm) {
		if (!op.isREG(64) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x1, x2, op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x22, false, 1, imm);
	}
	void vpinsrq(Xmm x,         Operand op, uint8 imm) {
		if (!op.isREG(64) && !op.isMEM) throw new XError(ERR.BAD_COMBINATION); opAVX_X_X_XMcvt(x,  x,  op, !op.isMEM, Kind.XMM, MM_0F3A | PP_66, 0x22, false, 1, imm);
	}
