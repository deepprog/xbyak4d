...
version (XBYAK64)
{
	...
	void badSSE()
	{
		...
		class Code : CodeGenerator
		{
			this()
			{
				tc.TEST_EXCEPTION!XError({ paddd(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ pslld(xm16, 1); });
				tc.TEST_EXCEPTION!XError({ movapd(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ movhpd(xm16, ptr[eax]); });
				tc.TEST_EXCEPTION!XError({ pextrb(eax, xm16, 1); });

				tc.TEST_EXCEPTION!XError({ lddqu(xm16, ptr[rax]); });
				tc.TEST_EXCEPTION!XError({ maskmovdqu(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ maskmovq(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ movapd(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movaps(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movd(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movd(xm16, ptr[rax]); });
				tc.TEST_EXCEPTION!XError({ movd(eax, xm16); });
				tc.TEST_EXCEPTION!XError({ movd(xm16, eax); });
				tc.TEST_EXCEPTION!XError({ movdq2q(mm1, xm16); });
				tc.TEST_EXCEPTION!XError({ movdqa(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movdqu(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movhlps(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ movlhps(xm16, xm1); });
				tc.TEST_EXCEPTION!XError({ movmskpd(rax, xm16); });
				tc.TEST_EXCEPTION!XError({ movmskps(rax, xm16); });
				tc.TEST_EXCEPTION!XError({ movntdq(ptr[rax], xmm16); });
				tc.TEST_EXCEPTION!XError({ movntdqa(xm16, ptr[rax]); });
				tc.TEST_EXCEPTION!XError({ movntpd(ptr[rax], xmm16); });
				tc.TEST_EXCEPTION!XError({ movntps(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movntq(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movq(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movq(xm16, ptr[rax]); });
				tc.TEST_EXCEPTION!XError({ movq(rax, xm16); });
				tc.TEST_EXCEPTION!XError({ movq(xm16, rax); });
				tc.TEST_EXCEPTION!XError({ movq2dq(xm16, mm1); });
				tc.TEST_EXCEPTION!XError({ movsd(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movss(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movupd(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ movups(ptr[rax], xm16); });
				tc.TEST_EXCEPTION!XError({ extractps(ptr[rax], xm16, 3); });
				tc.TEST_EXCEPTION!XError({ pextrb(ptr[rax], xm16, 3); });
				tc.TEST_EXCEPTION!XError({ pextrd(ptr[rax], xm16, 3); });
				tc.TEST_EXCEPTION!XError({ pextrw(ptr[rax], xm16, 3); });
				tc.TEST_EXCEPTION!XError({ pmovmskb(eax, xm16); });
			}
		}
...
{
	scope tc = TestCount(__FUNCTION__);
	tc.TEST_ASSERT(eax == eax);
	tc.TEST_ASSERT(ecx != xmm0);
	tc.TEST_ASSERT(ptr[eax] == ptr[eax]);
	tc.TEST_ASSERT(dword[eax] != ptr[eax]);
	tc.TEST_ASSERT(ptr[eax] != ptr[eax + 3]);
}
	...
	class Code : CodeGenerator
	{
		this()
		{
			...
			version (XBYAK64)
			{
				const mov_const[] tbl =
					[
						...
						{0xffffffff, 32, false},
						{0x100000000UL, 32, true},

						{uint64_t(-1), 64, false},
						{0x7fffffff, 64, false},
						{0xffffffffffffffffUL, 64, false},
						{0x80000000, 64, true},
						{0xffffffff, 64, true},
					];
			}
			...
			foreach (i, tmp; tbl)
			{
				...
				AddressFrame af = bit == 8 ? byte_ : bit == 16 ? word : bit == 32 ? dword : qword;
				if (tbl[i].error)
				{
					tc.TEST_EXCEPTION!XError({ mov(af[eax], v); });
				}
				else
				{
					mov(af[eax], v);
			...
			tc.TEST_EXCEPTION!XError({ mov(cx, al); });
			tc.TEST_EXCEPTION!XError({ mov(al, cx); });
			tc.TEST_EXCEPTION!XError({ mov(eax, cx); });
			tc.TEST_EXCEPTION!XError({ mov(eax, byte_[eax]); });
			tc.TEST_EXCEPTION!XError({ mov(word[eax], al); });
			...
			version (XBYAK64)
			{
				...
				for (int i = 4; i < 8; i++)
				{
					tc.TEST_EXCEPTION!XError({ add(ah, Reg8(i, true)); });
					tc.TEST_EXCEPTION!XError({ add(Reg8(i, true), dh); });
				}
				for (int i = 8; i < 32; i++)
				{
					tc.TEST_EXCEPTION!XError({ add(ah, Reg8(i)); });
					tc.TEST_EXCEPTION!XError({ add(Reg8(i), dh); });
				}
				tc.TEST_EXCEPTION!XError({ mov(ah, cx); });
				tc.TEST_EXCEPTION!XError({ mov(al, cx); });
...
version (XBYAK64)
{
	...
	void mov_8byte()
	{
		...
		class Code : CodeGenerator
		{
			...
			{
				super();
				mov(rax, ptr[cast(void*) 0x17fffffff]);
				mov(rax, ptr[cast(void*) 0x80000000uL]);
				mov(rax, ptr[cast(void*) 0xffffffffuL]);

				mov(rax, ptr[0x17fffffff]);
				mov(rax, ptr[0x80000000uL]);
				mov(rax, ptr[0xffffffffuL]);
			}
		}
...
void const_addressing()
{
	...
	version (XBYAK64)
	{
		const uint8_t[] tbl = [0x89, 0x14, 0x25, 0xff, 0xff, 0xff, 0x7f];
	}
	else
	{
		const uint8_t[] tbl = [0x89, 0x15, 0xff, 0xff, 0xff, 0x7f];
	}
	...
	class Code : CodeGenerator
	{
		this()
		{
			...
			for (int padding = 0; padding < 20; padding++)
			{
				...
				xbyak_align(alignSize);
				auto sun = cast(size_t) getCurr() % alignSize;
				tc.TEST_EQUAL(sun, 0u);
			}
...
void kmask()
{
	...
	class Code : CodeGenerator
	{
		this()
		{
			tc.TEST_EXCEPTION!Exception({ kmovb(k1, ax); });
			tc.TEST_EXCEPTION!Exception({ kmovw(k1, ax); });
			tc.TEST_EXCEPTION!Exception({ kmovd(k1, ax); });
			tc.TEST_EXCEPTION!Exception({ kmovq(k1, eax); });
			version (XBYAK64)
			{
		...
		void checkT_z()
		{
			...
			tc.TEST_EQUAL(p2 - p1, end - p2);

			//	writeln("size:", end-p2);
			for (int i = 0; i < end - p2; i++)
			{
				// writeln(p1[i], ":", p2[i]);
				tc.TEST_EQUAL(p1[i], p2[i]);
			}
		}
	}
...
version (XBYAK64)
{
	...
	void vfmaddps()
	{
		...
		scope c = new Code();
		tc.TEST_EQUAL(c.getSize(), tbl.length);
		auto ctbl = c.getCode();
	...
	@("vexpand_vpshufbitqmb") unittest
	{
		vexpand_vpshufbitqmb();
	}

	void vexpand_vpshufbitqmb()
	{
		scope tc = TestCount(__FUNCTION__);
		class Code : CodeGenerator
		{
	...
	void AMX()
	{
		...
		const uint8_t[] tbl = [
			0xc4, 0xe2, 0x78, 0x49, 0x44, 0x88, 0x40, 0xc4, 0xe2, 0x79, 0x49, 0x84,
			0xc4, 0x80, 0x00, 0x00,
			0x00, 0xc4, 0xe2, 0x7b, 0x4b, 0x5c, 0x57, 0x08, 0xc4, 0x82, 0x79, 0x4b,
			0x64, 0x08, 0x20, 0xc4,
			0xe2, 0x78, 0x49, 0xc0, 0xc4, 0x82, 0x7a, 0x4b, 0x54, 0x5a, 0x20, 0xc4,
			0xe2, 0x7b, 0x49, 0xf8,
			...
			0x62, 0xf3, 0x7c, 0x99, 0x56, 0xcd, 0x07, // vreducesh
			0x62, 0xf3, 0x64, 0x08, 0x57, 0x48, 0x20, 0x01,
			0x62, 0xf3, 0x54, 0x99, 0x57, 0xcc, 0x02, // vrndscaleph
			...
			0x62, 0xf3, 0x7c, 0x99, 0x08, 0xcd, 0x07, // vrndscalesh
			0x62, 0xf3, 0x64, 0x08, 0x0a, 0x48, 0x20, 0x01,
			0x62, 0xf3, 0x54, 0x99, 0x0a, 0xcc, 0x02, // vfpclassph
			...
			0x62, 0xf5, 0x7d, 0xdd, 0x5b, 0x48, 0x20, // vcvtph2psx
			...
			0x62, 0xf6, 0x7d, 0xdd, 0x13, 0x48, 0x20, // vcvtph2udq
			...
			0x62, 0xf5, 0x7c, 0xdd, 0x79, 0x48, 0x20, // vcvttph2dq
			...
			0x62, 0xf5, 0x7e, 0xdd, 0x5b, 0x48, 0x20, // vcvttph2udq
			...
			0x62, 0xf5, 0x7c, 0xdd, 0x78, 0x48, 0x20, // vcvtph2pd
			...
			0x62, 0xf5, 0x7c, 0xdd, 0x5a, 0x48, 0x20, // vcvtph2qq
			...
			0x62, 0xf5, 0x7d, 0xdd, 0x7b, 0x48, 0x20, // vcvtph2uqq
			...
			0x62, 0xf5, 0x7d, 0xdd, 0x79, 0x48, 0x20, // vcvttph2uqq
			...
			0x62, 0xf5, 0x7d, 0xdd, 0x78, 0x48, 0x20, // vcvtdq2ph
			...
			0x62, 0xf5, 0x7c, 0x58, 0x5b, 0x48, 0x10, // vcvtps2phx
			...
			0x62, 0xf5, 0x7d, 0x58, 0x1d, 0x48, 0x10, // vcvtudq2ph
			...
			0x62, 0xf5, 0x7f, 0x58, 0x7a, 0x48, 0x10, // vcvtpd2ph
			...
			0x62, 0xf5, 0xfd, 0x58, 0x5a, 0x48, 0x08, // vcvtqq2ph
			...
			0x62, 0xf5, 0xfc, 0x58, 0x5b, 0x48, 0x08, // vcvtuqq2ph
			...
			0x62, 0xf5, 0xff, 0x58, 0x7a, 0x48, 0x08, // vcvtph2uw
			...
			0x62, 0xf5, 0x7c, 0x58, 0x7d, 0x48, 0x20, // vcvtph2w
			...
			0x62, 0xf5, 0x7d, 0x58, 0x7d, 0x48, 0x20, // vcvttph2uw
			...
			0x62, 0xf5, 0x7c, 0x58, 0x7c, 0x48, 0x20, // vcvttph2w
			...
			0x62, 0xf5, 0x7d, 0x58, 0x7c, 0x48, 0x20, // vcvtuw2ph
			...
			0x62, 0xf5, 0x7f, 0x58, 0x7d, 0x48, 0x20, // vcvtw2ph
			...
			0x62, 0xf5, 0x7e, 0x58, 0x7d, 0x48, 0x20, // vcvtps2ph
...
void rao_int()
{
	...
	else
	{
		const uint8_t[] tbl = [
			// aadd
			0x0f, 0x38, 0xfc, 0x08, // aand
			0x66, 0x0f, 0x38, 0xfc, 0x08, // aor
			0xf2, 0x0f, 0x38, 0xfc, 0x08, // axor
			0xf3, 0x0f, 0x38, 0xfc, 0x08,
		];
	}
...
version (XBYAK64)
{
	...
	void CMPccXADD()
	{
		...
		const uint8_t[] tbl = [
			...
			0xc4, 0xa2, 0x69, 0xe4, 0x0c, 0x90, // 64bit reg
	...
	void crypto()
	{
		...
		const uint8_t[] tbl = [
			...
			0xC4, 0xE3, 0x41, 0xDE, 0x29, 0x34, // sm4
