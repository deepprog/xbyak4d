...
size_t VERSION = 0x07401;  // 0xABCD = A.BC(D)
...
enum XBYAK_ERR_LIST = [
    ...
    ["ERR_CODE_ISNOT_COPYABLE", "code is not copyable"],
    ["ERR_BAD_PARAMETER", "bad parameter"],
    ["ERR_CANT_PROTECT", "can`t protect"],
    ["ERR_CANT_USE_64BIT_DISP", "can`t use 64bit disp(use (void*))"],
    ...
    ["ERR_OVER_LOCAL_LABEL", "over local label"], // not used
    ["ERR_UNDER_LOCAL_LABEL", "under local label"],
    ["ERR_CANT_ALLOC", "can`t alloc"],
    ["ERR_ONLY_T_NEAR_IS_SUPPORTED_IN_AUTO_GROW", "T_SHORT is not supported in AutoGrow"],
    ["ERR_BAD_PROTECT_MODE", "bad protect mode"],
    ["ERR_BAD_PNUM", "bad pNum"],
    ["ERR_BAD_TNUM", "bad tNum"],
    ["ERR_BAD_VSIB_ADDRESSING", "bad vsib addressing"],
    ["ERR_CANT_CONVERT", "can`t convert"],
    ["ERR_LABEL_ISNOT_SET_BY_L", "label is not set by L()"],
    ["ERR_LABEL_IS_ALREADY_SET_BY_L", "label is already set by L()"],
    ["ERR_BAD_LABEL_STR", "bad label string"],
    ["ERR_MUNMAP", "err munmap"],
    ["ERR_OPMASK_IS_ALREADY_SET", "opmask is already set"],
    ["ERR_ROUNDING_IS_ALREADY_SET", "rounding is already set"],
    ["ERR_K0_IS_INVALID", "k0 is invalid"],
    ["ERR_EVEX_IS_INVALID", "evex is invalid"],
    ["ERR_SAE_IS_INVALID", "sae(suppress all exceptions) is invalid"],
    ["ERR_ER_IS_INVALID", "er(embedded rounding) is invalid"],
    ["ERR_INVALID_BROADCAST", "invalid broadcast"],
    ["ERR_INVALID_OPMASK_WITH_MEMORY", "invalid opmask with memory"],
    ["ERR_INVALID_ZERO", "invalid zero"],
    ["ERR_INVALID_RIP_IN_AUTO_GROW", "invalid rip in AutoGrow"],
    ["ERR_INVALID_MIB_ADDRESS", "invalid mib address"],
    ["ERR_X2APIC_IS_NOT_SUPPORTED", "x2APIC is not supported"],
    ["ERR_NOT_SUPPORTED", "not supported"],
    ["ERR_SAME_REGS_ARE_INVALID", "same regs are invalid"],
    ["ERR_INVALID_NF", "invalid NF"],
    ["ERR_INVALID_ZU", "invalid ZU"],
    ["ERR_CANT_USE_REX2", "can`t use rex2"],
    ["ERR_INVALID_DFV", "invalid dfv"],
    ["ERR_INVALID_REG_IDX", "invalid reg index"],
    ["ERR_BAD_ENCODING_MODE", "bad encoding mode"],
    ["ERR_CANT_USE_ABCDH", "can`t use [abcd]h with rex"],
    ["ERR_CANT_INIT_CPUTOPOLOGY", "can`t init CpuTopology"],
    ["ERR_INVALID_CPUMASK_INDEX", "invalid cpumask index"],
    ["ERR_INTERNAL", "internal error"] // Put it at last.
];

string make_enum_ERR() {
    string result = "enum ERR { ";
    foreach (xbyak_err; XBYAK_ERR_LIST) {
        result ~= xbyak_err[0] ~ ", ";
    }
    result ~= "}";
    return result;
...
else
{
    class XError : Exception
    {
        ERR err_;
    public:
        this(ERR err = ERR_NONE, string file = __FILE__, size_t line = __LINE__, Throwable next = null)
        {
            err_ = err;
...
version (Posix)
{
    ...
    uint32_t verifyInInt32(uint64_t x)
    {
        ...
        {
            if (!IsInInt32(x)) {
                mixin(XBYAK_THROW_RET(ERR_OFFSET_IS_TOO_BIG, "0"));
            }
        }
    ...
    mixin ImportEnumMembers!AddressMode;
...
version(XBYAK32)
{
    ...
    bool isBSR() const { return isKind(BSR); }
  ...
  else
  {
    ...
    void setRounding(int idx)
    {
        ...
        {
            mixin(XBYAK_THROW(ERR_ROUNDING_IS_ALREADY_SET));
        }
        rounding_ = idx;
    }
...
version (XBYAK64)
{
    ...
    public class Bsr : Reg
    {
    public:
        this(int idx, Kind kind = BSR, int bit = 1024)
        {
            super(idx, kind, bit);
        }
...
} // version (XBYAK_DISABLE_SEGMENT) else
  ...
  else
  {
    ...
    this(Reg r, int scale = 1)
    {
        ...
        if (scale != 1 && scale != 2 && scale != 4 && scale != 8) {
            mixin(XBYAK_THROW(ERR_BAD_SCALE));
        }
    ...
    void verify() const
    {
        ...
        if (index_.getBit() && index_.getBit() <= 64)
        {
            if (index_.getIdx()== Operand.ESP) {
                mixin(XBYAK_THROW(ERR_ESP_CANT_BE_INDEX));
            }
...
public:
    this(size_t maxSize, void* userPtr = null, Allocator allocator = null)
    {
        type_ = (userPtr == AutoGrow) ?
            AUTO_GROW : (userPtr == null || userPtr == DontSetProtectRWE) ?
            ALLOC_BUF : USER_BUF;
...
version(XBYAK_NO_EXCEPTION)
{
    ...
    {
        if (size > maxSize_) {
            mixin(XBYAK_THROW(ERR_OFFSET_IS_TOO_BIG));
        }
        size_ = size;
    }
    void dump() const
...
public:
    ...
    Label opAssign(ref Label rhs)
    {
        if (id)
        {
            mixin(XBYAK_THROW_RET(ERR_LABEL_IS_ALREADY_SET_BY_L, "this"));
        }
        id = rhs.id;
        mgr = rhs.mgr;
        if (mgr)
...
public:
    ...
    void leaveLocal()
    {
        if (stateList_[].walkLength <= 2)
            mixin(XBYAK_THROW(ERR_UNDER_LOCAL_LABEL));
    ...
    {
        if (label == "@b" || label == "@f") {
            mixin(XBYAK_THROW(ERR_BAD_LABEL_STR));
        }
        if (label == "@@") {
    ...
    bool getOffset(size_t* offset, ref string label) //const
    {
        ...
        if (label == "@b") {
            if (defList.Find("@f") != defList.End()) {
                ...
                mixin(XBYAK_THROW_RET(ERR_LABEL_IS_NOT_FOUND, "false"));
            }
        } else if (label == "@f") {
            if (defList.Find("@f") != defList.End()) {
                label = "@b";
...
else
{
    ...
    bool rex(Operand op1, Operand op2 = Reg(), uint64_t type = 0)
    {
        ...
        {
            mixin(XBYAK_THROW_RET(ERR_INVALID_NF, "false"));
        }
        ...
        if (p1.isMEM())
        {
            mixin(XBYAK_THROW_RET(ERR_BAD_COMBINATION, "false"));
        }
...
static const uint64_t T_MAP6 = 6uL << 8;
static const uint64_t T_MAP_MASK = 7uL << 8;
// er/sae field (bit11-13) : an insn has at most one of these
static const uint64_t T_ER_X = 1uL << 11; // xmm{er}
static const uint64_t T_ER_Y = 2uL << 11; // ymm{er}
static const uint64_t T_ER_Z = 3uL << 11; // zmm{er}
static const uint64_t T_ER_R = 4uL << 11; // reg{er}
static const uint64_t T_SAE_X = 5uL << 11; // xmm{sae}
static const uint64_t T_SAE_Y = 6uL << 11; // ymm{sae}
static const uint64_t T_SAE_Z = 7uL << 11; // zmm{sae}
static const uint64_t T_ER_SAE_MASK = 7uL << 11;
static const uint64_t T_W0 = 1uL << 14; // T_EW0 = T_W0
static const uint64_t T_W1 = 1uL << 15; // for VEX
...
static const uint64_t T_ALLOW_ABCDH = 1uL << 30; // allow [abcd]h reg
    ...
    pragma(inline, true);
    uint32_t getMap(uint64_t type) { return cast(uint32_t)((type & T_MAP_MASK) >> 8); }
    ...
    void verifySAE(Reg r, uint64_t type) const
    {
        ...
        mixin(XBYAK_THROW(ERR_SAE_IS_INVALID));
    }
    ...
    void verifyER(Reg r, uint64_t type) const
    {
        ...
        mixin(XBYAK_THROW(ERR_ER_IS_INVALID));
    }
    ...
    int evex(Reg reg, Reg base, Operand v, uint64_t type, int code, Address addr = null)
    {
        ...
        if (addr && addr.isBroadcast()) {
            if (!(type & (T_B32 | T_B64))) {
                mixin(XBYAK_THROW_RET(ERR_INVALID_BROADCAST, "0"));
            }
            b = true;
        }
        if (!(type & T_EVEX_MASK)) {
    ...
    void evexLeg(Reg r, Reg b, Reg x, Reg v, uint64_t type, int sc = NONE)
    {
        ...
        int L = 0;
        if ((type & T_NF) == 0 && NF) {
            mixin(XBYAK_THROW(ERR_INVALID_NF));
        }
        if ((type & T_ZU) == 0 && r.getZU()) {
            mixin(XBYAK_THROW(ERR_INVALID_ZU));
        }
        db(0x62);
...
version (XBYAK64)
{
    ...
    void opRR(Reg r1, Reg r2, uint64_t type, int code)
    {
        ...
        if (!(type & T_ALLOW_ABCDH) && (isBadCombination(r1, r2) || isBadCombination(r2, r1))) {
            mixin(XBYAK_THROW(ERR_CANT_USE_ABCDH));
        }
    ...
    void opMIB(Address addr, Reg reg, uint64_t type, int code)
    {
        if (addr.getMode() != inner.M_ModRM) {
            mixin(XBYAK_THROW(ERR_INVALID_MIB_ADDRESS));
        }
    ...
    void opAddr(Address addr, int reg)
    {
        if (!addr.permitVsib && addr.isVsib()) {
            mixin(XBYAK_THROW(ERR_BAD_VSIB_ADDRESSING));
        }
        if (addr.getMode() == inner.M_ModRM) {
            setSIB(addr, reg);
            ...
            if (addr.getLabel()) { // [rip + Label]
                ...
                if (addr.getMode() == inner.M_ripAddr) {
                    if (isAutoGrow()) {
                        mixin(XBYAK_THROW(ERR_INVALID_RIP_IN_AUTO_GROW));
                    }
...
{
        if (d.isREG()) {
            opROO(d, op, Reg(ext, REG, d.getBit()), T_APX|T_NF|T_ND1|T_OP_W1, 0xFE);
            return;
        }
}
else
...
version(XBYAK64)
{
    ...
    void opFpuFpu(Fpu reg1, Fpu reg2, uint32_t code1, uint32_t code2)
    {
        ...
        if (!code) {
            mixin(XBYAK_THROW(ERR_BAD_ST_COMBINATION));
        }
    ...
    void opVex(Reg r, Operand p1, Operand op2, in uint64_t type, int code, int imm8 = NONE)
    {
        ...
        if (op2.isMEM()) {
            ...
            if (useEvex || (type & T_EVEX_MASK) == T_EVEX_IF_MEM ||
                addr.isBroadcast() || addr.getOpmaskIdx() || addr.hasRex2()
            )
            {
    ...
    // xx_xy_xz
    void opX_XM(Operand op, Xmm x, uint64_t type, uint8_t code)
    {
    ...
    void opCvt_xx_xy_yz(Xmm x, Operand op, uint64_t type, int code, int imm8 = NONE)
    {
        if (!op.isMEM() && !(x.isKind(XMM | YMM) && op.isXMM()) && !(x.isZMM() && op.isYMM())) {
            mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
        }
        opVex(x, null, op, type, code, imm8);
    }
    // (x, x/m), (x, y/m256), (y, z/m) : e.g. vcvtpd2dq
    void opCvt_xx_yx_zy(Xmm x, Operand op, uint64_t type, int code)
    {
        if (!(x.isXMM() && op.isKind(XMM | YMM | MEM)) && !(x.isYMM() && op.isKind(ZMM | MEM)))
        {
            mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
        }
    ...
    void opCvtSi2X(Xmm x1, Xmm x2, Operand op, uint64_t type, uint64_t type64, uint64_t type32, uint8_t code)
    {
    ...
    void opCvt_xx_yx_zy_sized(Xmm x, Operand op, uint64_t type, int code)
    {
    ...
    void opCvt_xx_yx_zx(Xmm x, Operand op, uint64_t type, int code)
    {
    ...
    void opCvtBias(Xmm x1, Xmm x2, Operand op, uint64_t type, int code, bool dstXMM = false)
    {
    ...
    void opCvtX2Si(Reg r, Operand op, uint64_t type, int code)
    {
...
//      const int x_vy_x = 2;
        ...
        if (i1 == i2 || i1 == i3 || i2 == i3) {
            mixin(XBYAK_THROW(ERR_SAME_REGS_ARE_INVALID));
        }
...
version (XBYAK_DISABLE_AVX512)
{
        if (enc == EvexEncoding || enc == AVX10v2Encoding) {
            mixin(XBYAK_THROW_RET(ERR_EVEX_IS_INVALID, VexEncoding));
        }
}
        return enc;
    }
    ...
    {
        verifyDfv(dfv);
        opROO(Reg(15 - dfv, REG, (op1.getBit() | op2.getBit())), op1, op2, T_APX|T_OP_W1, code, 0, sc);
    }
    void opCcmpi(Operand op, int imm, int dfv, int sc)
    {
        verifyDfv(dfv);
        verifyMemHasSize(op);
        opROI(Reg(15 - dfv, REG, op.getBit()), op, imm, T_APX|T_OP_W1, 15, sc);
    }
    void opTesti(Operand op, int imm, int dfv, int sc)
    {
        ...
        opROO(Reg(15 - dfv, REG, opBit), op, Reg(0, REG, opBit), T_APX|T_OP_W1, 0xF6, immBit / 8, sc);
        db(imm, immBit / 8);
    }
...
version (XBYAK64)
{
    enum
    {
        ...
        xm24 = xmm24, xm25 = xmm25, xm26 = xmm26, xm27 = xmm27,
        xm28 = xmm28, xm29 = xmm29, xm30 = xmm30, xm31 = xmm31,
        ...
        ym24 = ymm24, ym25 = ymm25, ym26 = ymm26, ym27 = ymm27,
        ym28 = ymm28, ym29 = ymm29, ym30 = ymm30, ym31 = ymm31,
        ...
        zm24 = zmm24, zm25 = zmm25, zm26 = zmm26, zm27 = zmm27,
        zm28 = zmm28, zm29 = zmm29, zm30 = zmm30, zm31 = zmm31
    }
    ...
    void mov(Operand op, uint64_t imm)
    {
        if (op.isREG()) {
            ...
            if (immSize <= 4) {
                ...
                if (s != 0 && s != -1) {
                    mixin(XBYAK_THROW(ERR_IMM_IS_TOO_BIG));
                }
            } else {
...
version (XBYAK_TEST)
{
    ...
    void setDefaultEncoding(PreferredEncoding enc = EvexEncoding)
    {
        if (enc != VexEncoding && enc != EvexEncoding) {
            mixin(XBYAK_THROW(ERR_BAD_ENCODING_MODE));
        }
    ...
    void vmovd(Operand op1, Operand op2, PreferredEncoding enc = DefaultEncoding)
    {
        uint64_t[4] typeTbl = [
            ...
            T_MUST_EVEX | T_66 | T_0F | T_N4,
            T_MUST_EVEX | T_F3 | T_0F | T_N4, // avx10.2
        ];
    ...
    void vmovw(Operand op1, Operand op2, PreferredEncoding enc = DefaultEncoding)
    {
        uint64_t[4] typeTbl = [
            ...
            T_MUST_EVEX | T_F3 | T_MAP5 | T_N2,
            T_MUST_EVEX | T_F3 | T_MAP5 | T_N2, // avx10.2
        ];
...
else
{
    ...
    */
    void xbyak_align(size_t x = 16, int useMultiByteNop = 2)
    {
        if (x == 1) return;
        if (x < 1 || (x & (x - 1))) {
            mixin(XBYAK_THROW(ERR_BAD_ALIGN));
        }
...
string getVersionString() const { return "0.7401"; }
...
void add(Reg d, Operand op, uint32_t imm) { opROI(d, op, imm, T_NF|T_OP_W1, 0); }
void add(Reg d, Operand op1, Operand op2) { opROO(d, op1, op2, T_NF|T_OP_W1, 0x00); }
...
void and_(Reg d, Operand op, uint32_t imm) { opROI(d, op, imm, T_NF|T_OP_W1, 4); }
void and_(Reg d, Operand op1, Operand op2) { opROO(d, op1, op2, T_NF|T_OP_W1, 0x20); }
...
void div(Operand op) { opRext(op, 0, 6, T_APX|T_NF|T_OP_W1, 0xF6); }
...
void idiv(Operand op) { opRext(op, 0, 7, T_APX|T_NF|T_OP_W1, 0xF6); }
void imul(Operand op) { opRext(op, 0, 5, T_APX|T_NF|T_OP_W1, 0xF6); }
...
void movshdup(Xmm xmm, Operand op) { opSSE(xmm, op, T_F3|T_0F|T_W0|T_YMM|T_EVEX, 0x16, &isXMM_XMMorMEM, NONE); }
void movsldup(Xmm xmm, Operand op) { opSSE(xmm, op, T_F3|T_0F|T_W0|T_YMM|T_EVEX, 0x12, &isXMM_XMMorMEM, NONE); }
...
void mul(Operand op) { opRext(op, 0, 4, T_APX|T_NF|T_OP_W1, 0xF6); }
...
void neg(Operand op) { opRext(op, 0, 3, T_APX|T_NF|T_OP_W1, 0xF6); }
void neg(Reg d, Operand op) { opROO(d, op, Reg(3, REG, d.getBit()), T_APX|T_NF|T_OP_W1|T_ND1, 0xF6); }
void not_(Operand op) { opRext(op, 0, 2, T_APX|T_OP_W1, 0xF6); }
void not_(Reg d, Operand op) { opROO(d, op, Reg(2, REG, d.getBit()), T_APX|T_OP_W1|T_ND1, 0xF6); }

void or_(Operand op, uint32_t imm) { opOI(op, imm, 0x08, 1); }
void or_(Operand op1, Operand op2) { opRO_MR(op1, op2, 0x08); }
void or_(Reg d, Operand op, uint32_t imm) { opROI(d, op, imm, T_NF|T_OP_W1, 1); }
void or_(Reg d, Operand op1, Operand op2) { opROO(d, op1, op2, T_NF|T_OP_W1, 0x08); }
...
void pmovsxdq(Xmm xmm, Operand op)
 { opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX, 0x25, &isXMM_XMMorMEM, NONE); }
...
void pmovzxdq(Xmm xmm, Operand op)
 { opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX, 0x35, &isXMM_XMMorMEM, NONE); }
...
void sub(Reg d, Operand op, uint32_t imm) { opROI(d, op, imm, T_NF|T_OP_W1, 5); }
void sub(Reg d, Operand op1, Operand op2) { opROO(d, op1, op2, T_NF|T_OP_W1, 0x28); }
...
void vaddps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x58); }
...
void vaddss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_ER_X | T_N4, 0x58); }
...
void vandnps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x55); }
...
void vandps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x54); }
...
void vcomiss(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_N4|T_0F|T_W0|T_EVEX|T_SAE_X, 0x2F); }
void vcvtdq2pd(Xmm x, Operand op)
 { opCvt_xx_xy_yz(x, op, T_0F | T_F3 | T_YMM | T_EVEX | T_W0 | T_B32 | T_N8 | T_N_VL, 0xE6); }
void vcvtdq2ps(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_0F|T_W0|T_YMM|T_EVEX|T_ER_Z|T_B32, 0x5B); }
...
void vcvtneps2bf16(Xmm x, Operand op, PreferredEncoding encoding = DefaultEncoding)
 { opCvt_xx_yx_zy(x, op, T_F3|T_0F38|T_W0|T_YMM|T_SAE_Z|T_B32|orEvexIf(encoding, 0, T_MUST_EVEX, 0), 0x72); }
void vcvtpd2dq(Xmm x, Operand op)
 { opCvt_xx_yx_zy(x, op, T_0F | T_F2 | T_YMM | T_EVEX | T_EW1 | T_B64 | T_ER_Z, 0xE6); }
void vcvtpd2ps(Xmm x, Operand op)
 { opCvt_xx_yx_zy(x, op, T_0F | T_66 | T_YMM | T_EVEX | T_EW1 | T_B64 | T_ER_Z, 0x5A); }
void vcvtph2ps(Xmm x, Operand op)
 { opCvt_xx_xy_yz(x, op, T_0F38 | T_66 | T_W0 | T_EVEX | T_N8 | T_N_VL | T_SAE_Y, 0x13); }
void vcvtps2dq(Xmm xm, Operand op)
 { opAVX_X_XM_IMM(xm, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_ER_Z|T_B32, 0x5B); }
void vcvtps2pd(Xmm x, Operand op)
 { opCvt_xx_xy_yz(x, op, T_0F | T_YMM | T_EVEX | T_W0 | T_B32 | T_N8 | T_N_VL | T_SAE_Y, 0x5A); }
void vcvtps2ph(Operand op, Xmm x, uint8_t imm)
 { opCvt_xx_xy_yz(x, op, T_0F3A | T_66 | T_W0 | T_EVEX | T_N8 | T_N_VL | T_SAE_Y | T_M_K, 0x1D, imm); }
void vcvtsd2si(Reg32 r, Operand op)
 { opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, T_0F | T_F2 | T_W0 | T_EVEX | T_N4 | T_ER_X, 0x2D); }
...
void vcvtsi2sd(Xmm x1, Xmm x2, Operand op)
 { opCvtSi2X(x1, x2, op, T_0F | T_F2 | T_EVEX, T_W1 | T_EW1 | T_ER_R | T_N8, T_W0 | T_N4, 0x2A); }
void vcvtsi2ss(Xmm x1, Xmm x2, Operand op)
 { opCvtSi2X(x1, x2, op, T_0F | T_F3 | T_EVEX | T_ER_R, T_W1 | T_EW1 | T_N8, T_W0 | T_N4, 0x2A); }
void vcvtss2sd(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4|T_F3|T_0F|T_W0|T_EVEX|T_SAE_X, 0x5A); }
void vcvtss2si(Reg32 r, Operand op)
 { opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, T_0F | T_F3 | T_W0 | T_EVEX | T_ER_X | T_N8, 0x2D); }
void vcvttpd2dq(Xmm x, Operand op)
 { opCvt_xx_yx_zy(x, op, T_66 | T_0F | T_YMM | T_EVEX |T_EW1 | T_B64 | T_SAE_Z, 0xE6); }
void vcvttps2dq(Xmm xm, Operand op)
 { opAVX_X_XM_IMM(xm, op, T_F3|T_0F|T_W0|T_YMM|T_EVEX|T_SAE_Z|T_B32, 0x5B); }
void vcvttsd2si(Reg32 r, Operand op)
 { opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, T_0F | T_F2 | T_W0 | T_EVEX | T_N4 | T_SAE_X, 0x2C); }
void vcvttss2si(Reg32 r, Operand op)
 { opAVX_X_X_XM(Xmm(r.getIdx()), xm0, op, T_0F | T_F3 | T_W0 | T_EVEX | T_SAE_X | T_N8, 0x2C); }
...
void vdivps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x5E); }
...
void vdivss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_ER_X | T_N4, 0x5E); }
...
void vfmadd132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x98); }
...
void vfmadd132ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0x99); }
...
void vfmadd213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA8); }
...
void vfmadd213ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xA9); }
...
void vfmadd231ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB8); }
...
void vfmadd231ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xB9); }
...
void vfmaddsub132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x96); }
...
void vfmaddsub213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA6); }
...
void vfmaddsub231ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB6); }
...
void vfmsub132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9A); }
...
void vfmsub132ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0x9B); }
...
void vfmsub213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAA); }
...
void vfmsub213ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xAB); }
...
void vfmsub231ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBA); }
...
void vfmsub231ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xBB); }
...
void vfmsubadd132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x97); }
...
void vfmsubadd213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA7); }
...
void vfmsubadd231ps(Xmm x1, Xmm x2, Operand op)
 {opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB7); }
...
void vfnmadd132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9C); }
...
void vfnmadd132ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0x9D); }
...
void vfnmadd213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAC); }
...
void vfnmadd213ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xAD); }
...
void vfnmadd231ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBC); }
...
void vfnmadd231ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xBD); }
...
void vfnmsub132ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9E); }
...
void vfnmsub132ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0x9F); }
...
void vfnmsub213ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAE); }
...
void vfnmsub213ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xAF); }
...
void vfnmsub231ps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBE); }
...
void vfnmsub231ss(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F38 | T_W0 | T_EVEX | T_ER_X, 0xBF); }
...
void vgf2p8mulb(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_SAE_Z, 0xCF); }
...
void vmaxps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_SAE_Z | T_B32, 0x5F); }
...
void vmaxss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_SAE_X | T_N4, 0x5F); }
...
void vminps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_SAE_Z | T_B32, 0x5D); }
...
void vminss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_SAE_X | T_N4, 0x5D); }
...
void vmovaps(Address addr, Xmm xmm)
 { opAVX_X_XM_IMM(xmm, addr, T_0F|T_W0|T_YMM|T_EVEX|T_M_K, 0x29); }
void vmovaps(Xmm xm, Operand op)
 { opAVX_X_XM_IMM(xm, op, T_0F|T_W0|T_YMM|T_EVEX, 0x28); }
...
void vmovhlps(Xmm x1, Xmm x2, Operand op = Operand())
{
    if (!op.isNone() && !op.isXMM())
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opAVX_X_X_XM(x1, x2, op, T_0F | T_EVEX | T_W0, 0x12);
}
...
void vmovhps(Address addr, Xmm x)
 { opAVX_X_X_XM(x, xm0, addr, T_N8|T_0F|T_W0|T_EVEX, 0x17); }
void vmovhps(Xmm x, Operand op1, Operand op2 = Operand())
{
    if (!op2.isNone() && !op2.isMEM())
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opAVX_X_X_XM(x, op1, op2, T_N8|T_0F|T_W0|T_EVEX, 0x16);
}
void vmovlhps(Xmm x1, Xmm x2, Operand op = Operand())
{
    if (!op.isNone() && !op.isXMM())
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opAVX_X_X_XM(x1, x2, op, T_0F | T_EVEX | T_W0, 0x16);
}
...
void vmovlps(Address addr, Xmm x)
 { opAVX_X_X_XM(x, xm0, addr, T_N8|T_0F|T_W0|T_EVEX, 0x13); }
void vmovlps(Xmm x, Operand op1, Operand op2 = Operand())
{
    if (!op2.isNone() && !op2.isMEM())
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opAVX_X_X_XM(x, op1, op2, T_N8 | T_0F | T_W0 | T_EVEX, 0x12);
}
...
void vmovntdq(Address addr, Xmm x) { opVex(x, null, addr, T_0F | T_66 | T_YMM | T_EVEX | T_W0, 0xE7); }
void vmovntdqa(Xmm x, Address addr) { opVex(x, null, addr, T_0F38 | T_66 | T_YMM | T_EVEX | T_W0, 0x2A); }
void vmovntpd(Address addr, Xmm x) { opVex(x, null, addr, T_0F | T_66 | T_YMM | T_EVEX | T_EW1, 0x2B); }
void vmovntps(Address addr, Xmm x) { opVex(x, null, addr, T_0F | T_YMM | T_EVEX | T_W0, 0x2B); }
...
void vmovshdup(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_F3|T_0F|T_W0|T_YMM|T_EVEX, 0x16); }
void vmovsldup(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_F3|T_0F|T_W0|T_YMM|T_EVEX, 0x12); }
void vmovss(Address addr, Xmm x) { opAVX_X_X_XM(x, xm0, addr, T_N4|T_F3|T_0F|T_W0|T_EVEX | T_M_K, 0x11); }
void vmovss(Xmm x, Address addr) { opAVX_X_X_XM(x, xm0, addr, T_N4|T_F3|T_0F|T_W0|T_EVEX, 0x10); }
void vmovss(Xmm x1, Xmm x2, Operand op = Operand())
{
    if (!op.isNone() && !op.isXMM())
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opAVX_X_X_XM(x1, x2, op, T_N4 | T_F3 | T_0F | T_W0 | T_EVEX, 0x10);
}
...
void vmovups(Address addr, Xmm xmm) { opAVX_X_XM_IMM(xmm, addr, T_0F|T_W0|T_YMM|T_EVEX|T_M_K, 0x11); }
void vmovups(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_0F|T_W0|T_YMM|T_EVEX, 0x10); }
...
void vmulps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x59); }
...
void vmulss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_ER_X | T_N4, 0x59); }
...
void vorps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x56); }
void vpabsb(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_66|T_0F38|T_YMM|T_EVEX, 0x1C); }
void vpabsd(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x1E); }
void vpabsw(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_66|T_0F38|T_YMM|T_EVEX, 0x1D); }
void vpackssdw(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x6B); }
void vpacksswb(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_YMM|T_EVEX, 0x63); }
void vpackusdw(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x2B); }
...
void vpaddd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0xFE); }
...
void vpdpbusd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
 { opEncoding(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_SAE_Z | T_B32, 0x50, encoding); }
void vpdpbusds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
 { opEncoding(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_SAE_Z | T_B32, 0x51, encoding); }
void vpdpwssd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
 { opEncoding(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_SAE_Z | T_B32, 0x52, encoding); }
void vpdpwssds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
 { opEncoding(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_SAE_Z | T_B32, 0x53, encoding); }
...
void vpermd(Ymm y1, Ymm y2, Operand op)
 { opAVX_X_X_XM(y1, y2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_B32, 0x36); }
...
void vpermilps(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_YMM | T_EVEX | T_B32, 0x0C); }
void vpermilps(Xmm xm, Operand op, uint8_t imm)
 { opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A | T_W0 | T_YMM | T_EVEX | T_B32, 0x04, imm); }
...
void vpermps(Ymm y1, Ymm y2, Operand op)
 { opAVX_X_X_XM(y1, y2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x16); }
...
void vpextrd(Operand op, Xmm x, uint8_t imm)
{
    if (!((op.isREG(32) || op.isMEM()) && x.isXMM()))
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opVex(x, null, op, T_0F3A | T_66 | T_W0 | T_EVEX | T_N4, 0x16, imm);
}
...
void vpextrw(Operand op, Xmm x, uint8_t imm)
{
    if (!((op.isREG(16|i32e) || op.isMEM()) && x.isXMM()))
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    if (op.isREG() && x.getIdx() < 16 && op.getIdx() < 16)
    {
...
void vpinsrd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
{
    if (!(x1.isXMM() && x2.isXMM() && (op.isREG(32) || op.isMEM())))
        mixin(XBYAK_THROW(ERR_BAD_COMBINATION));
    opVex(x1, x2, op, T_0F3A | T_66 | T_W0 | T_EVEX | T_N4, 0x22, imm);
}
...
void vpmaxsd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x3D); }
...
void vpmaxud(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x3F); }
...
void vpminsd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x39); }
...
void vpminud(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x3B); }
...
void vpmovsxdq(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_N8|T_N_VL|T_66|T_0F38|T_W0|T_YMM|T_EVEX, 0x25); }
...
void vpmovzxdq(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_N8|T_N_VL|T_66|T_0F38|T_W0|T_YMM|T_EVEX, 0x35); }
...
void vpmulld(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x40); }
...
void vpshufd(Xmm xm, Operand op, uint8_t imm) { opAVX_X_XM_IMM(xm, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x70, imm); }
...
void vpsllvd(Xmm x1, Xmm x2, Operand op)
 { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_EVEX|T_B32, 0x47); }
...
void vpunpckhdq(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x6A); }
...
void vpunpckldq(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x62); }
...
void vshufps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
 { opAVX_X_X_XM(x1, x2, op, T_0F | T_W0 | T_YMM | T_EVEX | T_B32, 0xC6, imm); }
void vsm3msg1(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_0F38|T_W0|T_EVEX, 0xDA); }
void vsm3msg2(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_EVEX, 0xDA); }
void vsm3rnds2(Xmm x1, Xmm x2, Operand op, uint8_t imm)
 { opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_W0 | T_EVEX, 0xDE, imm); }
void vsm4key4(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_F3|T_0F38|T_W0|T_EVEX, 0xDA); }
void vsm4rnds4(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_F2|T_0F38|T_W0|T_EVEX, 0xDA); }
void vsqrtpd(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_66|T_0F|T_EW1|T_YMM|T_EVEX|T_ER_Z|T_B64, 0x51); }
void vsqrtps(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_0F|T_W0|T_YMM|T_EVEX|T_ER_Z|T_B32, 0x51); }
void vsqrtsd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_N8|T_F2|T_0F|T_EW1|T_EVEX|T_ER_X, 0x51); }
void vsqrtss(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_N4|T_F3|T_0F|T_W0|T_EVEX|T_ER_X, 0x51); }
...
void vsubps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x5C); }
...
void vsubss(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_W0 | T_EVEX | T_ER_X | T_N4, 0x5C); }
...
void vucomiss(Xmm xm, Operand op) { opAVX_X_XM_IMM(xm, op, T_N4|T_0F|T_W0|T_EVEX|T_SAE_X, 0x2E); }
void vunpckhpd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_EW1|T_YMM|T_EVEX|T_B64, 0x15); }
void vunpckhps(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x15); }
void vunpcklpd(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_EW1|T_YMM|T_EVEX|T_B64, 0x14); }
void vunpcklps(Xmm x1, Xmm x2, Operand op) { opAVX_X_X_XM(x1, x2, op, T_0F|T_W0|T_YMM|T_EVEX|T_B32, 0x14); }
...
void vxorps(Xmm xmm, Operand op1, Operand op2 = Operand())
 { opAVX_X_X_XM(xmm, op1, op2, T_0F | T_W0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x57); }
...
void xor_(Reg d, Operand op, uint32_t imm) { opROI(d, op, imm, T_NF|T_OP_W1, 6); }
void xor_(Reg d, Operand op1, Operand op2) { opROO(d, op1, op2, T_NF|T_OP_W1, 0x30); }
...
version (XBYAK64)
{
    ...
    void vmovq(Xmm x, Reg64 r) { opAVX_X_X_XM(x, xm0, r, T_66 | T_0F | T_W1 | T_EVEX | T_EW1, 0x6E); }
    void vmovq(Reg64 r, Xmm x) { opAVX_X_X_XM(x, xm0, r, T_66 | T_0F | T_W1 | T_EVEX | T_EW1, 0x7E); }
...
else
{
    ...
    void vaddph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_W0 | T_YMM | T_MUST_EVEX | T_ER_Z | T_B16, 0x58); }
    void vaddsh(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_F3 | T_W0 | T_MUST_EVEX | T_ER_X | T_N2, 0x58); }
    void valignd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX, 0x03, imm); }
    ...
    void vblendmps(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x65); }
    void vbroadcastf32x2(Ymm y, Operand op)
     { opAVX_X_XM_IMM(y, op, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N8, 0x19); }
    void vbroadcastf32x4(Ymm y, Address addr)
     { opAVX_X_XM_IMM(y, addr, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N16, 0x1A); }
    void vbroadcastf32x8(Zmm y, Address addr)
     { opAVX_X_XM_IMM(y, addr, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N32, 0x1B); }
    ...
    void vbroadcasti32x2(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N8, 0x59); }
    void vbroadcasti32x4(Ymm y, Operand op)
     { opAVX_X_XM_IMM(y, op, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N16, 0x5A); }
    void vbroadcasti32x8(Zmm z, Operand op)
     { opAVX_X_XM_IMM(z, op, T_66 | T_0F38 | T_YMM | T_MUST_EVEX | T_W0 | T_N32, 0x5B); }
    ...
    void vcmpbf16(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opVex(k, x, op, T_MUST_EVEX|T_F2|T_0F3A|T_W0|T_YMM|T_B16, 0xC2, imm); }
    ...
    void vcmpph(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0xC2, imm); }
    void vcmpps(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_0F|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0xC2, imm); }
    ...
    void vcmpsh(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_N2|T_F3|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0xC2, imm); }
    void vcmpss(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_N4|T_F3|T_0F|T_W0|T_SAE_Z|T_MUST_EVEX, 0xC2, imm); }
    ...
    void vcvt2ph2bf8(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N16|T_N_VL|T_F2|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvt2ph2bf8s(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N16|T_N_VL|T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvt2ph2hf8(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N16|T_N_VL|T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x18); }
    void vcvt2ph2hf8s(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N16|T_N_VL|T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x1B); }
    void vcvt2ps2phx(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x67); }
    void vcvtbf162ibs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x69); }
    void vcvtbf162iubs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x6B); }
    void vcvtbiasph2bf8(Xmm x1, Xmm x2, Operand op)
     { opCvtBias(x1, x2, op, T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvtbiasph2bf8s(Xmm x1, Xmm x2, Operand op)
     { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvtbiasph2hf8(Xmm x1, Xmm x2, Operand op)
     { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x18); }
    void vcvtbiasph2hf8s(Xmm x1, Xmm x2, Operand op)
     { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x1B); }
    void vcvtdq2ph(Xmm x, Operand op)
     { opCvt_xx_yx_zy_sized(x, op, T_N16|T_N_VL|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x5B); }
    void vcvthf82ph(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_MUST_EVEX|T_F2|T_MAP5|T_W0|T_YMM|T_N8|T_N_VL, 0x1E); }
    void vcvtne2ps2bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_F2|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x72); }
    void vcvtpd2ph(Xmm x, Operand op)
     { opCvt_xx_yx_zx(x, op, T_N16|T_N_VL|T_66|T_MAP5|T_EW1|T_ER_Z|T_MUST_EVEX|T_B64, 0x5A); }
    ...
    void vcvtpd2udq(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_0F|T_EW1|T_YMM|T_ER_Z|T_MUST_EVEX|T_B64, 0x79); }
    ...
    void vcvtph2bf8(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvtph2bf8s(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x74); }
    void vcvtph2dq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_ER_Y|T_MUST_EVEX|T_B16, 0x5B); }
    void vcvtph2hf8(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x18); }
    void vcvtph2hf8s(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x1B); }
    ...
    void vcvtph2pd(Xmm x, Operand op)
     { opX_XM(op, x, T_N4|T_N_VL|T_MAP5|T_W0|T_YMM|T_SAE_X|T_MUST_EVEX|T_B16, 0x5A); }
    void vcvtph2psx(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_MAP6|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B16, 0x13); }
    void vcvtph2qq(Xmm x, Operand op)
     { opX_XM(op, x, T_N4|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_ER_X|T_MUST_EVEX|T_B16, 0x7B); }
    void vcvtph2udq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_MAP5|T_W0|T_YMM|T_ER_Y|T_MUST_EVEX|T_B16, 0x79); }
    void vcvtph2uqq(Xmm x, Operand op)
     { opX_XM(op, x, T_N4|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_ER_X|T_MUST_EVEX|T_B16, 0x79); }
    void vcvtph2uw(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x7D); }
    void vcvtph2w(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x7D); }
    void vcvtps2ibs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x69); }
    void vcvtps2iubs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x6B); }
    void vcvtps2phx(Xmm x, Operand op)
     { opCvt_xx_yx_zy_sized(x, op, T_N16|T_N_VL|T_66|T_MAP5|T_W0|T_ER_Z|T_MUST_EVEX|T_B32, 0x1D); }
    void vcvtps2qq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_0F|T_W0|T_YMM|T_ER_Y|T_MUST_EVEX|T_B32, 0x7B); }
    void vcvtps2udq(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_0F|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x79); }
    void vcvtps2uqq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_0F|T_W0|T_YMM|T_ER_Y|T_MUST_EVEX|T_B32, 0x79); }
    ...
    void vcvtqq2ph(Xmm x, Operand op)
     { opCvt_xx_yx_zx(x, op, T_N16|T_N_VL|T_MAP5|T_EW1|T_ER_Z|T_MUST_EVEX|T_B64, 0x5B); }
    void vcvtqq2ps(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_0F|T_EW1|T_YMM|T_ER_Z|T_MUST_EVEX|T_B64, 0x5B); }
    ...
    {
        uint64_t type = (T_N8|T_F2|T_0F|T_ER_X|T_MUST_EVEX)|(r.isREG(64) ? T_EW1 : T_W0);
    ...
    void vcvtsh2si(Reg32e r, Operand op)
     { opCvtX2Si(r, op, T_N2|T_F3|T_MAP5|T_ER_X|T_MUST_EVEX, 0x2D); }
    void vcvtsh2ss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_MAP6|T_W0|T_SAE_X|T_MUST_EVEX, 0x13); }
    void vcvtsh2usi(Reg32e r, Operand op)
     { opCvtX2Si(r, op, T_N2|T_F3|T_MAP5|T_ER_X|T_MUST_EVEX, 0x79); }
    void vcvtsi2sh(Xmm x1, Xmm x2, Operand op)
     { opCvtSi2X(x1, x2, op, T_F3|T_MAP5|T_ER_R|T_MUST_EVEX|T_M_K, T_EW1 | T_N8, T_W0 | T_N4, 0x2A); }
    void vcvtss2sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_MAP5|T_W0|T_ER_X|T_MUST_EVEX, 0x1D); }
    void vcvtss2usi(Reg32e r, Operand op)
     { opCvtX2Si(r, op, T_N4|T_F3|T_0F|T_ER_X|T_MUST_EVEX, 0x79); }
    void vcvttbf162ibs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x68); }
    void vcvttbf162iubs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x6A); }
    void vcvttpd2dqs(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_MAP5|T_EW1|T_YMM|T_SAE_Y|T_SAE_Z|T_MUST_EVEX|T_B64, 0x6D); }
    ...
    void vcvttpd2udq(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_0F|T_EW1|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B64, 0x78); }
    void vcvttpd2udqs(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_MAP5|T_EW1|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B64, 0x6C); }
    ...
    void vcvttpd2uqqs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_EW1|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B64, 0x6C); }
    void vcvttph2dq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_MAP5|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B16, 0x5B); }
    ...
    void vcvttph2qq(Xmm x, Operand op)
     { opX_XM(op, x, T_N4|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_SAE_X|T_MUST_EVEX|T_B16, 0x7A); }
    void vcvttph2udq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_MAP5|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B16, 0x78); }
    void vcvttph2uqq(Xmm x, Operand op)
     { opX_XM(op, x, T_N4|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_SAE_X|T_MUST_EVEX|T_B16, 0x78); }
    void vcvttph2uw(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x7C); }
    void vcvttph2w(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x7C); }
    void vcvttps2dqs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x6D); }
    void vcvttps2ibs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x68); }
    void vcvttps2iubs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x6A); }
    void vcvttps2qq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_0F|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B32, 0x7A); }
    void vcvttps2qqs(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B32, 0x6D); }
    void vcvttps2udq(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_0F|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x78); }
    void vcvttps2udqs(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP5|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x6C); }
    void vcvttps2uqq(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_0F|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B32, 0x78); }
    void vcvttps2uqqs(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_66|T_MAP5|T_W0|T_YMM|T_SAE_Y|T_MUST_EVEX|T_B32, 0x6C); }
    void vcvttsd2sis(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N8|T_F2|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x6D); }
    void vcvttsd2usi(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N8|T_F2|T_0F|T_SAE_X|T_MUST_EVEX, 0x78); }
    void vcvttsd2usis(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N8|T_F2|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x6C); }
    void vcvttsh2si(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N2|T_F3|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x2C); }
    void vcvttsh2usi(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N2|T_F3|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x78); }
    void vcvttss2sis(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N4|T_F3|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x6D); }
    void vcvttss2usi(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N4|T_F3|T_0F|T_SAE_X|T_MUST_EVEX, 0x78); }
    void vcvttss2usis(Reg32e r, Operand op) { opCvtX2Si(r, op, T_N4|T_F3|T_MAP5|T_SAE_X|T_MUST_EVEX, 0x6C); }
    void vcvtudq2pd(Xmm x, Operand op)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x7A); }
    void vcvtudq2ph(Xmm x, Operand op)
     { opCvt_xx_yx_zy_sized(x, op, T_N16|T_N_VL|T_F2|T_MAP5|T_W0|T_ER_Z|T_MUST_EVEX|T_B32, 0x7A); }
    void vcvtudq2ps(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_0F|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x7A); }
    ...
    void vcvtuqq2ph(Xmm x, Operand op)
     { opCvt_xx_yx_zx(x, op, T_N16|T_N_VL|T_F2|T_MAP5|T_EW1|T_ER_Z|T_MUST_EVEX|T_B64, 0x7A); }
    void vcvtuqq2ps(Xmm x, Operand op)
     { opCvt_xx_yx_zy(x, op, T_F2|T_0F|T_EW1|T_YMM|T_ER_Z|T_MUST_EVEX|T_B64, 0x7A); }
    void vcvtusi2sd(Xmm x1, Xmm x2, Operand op)
     { opCvtSi2X(x1, x2, op, T_F2|T_0F|T_MUST_EVEX, T_W1|T_EW1|T_ER_R|T_N8, T_W0|T_N4, 0x7B); }
    void vcvtusi2sh(Xmm x1, Xmm x2, Operand op)
     { opCvtSi2X(x1, x2, op, T_F3|T_MAP5|T_ER_R|T_MUST_EVEX|T_M_K, T_EW1 | T_N8, T_W0 | T_N4, 0x7B); }
    void vcvtusi2ss(Xmm x1, Xmm x2, Operand op)
     { opCvtSi2X(x1, x2, op, T_F3|T_0F|T_MUST_EVEX|T_ER_R, T_W1|T_EW1|T_N8, T_W0|T_N4, 0x7B); }
    void vcvtuw2ph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F2|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x7D); }
    void vcvtw2ph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x7D); }
    void vdbpsadbw(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX, 0x42, imm); }
    void vdivbf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x5E); }
    void vdivph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_ER_Z|T_B16, 0x5E); }
    void vdivsh(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_F3|T_W0|T_MUST_EVEX|T_ER_X|T_N2, 0x5E); }
    void vdpbf16ps(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_F3|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x52); }
    ...
    void vfixupimmps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x54, imm); }
    ...
    void vfixupimmss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F3A|T_W0|T_SAE_Z|T_MUST_EVEX, 0x55, imm); }
    void vfmadd132bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x98); }
    void vfmadd132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x98); }
    void vfmadd132sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0x99); }
    void vfmadd213bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xA8); }
    void vfmadd213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xA8); }
    void vfmadd213sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xA9); }
    void vfmadd231bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xB8); }
    void vfmadd231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xB8); }
    void vfmadd231sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xB9); }
    void vfmaddcph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_F3|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x56); }
    void vfmaddsub132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x96); }
    void vfmaddsub213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xA6); }
    void vfmaddsub231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xB6); }
    void vfmsub132bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x9A); }
    void vfmsub132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x9A); }
    void vfmsub132sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0x9B); }
    void vfmsub213bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xAA); }
    void vfmsub213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xAA); }
    void vfmsub213sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xAB); }
    void vfmsub231bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xBA); }
    void vfmsub231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xBA); }
    void vfmsub231sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xBB); }
    void vfmsubadd132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x97); }
    void vfmsubadd213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xA7); }
    void vfmsubadd231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xB7); }
    void vfmulcph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_F3|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0xD6); }
    void vfnmadd132bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x9C); }
    void vfnmadd132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x9C); }
    void vfnmadd132sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0x9D); }
    void vfnmadd213bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xAC); }
    void vfnmadd213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xAC); }
    void vfnmadd213sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xAD); }
    void vfnmadd231bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xBC); }
    void vfnmadd231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xBC); }
    void vfnmadd231sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xBD); }
    void vfnmsub132bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x9E); }
    void vfnmsub132ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x9E); }
    void vfnmsub132sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0x9F); }
    void vfnmsub213bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xAE); }
    void vfnmsub213ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xAE); }
    void vfnmsub213sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xAF); }
    void vfnmsub231bf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0xBE); }
    void vfnmsub231ph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0xBE); }
    void vfnmsub231sh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0xBF); }
    void vfpclassbf16(Opmask k, Operand op, uint8_t imm)
     { opVex(k.changeBit(op.getBit()), null, op, T_MUST_EVEX|T_F2|T_0F3A|T_W0|T_YMM|T_B16, 0x66, imm); }
    void vfpclasspd(Opmask k, Operand op, uint8_t imm)
    {
        if (!op.isBit(128|256|512))
            mixin(XBYAK_THROW(ERR_BAD_MEM_SIZE));
        opVex(k.changeBit(op.getBit()), null, op, T_66|T_0F3A|T_MUST_EVEX|T_YMM|T_EW1|T_B64, 0x66, imm);
    }
    void vfpclassph(Opmask k, Operand op, uint8_t imm)
    {
        if (!op.isBit(128|256|512))
            mixin(XBYAK_THROW(ERR_BAD_MEM_SIZE));
        opVex(k.changeBit(op.getBit()), null, op, T_0F3A|T_MUST_EVEX|T_YMM|T_W0|T_B16, 0x66, imm);
    }
    void vfpclassps(Opmask k, Operand op, uint8_t imm)
    {
        if (!op.isBit(128|256|512))
            mixin(XBYAK_THROW(ERR_BAD_MEM_SIZE));
        opVex(k.changeBit(op.getBit()), null, op, T_66|T_0F3A|T_MUST_EVEX|T_YMM|T_W0|T_B32, 0x66, imm);
    }
    ...
    void vfpclasssh(Opmask k, Operand op, uint8_t imm)
    {
        if (!op.isXMEM())
            mixin(XBYAK_THROW(ERR_BAD_MEM_SIZE));
        opVex(k, null, op, T_0F3A|T_MUST_EVEX|T_W0|T_N2, 0x67, imm);
    }
    void vfpclassss(Opmask k, Operand op, uint8_t imm)
    {
        if (!op.isXMEM())
            mixin(XBYAK_THROW(ERR_BAD_MEM_SIZE));
        opVex(k, null, op, T_66|T_0F3A|T_MUST_EVEX|T_W0|T_N4, 0x67, imm);
    }
    ...
    void vgatherqps(Xmm x, Address addr)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_VSIB, 0x93, 2); }
    void vgetexpbf16(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x42); }
    ...
    void vgetexpph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP6|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x42); }
    void vgetexpps(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x42); }
    ...
    void vgetexpsh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_SAE_X|T_MUST_EVEX, 0x43); }
    void vgetexpss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_SAE_X|T_MUST_EVEX, 0x43); }
    void vgetmantbf16(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_F2|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x26, imm); }
    ...
    void vgetmantph(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x26, imm); }
    void vgetmantps(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x26, imm); }
    ...
    void vgetmantsh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x27, imm); }
    ...
    void vmaxbf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x5F); }
    void vmaxph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_SAE_Z|T_B16, 0x5F); }
    void vmaxsh(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_F3|T_W0|T_MUST_EVEX|T_SAE_X|T_N2, 0x5F); }
    ...
    void vminmaxph(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x52, imm); }
    void vminmaxps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x52, imm); }
    ...
    void vminmaxsh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x53, imm); }
    void vminmaxss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x53, imm); }
    void vminph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_SAE_Z|T_B16, 0x5D); }
    ...
    void vmovsh(Xmm x, Address addr)
     { opAVX_X_X_XM(x, xm0, addr, T_N2|T_F3|T_MAP5|T_W0|T_MUST_EVEX, 0x10); }
    void vmovsh(Xmm x1, Xmm x2, Xmm x3)
     { opAVX_X_X_XM(x1, x2, x3, T_N2|T_F3|T_MAP5|T_W0|T_MUST_EVEX, 0x10); }
    void vmpsadbw(Xmm x1, Xmm x2, Operand op, uint8_t imm, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_0F3A|T_YMM, 0x42, encoding, imm, T_66|T_W0|T_YMM, T_F3|T_0F3A|T_W0|T_B32, 1); }
    void vmulbf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x59); }
    void vmulph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_ER_Z|T_B16, 0x59); }
    void vmulsh(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_F3|T_W0|T_MUST_EVEX|T_ER_X|T_N2, 0x59); }
    void vp2intersectd(Opmask k, Xmm x, Operand op)
    {
        if (k.getOpmaskIdx() != 0)
            mixin(XBYAK_THROW(ERR_OPMASK_IS_ALREADY_SET));
        opAVX_K_X_XM(k, x, op, T_F2|T_0F38|T_YMM|T_EVEX|T_W0|T_B32, 0x68);
    }
    ...
    void vpblendmb(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x66); }
    void vpblendmd(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x64); }
    ...
    void vpbroadcastb(Xmm x, Reg8 r) { opVex(x, null, r, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x7A); }
    void vpbroadcastd(Xmm x, Reg32 r) { opVex(x, null, r, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x7C); }
    void vpbroadcastmb2q(Xmm x, Opmask k) { opVex(x, null, k, T_F3|T_0F38|T_YMM|T_MUST_EVEX|T_EW1, 0x2A); }
    void vpbroadcastmw2d(Xmm x, Opmask k) { opVex(x, null, k, T_F3|T_0F38|T_YMM|T_MUST_EVEX|T_W0, 0x3A); }
    void vpbroadcastw(Xmm x, Reg16 r) { opVex(x, null, r, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x7B); }
    void vpcmpb(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX, 0x3F, imm); }
    void vpcmpd(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x1F, imm); }
    ...
    void vpcmpgtd(Opmask k, Xmm x, Operand op) { opAVX_K_X_XM(k, x, op, T_66|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x66); }
    ...
    void vpcmpub(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX, 0x3E, imm); }
    void vpcmpud(Opmask k, Xmm x, Operand op, uint8_t imm)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x1E, imm); }
    ...
    void vpcompressb(Operand op, Xmm x)
     { opAVX_X_XM_IMM(x, op, T_N1|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x63); }
    void vpcompressd(Operand op, Xmm x)
     { opAVX_X_XM_IMM(x, op, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x8B); }
    void vpcompressq(Operand op, Xmm x)
     { opAVX_X_XM_IMM(x, op, T_N8|T_66|T_0F38|T_EW1|T_YMM|T_MUST_EVEX|T_M_K, 0x8B); }
    void vpcompressw(Operand op, Xmm x)
     { opAVX_X_XM_IMM(x, op, T_N2|T_66|T_0F38|T_EW1|T_YMM|T_MUST_EVEX|T_M_K, 0x63); }
    void vpconflictd(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0xC4); }
    ...
    void vpdpbssd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F2|T_0F38|T_YMM, 0x50, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpbssds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F2|T_0F38|T_YMM, 0x51, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpbsud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F3|T_0F38|T_YMM, 0x50, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpbsuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F3|T_0F38|T_YMM, 0x51, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpbuud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_0F38|T_YMM, 0x50, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpbuuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_0F38|T_YMM, 0x51, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwsud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F3|T_0F38|T_YMM, 0xD2, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwsuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_F3|T_0F38|T_YMM, 0xD3, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwusd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_66|T_0F38|T_YMM, 0xD2, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwusds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_66|T_0F38|T_YMM, 0xD3, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwuud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_0F38|T_YMM, 0xD2, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpdpwuuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
     { opEncoding(x1, x2, op, T_0F38|T_YMM, 0xD3, encoding, NONE, T_W0, T_W0|T_B32, 1); }
    void vpermb(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x8D); }
    void vpermi2b(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x75); }
    void vpermi2d(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x76); }
    ...
    void vpermi2ps(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x77); }
    ...
    void vpermt2b(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x7D); }
    void vpermt2d(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x7E); }
    ...
    void vpermt2ps(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x7F); }
    ...
    void vpexpandb(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_N1|T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX, 0x62); }
    void vpexpandd(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x89); }
    ...
    void vpgatherdd(Xmm x, Address addr)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_VSIB, 0x90, 0); }
    ...
    void vpgatherqd(Xmm x, Address addr)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_VSIB, 0x91, 2); }
    ...
    void vplzcntd(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x44); }
    ...
    void vpmovb2m(Opmask k, Xmm x) { opVex(k, null, x, T_F3|T_0F38|T_MUST_EVEX|T_YMM|T_W0, 0x29); }
    void vpmovd2m(Opmask k, Xmm x) { opVex(k, null, x, T_F3|T_0F38|T_MUST_EVEX|T_YMM|T_W0, 0x39); }
    void vpmovdb(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x31); }
    void vpmovdw(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x33); }
    void vpmovm2b(Xmm x, Opmask k) { opVex(x, null, k, T_F3|T_0F38|T_MUST_EVEX|T_YMM|T_W0, 0x28); }
    void vpmovm2d(Xmm x, Opmask k) { opVex(x, null, k, T_F3|T_0F38|T_MUST_EVEX|T_YMM|T_W0, 0x38); }
    ...
    void vpmovqb(Operand op, Xmm x)
     { opX_XM(op, x, T_N2|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x32); }
    void vpmovqd(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x35); }
    void vpmovqw(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x34); }
    void vpmovsdb(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x21); }
    void vpmovsdw(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x23); }
    void vpmovsqb(Operand op, Xmm x)
     { opX_XM(op, x, T_N2|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x22); }
    void vpmovsqd(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x25); }
    void vpmovsqw(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x24); }
    void vpmovswb(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x20); }
    void vpmovusdb(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x11); }
    void vpmovusdw(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x13); }
    void vpmovusqb(Operand op, Xmm x)
     { opX_XM(op, x, T_N2|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x12); }
    void vpmovusqd(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x15); }
    void vpmovusqw(Operand op, Xmm x)
     { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x14); }
    void vpmovuswb(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x10); }
    void vpmovw2m(Opmask k, Xmm x)
     { opVex(k, null, x, T_F3|T_0F38|T_MUST_EVEX|T_YMM|T_EW1, 0x29); }
    void vpmovwb(Operand op, Xmm x)
     { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x30); }
    ...
    void vpopcntb(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX, 0x54); }
    void vpopcntd(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x55); }
    ...
    void vpord(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0xEB); }
    ...
    void vprold(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_X_XM(Xmm(x.getKind(), 1), x, op, T_66|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x72, imm); }
    ...
    void vprolvd(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x15); }
    ...
    void vprord(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_X_XM(Xmm(x.getKind(), 0), x, op, T_66|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x72, imm); }
    ...
    void vprorvd(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x14); }
    ...
    void vpscatterdd(Address addr, Xmm x)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K|T_VSIB, 0xA0, 0); }
    ...
    void vpscatterqd(Address addr, Xmm x)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K|T_VSIB, 0xA1, 2); }
    ...
    void vpshldd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x71, imm); }
    ...
    void vpshldvd(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x71); }
    ...
    void vpshrdd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x73, imm); }
    ...
    void vpshrdvd(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x73); }
    ...
    void vpshufbitqmb(Opmask k, Xmm x, Operand op)
     { opVex(k, x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x8F); }
    ...
    void vpternlogd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x25, imm); }
    ...
    void vptestmb(Opmask k, Xmm x, Operand op)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x26); }
    void vptestmd(Opmask k, Xmm x, Operand op)
     { opAVX_K_X_XM(k, x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x27); }
    ...
    void vptestnmb(Opmask k, Xmm x, Operand op)
     { opAVX_K_X_XM(k, x, op, T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x26); }
    void vptestnmd(Opmask k, Xmm x, Operand op)
     { opAVX_K_X_XM(k, x, op, T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x27); }
    ...
    void vpxord(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0xEF); }
    ...
    void vrangeps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x50, imm); }
    ...
    void vrangess(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x51, imm); }
    ...
    void vrcp14ps(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x4C); }
    ...
    void vrcp14ss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_MUST_EVEX, 0x4D); }
    ...
    void vrcp28ps(Zmm z, Operand op)
     { opAVX_X_XM_IMM(z, op, T_66|T_0F38|T_MUST_EVEX|T_YMM|T_W0|T_B32|T_SAE_Z, 0xCA); }
    ...
    void vrcp28ss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_SAE_X|T_MUST_EVEX, 0xCB); }
    void vrcpbf16(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x4C); }
    void vrcpph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x4C); }
    void vrcpsh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_MUST_EVEX, 0x4D); }
    void vreducebf16(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_F2|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x56, imm); }
    ...
    void vreduceph(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x56, imm); }
    void vreduceps(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x56, imm); }
    ...
    void vreducesh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x57, imm); }
    void vreducess(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x57, imm); }
    void vrndscalebf16(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_F2|T_0F3A|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x08, imm); }
    ...
    void vrndscaleph(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B16, 0x08, imm); }
    void vrndscaleps(Xmm x, Operand op, uint8_t imm)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F3A|T_W0|T_YMM|T_SAE_Z|T_MUST_EVEX|T_B32, 0x08, imm); }
    ...
    void vrndscalesh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x0A, imm); }
    void vrndscaless(Xmm x1, Xmm x2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F3A|T_W0|T_SAE_X|T_MUST_EVEX, 0x0A, imm); }
    ...
    void vrsqrt14ps(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x4E); }
    ...
    void vrsqrt14ss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX, 0x4F); }
    ...
    void vrsqrt28ps(Zmm z, Operand op)
     { opAVX_X_XM_IMM(z, op, T_66|T_0F38|T_MUST_EVEX|T_YMM|T_W0|T_B32|T_SAE_Z, 0xCC); }
    ...
    void vrsqrt28ss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_SAE_X|T_MUST_EVEX, 0xCD); }
    void vrsqrtbf16(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x4E); }
    void vrsqrtph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x4E); }
    void vrsqrtsh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_MUST_EVEX, 0x4F); }
    void vscalefbf16(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x2C); }
    void vscalefbf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_MAP6|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x2C); }
    ...
    void vscalefph(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP6|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x2C); }
    void vscalefps(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_0F38|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B32, 0x2C); }
    ...
    void vscalefsh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_66|T_MAP6|T_W0|T_ER_X|T_MUST_EVEX, 0x2D); }
    void vscalefss(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N4|T_66|T_0F38|T_W0|T_ER_X|T_MUST_EVEX, 0x2D); }
    ...
    void vscatterqps(Address addr, Xmm x)
     { opGather2(x, addr, T_N4|T_66|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K|T_VSIB, 0xA3, 2); }
    void vshuff32x4(Ymm y1, Ymm y2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(y1, y2, op, T_66|T_0F3A|T_YMM|T_MUST_EVEX|T_W0|T_B32, 0x23, imm); }
    ...
    void vshufi32x4(Ymm y1, Ymm y2, Operand op, uint8_t imm)
     { opAVX_X_X_XM(y1, y2, op, T_66|T_0F3A|T_YMM|T_MUST_EVEX|T_W0|T_B32, 0x43, imm); }
    ...
    void vsqrtbf16(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x51); }
    void vsqrtph(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_MAP5|T_W0|T_YMM|T_ER_Z|T_MUST_EVEX|T_B16, 0x51); }
    void vsqrtsh(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_N2|T_F3|T_MAP5|T_W0|T_ER_X|T_MUST_EVEX, 0x51); }
    void vsubbf16(Xmm x1, Xmm x2, Operand op)
     { opAVX_X_X_XM(x1, x2, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B16, 0x5C); }
    void vsubph(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_ER_Z|T_B16, 0x5C); }
    void vsubsh(Xmm xmm, Operand op1, Operand op2 = Operand())
     { opAVX_X_X_XM(xmm, op1, op2, T_MAP5|T_F3|T_W0|T_MUST_EVEX|T_ER_X|T_N2, 0x5C); }
    void vucomish(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_N2|T_MAP5|T_W0|T_SAE_X|T_MUST_EVEX, 0x2E); }
    ...
    void vucomxsh(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_N2|T_F3|T_MAP5|T_W0|T_SAE_X|T_MUST_EVEX, 0x2E); }
    void vucomxss(Xmm x, Operand op)
     { opAVX_X_XM_IMM(x, op, T_N4|T_F3|T_0F|T_W0|T_SAE_X|T_MUST_EVEX, 0x2E); }
    ...
    version (XBYAK64)
    {
        ...
        void top4busd(Tmm t1, Zmm z2, Zmm z3) { opVex(t1,z3, z2, T_66|T_0F38|T_W0|T_MUST_EVEX, 0x5E); }
        void top4buud(Tmm t1, Zmm z2, Zmm z3) { opVex(t1,z3, z2, T_0F38|T_W0|T_MUST_EVEX, 0x5E); }
        void top4mxbf8ps(Tmm t1, Zmm z2, Zmm z3, uint8_t imm)
         { opVex(t1,z3, z2, T_0F3A|T_W0|T_MUST_EVEX, 0x8D, imm); }
        void top4mxbhf8ps(Tmm t1, Zmm z2, Zmm z3, uint8_t imm)
         { opVex(t1,z3, z2, T_F2|T_0F3A|T_W0|T_MUST_EVEX, 0x8D, imm); }
        void top4mxbssps(Tmm t1, Zmm z2, Zmm z3, uint8_t imm)
         { opVex(t1,z3, z2, T_F2|T_0F3A|T_W0|T_MUST_EVEX, 0x8F, imm); }
        void top4mxhbf8ps(Tmm t1, Zmm z2, Zmm z3, uint8_t imm)
         { opVex(t1,z3, z2, T_F3|T_0F3A|T_W0|T_MUST_EVEX, 0x8D, imm); }
        void top4mxhf8ps(Tmm t1, Zmm z2, Zmm z3, uint8_t imm)
         { opVex(t1,z3, z2, T_66|T_0F3A|T_W0|T_MUST_EVEX, 0x8D, imm); }
        void vcvtbf42hf8(Xmm x, Operand op)
         { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_MAP5|T_W0|T_YMM|T_MUST_EVEX, 0x37); }
        void vcvtbf62hf8(Xmm x, Xmm op)
         { opVex(x, null, op, T_66|T_MAP5|T_EW1|T_YMM|T_MUST_EVEX, 0x37); }
        void vcvtbf82bf4s(Operand op, Xmm x)
         { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_MAP5|T_EW1|T_YMM|T_MUST_EVEX, 0x3D); }
        void vcvtbf82bf6s(Xmm x, Xmm op)
         { opVex(op, null, x, T_F3|T_MAP5|T_EW1|T_YMM|T_MUST_EVEX, 0x3E); }
        void vcvtbf82ps(Xmm x, Operand op)
         { opX_XM(op, x, T_N4|T_N_VL|T_MAP5|T_EW1|T_YMM|T_MUST_EVEX, 0x36); }
        void vcvtbiasps2bf8(Xmm x1, Xmm x2, Operand op)
         { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x39, true); }
        void vcvtbiasps2bf8s(Xmm x1, Xmm x2, Operand op)
         { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x3B, true); }
        void vcvtbiasps2hf8(Xmm x1, Xmm x2, Operand op)
         { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x38, true); }
        void vcvtbiasps2hf8s(Xmm x1, Xmm x2, Operand op)
         { opCvtBias(x1, x2, op, T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x3A, true); }
        void vcvthf62hf8(Xmm x, Xmm op) { opVex(x, null, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX, 0x37); }
        void vcvthf82bf4s(Operand op, Xmm x)
         { opCvt_xx_xy_yz(x, op, T_N8|T_N_VL|T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX, 0x3D); }
        void vcvthf82hf6s(Xmm x, Xmm op) { opVex(op, null, x, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX, 0x3C); }
        void vcvthf82ps(Xmm x, Operand op) { opX_XM(op, x, T_N4|T_N_VL|T_MAP5|T_W0|T_YMM|T_MUST_EVEX, 0x36); }
        void vcvtps2bf8(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x39); }
        void vcvtps2bf8s(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x3B); }
        void vcvtps2hf8(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x38); }
        void vcvtps2hf8s(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_F3|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x3A); }
        void vcvtrops2hf8(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x38); }
        void vcvtrops2hf8s(Xmm x, Operand op) { opCvt_xx_yx_zx(x, op, T_66|T_MAP5|T_W0|T_YMM|T_MUST_EVEX|T_B32, 0x3A); }
        ...
        void vpmovssdb(Operand op, Xmm x)
         { opX_XM(op, x, T_N4|T_N_VL|T_F3|T_0F38|T_W0|T_YMM|T_MUST_EVEX|T_M_K, 0x41); }
        void vunpackb(Xmm x, Operand op, uint8_t imm)
         { opAVX_X_XM_IMM(x, op, T_0F3A|T_W0|T_YMM|T_MUST_EVEX, 0x3D, imm); }
    }
