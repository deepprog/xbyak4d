...
version (XBYAK_NO_EXCEPTION)
{
    ...
    {
        return "local.SetError(" ~ typeof(err).stringof ~ "." ~ to!string(
            err) ~ "); return " ~ r ~ ";";
    }
...
static:
    ...
    bool IsInDisp8(uint32_t x)
    {
        return 0xFFFFFF80 <= x || x <= 0x7F;
    }

    bool IsInInt32(uint64_t x)
    {
...
class Allocator
{
    this(string = "")
    {
    } // same interface with MmapAllocator
    // ~this() {}
...
public:
    ...
    bool hasIdxBit(int bit) const
    {
        return cast(bool)(idx_ & (1 << bit));
    }
    ...
    bool isZMM() const
    {
        return isKind(Kind.ZMM);
    }

    bool isSIMD() const
    {
        return isKind(Kind.XMM | Kind.YMM | Kind.ZMM);
    }

    bool isTMM() const
    {
        return isKind(Kind.TMM);
    }

    bool isXMEM() const
    {
        return isKind(Kind.XMM | Kind.MEM);
    }

    bool isYMEM() const
    {
        return isKind(Kind.YMM | Kind.MEM);
    }

    bool isZMEM() const
    {
        return isKind(Kind.ZMM | Kind.MEM);
    }

    bool isOPMASK() const
    {
        return isKind(Kind.OPMASK);
    }

    bool isBNDREG() const
    {
        return isKind(Kind.BNDREG);
    }
    ...
    bool hasRex2NF() const
    {
        return hasRex2() || NF_;
    }

    bool hasRex2NFZU() const
    {
        return hasRex2() || NF_ || ZU_;
    }
    ...
    {
        if ((kind & (Kind.XMM | Kind.YMM | Kind.ZMM | Kind.TMM)) == 0)
            return;
        kind_ = kind;
    ...
    void setBit(int bit)
    {
        if (bit != 8 && bit != 16 && bit != 32 && bit != 64 &&
            bit != 128 && bit != 256 && bit != 512 && bit != 8192)
            goto ERR;
        if (isBit(bit))
            return;
        ...
        if (isKind(Kind.REG | Kind.XMM | Kind.YMM | Kind.ZMM | Kind.TMM))
        {
            ...
            if (isREG(8) && (4 <= idx && idx < 8) && !isExt8bit)
                goto ERR;
            Kind kind = Kind.REG;
            switch (bit)
            {
            ...
            bit_ = bit;
            if (bit >= 128)
                return; // keep mask_ and rounding_
            mask_ = 0;
            rounding_ = 0;
            return;
        }
...
public:
    ...
    Reg getBase() const
    {
        return cast(Reg) base_;
    }

    Reg getIndex() const
    {
        return cast(Reg) index_;
    }
...
class CodeArray
{
    ...
    struct AddrInfo
    {
        ...
        {
            uint64_t disp = (mode == inner.LabelMode.LaddTop) ?
                jmpAddr + cast(size_t) top : (mode == inner.LabelMode.LasIs) ?
        jmpAddr : jmpAddr - cast(size_t) top;
            if (jmpSize == 4)
            {
...
public:
    ...
    this(size_t maxSize, void* userPtr = null, Allocator allocator = null)
    {
        type_ = (userPtr == AutoGrow ?
                Type.AUTO_GROW : (userPtr == null || userPtr == DontSetProtectRWE) ?
                Type.ALLOC_BUF : Type.USER_BUF);
    ...
    {
        return setProtectMode(ProtectMode.PROTECT_RE, throwException);
    }

    bool setProtectModeRW(bool throwException = true)
    {
        return setProtectMode(ProtectMode.PROTECT_RW, throwException);
    }
...
public:
    ...
    Address cloneNoOptimize()
    {
        Address addr = new Address(this);
        addr.optimize_ = false;
        return addr;
    }
    ...
    {
        return !e_.getBase().getBit() && !e_.getIndex().getBit();
    } // for mov eax
    size_t getDisp() const
    {
        return e_.getDisp();
    ...
    override bool hasRex2() const
    {
        return e_.getBase().hasRex2() || e_.getIndex().hasRex2();
    }
...
private:
    ...
    )
    {
        ...
        int rounding =
            verifyDuplicate(
                reg.getRounding(),
                base.getRounding(),
                v ? v.getRounding() : 0, ERR.ROUNDING_IS_ALREADY_SET
            );
        int disp8N = 1;
        if (rounding)
        {
            ...
            {
                isUzero = verifySAE(reg, base, type);
                LL = 0;
            }
            else
            {
                isUzero = verifyER(reg, base, type);
                LL = rounding - 1;
            }
            if (isUzero)
                U = 0; // avx10.2 Evex.U
        ...
        if (aaa == 0)
        {
            aaa =
                verifyDuplicate(
                    ...
                    reg.getOpmaskIdx(),
                    (v ? v.getOpmaskIdx() : 0),
                    ERR.OPMASK_IS_ALREADY_SET
                );
        }
        if (aaa == 0)
            z = false; // clear T_z if mask is not set
        db(0x62);
    ...
    void opMR(Address addr, Reg r, uint64_t type, int code, uint64_t type2 = 0, int code2 = NONE)
    {
        if (code2 == NONE)
            code2 = code;
        if (type2 && opROO(Reg(), addr, r, type2, code2))
            return;
        if (addr.is64bitDisp())
        {
    ...
    bool isNEAR(LabelType type) const
    {
        return type == T_NEAR || (type == T_AUTO && isDefaultJmpNEAR_);
    }
    ...
    {
        if (opROO(Reg(), op, Reg(), T_APX | T_ZU | T_F2, 0x40 | ext))
            return;
    ...
    bool isInDisp16(uint32_t x) const
    {
        return 0xFFFF8000 <= x || x <= 0x7FFF;
    }
    // allow add(ax, 0x8000);
    bool isInDisp16relaxed(uint32_t x) const
    {
        uint32_t v = x & 0xffff0000;
        return v == 0 || v == 0xffff0000;
    }
    ...
    void opOI(Operand op, uint32_t imm, int code, int ext)
    {
        ...
        if (
            op.isREG() &&
            op.getIdx() == 0 &&
            (op.getBit() == immBit || (op.isBit(64) && immBit == 32))
            )
        { // rax, eax, ax, al
            rex(op);
    ...
    void opGather2(Xmm x, Address addr, uint64_t type, uint8_t code, int mode)
    {
        ...
        if ((type & T_M_K) && addr.getOpmaskIdx())
            maskIdx = addr.getOpmaskIdx();
        if (maskIdx == 0)
        {
    ...
    */
    ...
    void opEncoding(
        ...
        Operand op,
        uint64_t type,
        int code, PreferredEncoding encoding,
        int imm = NONE,
        uint64_t typeVex = 0,
        uint64_t typeEvex = 0,
        int sel = 0
    ...
    void opKmov(Opmask k, Operand op, bool rev, int size)
    {
        ...
        case 16:
            ...
            break;
        case 32:
            type |= isReg ? T_W0 | T_F2 : T_W1 | T_66;
            break;
        case 64:
            type |= isReg ? T_W1 | T_F2 : T_W1;
            break;
        default:
            assert(0);
        }
        ...
        if (opROO(Reg(), p2, p1, T_APX | type, code))
            return;
    ...
    void opSSE_APX(Xmm x, Operand op, uint64_t type1, uint8_t code1, uint64_t type2, uint8_t code2, int imm = NONE)
    {
        if (
            x.getIdx() <= 15 &&
            op.hasRex2() &&
            opROO(Reg(), op, x, type2, code2, imm != NONE ? 1 : 0)
            )
        {
            if (imm != NONE)
                db(imm);
...
public:
    ...
    enum
    {
        ...
        eax = Reg32(Operand.EAX),
        ecx = Reg32(Operand.ECX),
        edx = Reg32(Operand.EDX),
        ebx = Reg32(Operand.EBX),
        esp = Reg32(Operand.ESP),
        ebp = Reg32(Operand.EBP),
        esi = Reg32(Operand.ESI),
        edi = Reg32(Operand.EDI),
        ax = Reg16(Operand.EAX),
        cx = Reg16(Operand.ECX),
        dx = Reg16(Operand.EDX),
        bx = Reg16(Operand.EBX),
        sp = Reg16(Operand.ESP),
        bp = Reg16(Operand.EBP),
        si = Reg16(Operand.ESI),
        di = Reg16(Operand.EDI),
        al = Reg8(Operand.AL),
        cl = Reg8(Operand.CL),
        dl = Reg8(Operand.DL),
        bl = Reg8(Operand.BL),
    ...
    version (XBYAK64)
    {
        enum
        {
            rax = Reg64(Operand.RAX),
            rcx = Reg64(Operand.RCX),
            rdx = Reg64(Operand.RDX),
            rbx = Reg64(Operand.RBX),
            rsp = Reg64(Operand.RSP),
            rbp = Reg64(Operand.RBP),
            rsi = Reg64(Operand.RSI),
            rdi = Reg64(Operand.RDI),
            r8 = Reg64(Operand.R8),
            r9 = Reg64(Operand.R9),
            r10 = Reg64(Operand.R10),
            r11 = Reg64(Operand.R11),
            r12 = Reg64(Operand.R12),
            r13 = Reg64(Operand.R13),
            r14 = Reg64(Operand.R14),
            r15 = Reg64(Operand.R15),
            r16 = Reg64(Operand.R16),
            r17 = Reg64(Operand.R17),
            r18 = Reg64(Operand.R18),
            r19 = Reg64(Operand.R19),
            r20 = Reg64(Operand.R20),
            r21 = Reg64(Operand.R21),
            r22 = Reg64(Operand.R22),
            r23 = Reg64(Operand.R23),
            r24 = Reg64(Operand.R24),
            r25 = Reg64(Operand.R25),
            r26 = Reg64(Operand.R26),
            r27 = Reg64(Operand.R27),
            r28 = Reg64(Operand.R28),
            r29 = Reg64(Operand.R29),
            r30 = Reg64(Operand.R30),
            r31 = Reg64(Operand.R31),

            r8d = Reg32(Operand.R8D),
            r9d = Reg32(Operand.R9D),
            r10d = Reg32(Operand.R10D),
            r11d = Reg32(Operand.R11D),
            r12d = Reg32(Operand.R12D),
            r13d = Reg32(Operand.R13D),
            r14d = Reg32(Operand.R14D),
            r15d = Reg32(Operand.R15D),
            r16d = Reg32(Operand.R16D),
            r17d = Reg32(Operand.R17D),
            r18d = Reg32(Operand.R18D),
            r19d = Reg32(Operand.R19D),
            r20d = Reg32(Operand.R20D),
            r21d = Reg32(Operand.R21D),
            r22d = Reg32(Operand.R22D),
            r23d = Reg32(Operand.R23D),
            r24d = Reg32(Operand.R24D),
            r25d = Reg32(Operand.R25D),
            r26d = Reg32(Operand.R26D),
            r27d = Reg32(Operand.R27D),
            r28d = Reg32(Operand.R28D),
            r29d = Reg32(Operand.R29D),
            r30d = Reg32(Operand.R30D),
            r31d = Reg32(Operand.R31D),

            r8w = Reg16(Operand.R8W),
            r9w = Reg16(Operand.R9W),
            r10w = Reg16(Operand.R10W),
            r11w = Reg16(Operand.R11W),
            r12w = Reg16(Operand.R12W),
            r13w = Reg16(Operand.R13W),
            r14w = Reg16(Operand.R14W),
            r15w = Reg16(Operand.R15W),
            r16w = Reg16(Operand.R16W),
            r17w = Reg16(Operand.R17W),
            r18w = Reg16(Operand.R18W),
            r19w = Reg16(Operand.R19W),
            r20w = Reg16(Operand.R20W),
            r21w = Reg16(Operand.R21W),
            r22w = Reg16(Operand.R22W),
            r23w = Reg16(Operand.R23W),
            r24w = Reg16(Operand.R24W),
            r25w = Reg16(Operand.R25W),
            r26w = Reg16(Operand.R26W),
            r27w = Reg16(Operand.R27W),
            r28w = Reg16(Operand.R28W),
            r29w = Reg16(Operand.R29W),
            r30w = Reg16(Operand.R30W),
            r31w = Reg16(Operand.R31W),

            r8b = Reg8(Operand.R8B),
            r9b = Reg8(Operand.R9B),
            r10b = Reg8(Operand.R10B),
            r11b = Reg8(Operand.R11B),
            r12b = Reg8(Operand.R12B),
            r13b = Reg8(Operand.R13B),
            r14b = Reg8(Operand.R14B),
            r15b = Reg8(Operand.R15B),
            r16b = Reg8(Operand.R16B),
            r17b = Reg8(Operand.R17B),
            r18b = Reg8(Operand.R18B),
            r19b = Reg8(Operand.R19B),
            r20b = Reg8(Operand.R20B),
            r21b = Reg8(Operand.R21B),
            r22b = Reg8(Operand.R22B),
            r23b = Reg8(Operand.R23B),
            r24b = Reg8(Operand.R24B),
            r25b = Reg8(Operand.R25B),
            r26b = Reg8(Operand.R26B),
            r27b = Reg8(Operand.R27B),
            r28b = Reg8(Operand.R28B),
            r29b = Reg8(Operand.R29B),
            r30b = Reg8(Operand.R30B),
            r31b = Reg8(Operand.R31B),
...
public:
    ...
    // src : used by L()
    void assignL(ref Label dst, Label src)
    {
    ...
    */
    ...
    {
        isDefaultJmpNEAR_ = isNear;
    }

    void jmp(Operand op, LabelType type = T_AUTO)
    {
        opJmpOp(op, type, 4);
    }

    void jmp(string label, LabelType type = T_AUTO)
    {
    ...
    {
        jmp(to!string(label), type);
    }

    void jmp(ref Label label, LabelType type = T_AUTO)
    {
        opJmp(label, type, 0xEB, 0xE9, 0);
    }
    ...
    void call(Operand op, LabelType type = T_AUTO)
    {
        opJmpOp(op, type, 2);
    }
    ...
    version (XBYAK64)
    {
        void mov(Operand op1, Operand op2)
        {
            ...
            else if (op1.isMEM() && op2.isREG() && op2.getIdx() == 0) // mov [disp], eax|ax|al
            {
                reg = op2.getReg();
                addr = op1.getAddress();
                code = 0xA2;
    ...
    void xchg(Operand op1, Operand op2)
    {
        ...
        if (p1.isREG() && p2.isREG())
            swap(p1, p2); // adapt to NASM 2.16.03 behavior to pass tests
        opRO(cast(Reg) p1, p2, 0, 0x86 | (p1.isBit(8) ? 0 : 1), (p1.isREG() && (
                p1.getBit() == p2.getBit())));
    }
    ...
    else
    {
        ...
        {
            opRO(Reg8(seg.getIdx()), op, T_ALLOW_DIFF_SIZE | T_ALLOW_ABCDH, 0x8C, op.isREG(
                    16 | i32e));
        }

        void mov(Segment seg, Operand op)
        {
            opRO(Reg8(seg.getIdx()),
                op.isREG(16 | i32e) ? cast(Operand)(op.getReg().cvt32()) : op,
                T_ALLOW_DIFF_SIZE | T_ALLOW_ABCDH,
                0x8E,
                op.isREG(16 | i32e));
        }
    }
...
public:
    ...
    */
    ...
    void readyRE()
    {
        return ready(ProtectMode.PROTECT_RE);
    }
    ...
    */
    ...
    else
    {
        ...
        void aadd(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F38, 0x0FC, T_APX);
        }

        void aand(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F38 | T_66, 0x0FC, T_APX | T_66);
        }

        void adc(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x10, 2);
        }
        ...
        {
            opRO_MR(op1, op2, 0x10);
        }

        void adc(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NONE, 2);
        }

        void adc(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NONE, 0x10);
        }

        void adcx(Reg32e d, Reg32e reg, Operand op)
        {
            opROO(d, op, reg, T_66, 0x66);
        }

        void adcx(Reg32e reg, Operand op)
        {
            if (!reg.isREG(16 | i32e) && reg.getBit() == op.getBit())
            {
                mixin(XBYAK_THROW(ERR.BAD_SIZE_OF_REGISTER));
            }
            if (opROO(Reg(), op, reg, T_66, 0x66))
                return;
            opRO(reg, op, T_66 | T_0F38, 0xF6);
        }

        void add(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x00, 0);
        }
        ...
        {
            opRO_MR(op1, op2, 0x00);
        }

        void add(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NF | T_CODE1_IF1, 0);
        }

        void add(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NF | T_CODE1_IF1, 0x00);
        }

        void addpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x58, &isXMM_XMMorMEM);
        }

        void addps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x58, &isXMM_XMMorMEM);
        }

        void addsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x58, &isXMM_XMMorMEM);
        }

        void addss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x58, &isXMM_XMMorMEM);
        }

        void addsubpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F | T_YMM, 0xD0, &isXMM_XMMorMEM);
        }

        void addsubps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F2 | T_0F | T_YMM, 0xD0, &isXMM_XMMorMEM);
        }

        void adox(Reg32e d, Reg32e reg, Operand op)
        {
            opROO(d, op, reg, T_F3, 0x66);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDE, &isXMM_XMMorMEM);
        }

        void aesdeclast(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDF, &isXMM_XMMorMEM);
        }

        void aesenc(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDC, &isXMM_XMMorMEM);
        }

        void aesenclast(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDD, &isXMM_XMMorMEM);
        }

        void aesimc(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_W0, 0xDB, &isXMM_XMMorMEM, NONE);
        }

        void aeskeygenassist(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0xDF, &isXMM_XMMorMEM, imm);
        }

        void and_(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x20, 4);
        }

        void and_(Operand op1, Operand op2)
        {
            opRO_MR(op1, op2, 0x20);
        }

        void and_(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NF | T_CODE1_IF1, 4);
        }

        void and_(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NF | T_CODE1_IF1, 0x20);
        }

        void andn(Reg32e r1, Reg32e r2, Operand op)
        {
            opRRO(r1, r2, op, T_APX | T_0F38 | T_NF, 0xf2);
        }

        void andnpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x55, &isXMM_XMMorMEM);
        }

        void andnps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x55, &isXMM_XMMorMEM);
        }

        void andpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x54, &isXMM_XMMorMEM);
        }

        void andps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x54, &isXMM_XMMorMEM);
        }

        void aor(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F38 | T_F2, 0x0FC, T_APX | T_F2);
        }

        void axor(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F38 | T_F3, 0x0FC, T_APX | T_F3);
        }

        void bextr(Reg32e r1, Operand op, Reg32e r2)
        {
            opRRO(r1, r2, op, T_APX | T_0F38 | T_NF, 0xf7);
        }

        void blendpd(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x0D, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void blendps(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x0C, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void blendvpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x15, &isXMM_XMMorMEM, NONE);
        }

        void blendvps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x14, &isXMM_XMMorMEM, NONE);
        }

        void blsi(Reg32e r, Operand op)
        {
            opRRO(Reg32e(3, r.getBit()), r, op, T_APX | T_0F38 | T_NF, 0xf3);
        }

        void blsmsk(Reg32e r, Operand op)
        {
            opRRO(Reg32e(2, r.getBit()), r, op, T_APX | T_0F38 | T_NF, 0xf3);
        }

        void blsr(Reg32e r, Operand op)
        {
            opRRO(Reg32e(1, r.getBit()), r, op, T_APX | T_0F38 | T_NF, 0xf3);
        }
        ...
        {
            opRext(op, i32e, bnd.getIdx(), T_F3 | T_0F, 0x1A, !op.isMEM());
        }

        void bndcn(BoundsReg bnd, Operand op)
        {
            opRext(op, i32e, bnd.getIdx(), T_F2 | T_0F, 0x1B, !op.isMEM());
        }

        void bndcu(BoundsReg bnd, Operand op)
        {
            opRext(op, i32e, bnd.getIdx(), T_F2 | T_0F, 0x1A, !op.isMEM());
        }

        void bndldx(BoundsReg bnd, Address addr)
        {
            opMIB(addr, bnd, T_0F, 0x1A);
        }

        void bndmk(BoundsReg bnd, Address addr)
        {
            opMR(addr, bnd, T_F3 | T_0F, 0x1B);
        }

        void bndmov(Address addr, BoundsReg bnd)
        {
            opMR(addr, bnd, T_66 | T_0F, 0x1B);
        }

        void bndmov(BoundsReg bnd, Operand op)
        {
            opRO(bnd, op, T_66 | T_0F, 0x1A, op.isBNDREG());
        }

        void bndstx(Address addr, BoundsReg bnd)
        {
            opMIB(addr, bnd, T_0F, 0x1B);
        }

        void bsf(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0xBC, op.isREG(16 | i32e));
        }

        void bsr(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0xBD, op.isREG(16 | i32e));
        }

        void bt(Operand op, Reg reg)
        {
            opRO(reg, op, T_0F, 0xA3, op.isREG(16 | i32e) && op.getBit() == reg.getBit());
        }

        void bt(Operand op, uint8_t imm)
        {
            opRext(op, 16 | i32e, 4, T_0F, 0xba, false, 1);
            db(imm);
        }

        void btc(Operand op, Reg reg)
        {
            opRO(reg, op, T_0F, 0xBB, op.isREG(16 | i32e) && op.getBit() == reg.getBit());
        }

        void btc(Operand op, uint8_t imm)
        {
            opRext(op, 16 | i32e, 7, T_0F, 0xba, false, 1);
            db(imm);
        }

        void btr(Operand op, Reg reg)
        {
            opRO(reg, op, T_0F, 0xB3, op.isREG(16 | i32e) && op.getBit() == reg.getBit());
        }

        void btr(Operand op, uint8_t imm)
        {
            opRext(op, 16 | i32e, 6, T_0F, 0xba, false, 1);
            db(imm);
        }

        void bts(Operand op, Reg reg)
        {
            opRO(reg, op, T_0F, 0xAB, op.isREG(16 | i32e) && op.getBit() == reg.getBit());
        }

        void bts(Operand op, uint8_t imm)
        {
            opRext(op, 16 | i32e, 5, T_0F, 0xba, false, 1);
            db(imm);
        }

        void bzhi(Reg32e r1, Operand op, Reg32e r2)
        {
            opRRO(r1, r2, op, T_APX | T_0F38 | T_NF, 0xf5);
        }
        ...
        void ccmpa(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 7);
        }

        void ccmpa(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 7);
        }

        void ccmpae(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 3);
        }

        void ccmpae(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 3);
        }

        void ccmpb(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 2);
        }

        void ccmpb(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 2);
        }

        void ccmpbe(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 6);
        }

        void ccmpbe(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 6);
        }

        void ccmpc(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 2);
        }

        void ccmpc(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 2);
        }

        void ccmpe(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 4);
        }

        void ccmpe(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 4);
        }

        void ccmpf(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 11);
        }

        void ccmpf(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 11);
        }

        void ccmpg(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 15);
        }

        void ccmpg(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 15);
        }

        void ccmpge(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 13);
        }

        void ccmpge(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 13);
        }

        void ccmpl(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 12);
        }

        void ccmpl(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 12);
        }

        void ccmple(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 14);
        }

        void ccmple(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 14);
        }

        void ccmpna(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 6);
        }

        void ccmpna(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 6);
        }

        void ccmpnae(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 2);
        }

        void ccmpnae(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 2);
        }

        void ccmpnb(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 3);
        }

        void ccmpnb(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 3);
        }

        void ccmpnbe(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 7);
        }

        void ccmpnbe(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 7);
        }

        void ccmpnc(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 3);
        }

        void ccmpnc(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 3);
        }

        void ccmpne(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 5);
        }

        void ccmpne(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 5);
        }

        void ccmpng(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 14);
        }

        void ccmpng(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 14);
        }

        void ccmpnge(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 12);
        }

        void ccmpnge(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 12);
        }

        void ccmpnl(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 13);
        }

        void ccmpnl(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 13);
        }

        void ccmpnle(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 15);
        }

        void ccmpnle(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 15);
        }

        void ccmpno(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 1);
        }

        void ccmpno(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 1);
        }

        void ccmpns(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 9);
        }

        void ccmpns(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 9);
        }

        void ccmpnz(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 5);
        }

        void ccmpnz(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 5);
        }

        void ccmpo(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 0);
        }

        void ccmpo(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 0);
        }

        void ccmps(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 8);
        }

        void ccmps(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 8);
        }

        void ccmpt(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 10);
        }

        void ccmpt(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 10);
        }

        void ccmpz(Operand op, int imm, int dfv = 0)
        {
            opCcmpi(op, imm, dfv, 4);
        }

        void ccmpz(Operand op1, Operand op2, int dfv = 0)
        {
            opCcmp(op1, op2, dfv, 0x38, 4);
        }
        ...
        void cfcmovb(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x42);
        }

        void cfcmovb(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x42);
        }

        void cfcmovbe(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x46);
        }

        void cfcmovbe(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x46);
        }

        void cfcmovl(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4C);
        }

        void cfcmovl(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4C);
        }

        void cfcmovle(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4E);
        }

        void cfcmovle(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4E);
        }

        void cfcmovnb(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x43);
        }

        void cfcmovnb(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x43);
        }

        void cfcmovnbe(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x47);
        }

        void cfcmovnbe(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x47);
        }

        void cfcmovnl(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4D);
        }

        void cfcmovnl(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4D);
        }

        void cfcmovnle(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4F);
        }

        void cfcmovnle(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4F);
        }

        void cfcmovno(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x41);
        }

        void cfcmovno(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x41);
        }

        void cfcmovnp(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4B);
        }

        void cfcmovnp(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4B);
        }

        void cfcmovns(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x49);
        }

        void cfcmovns(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x49);
        }

        void cfcmovnz(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x45);
        }

        void cfcmovnz(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x45);
        }

        void cfcmovo(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x40);
        }

        void cfcmovo(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x40);
        }

        void cfcmovp(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x4A);
        }

        void cfcmovp(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x4A);
        }

        void cfcmovs(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x48);
        }

        void cfcmovs(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x48);
        }

        void cfcmovz(Operand op1, Operand op2)
        {
            opCfcmov(Reg(), op1, op2, 0x44);
        }

        void cfcmovz(Reg d, Reg r, Operand op)
        {
            opCfcmov(d | T_nf, op, r, 0x44);
        }
        ...
        void cldemote(Address addr)
        {
            opMR(addr, eax, T_0F, 0x1C);
        }

        void clflush(Address addr)
        {
            opMR(addr, Reg32(7), T_0F, 0xAE);
        }

        void clflushopt(Address addr)
        {
            opMR(addr, Reg32(7), T_66 | T_0F, 0xAE);
        }
        ...
        void clwb(Address addr)
        {
            opMR(addr, esi, T_66 | T_0F, 0xAE);
        }
        ...
        void cmova(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 7);
        }

        void cmova(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 7, op.isREG(16 | i32e));
        }

        void cmovae(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 3);
        }

        void cmovae(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 3, op.isREG(16 | i32e));
        }

        void cmovb(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 2);
        }

        void cmovb(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 2, op.isREG(16 | i32e));
        }

        void cmovbe(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 6);
        }

        void cmovbe(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 6, op.isREG(16 | i32e));
        }

        void cmovc(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 2);
        }
        ...
        void cmove(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 4);
        }

        void cmove(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 4, op.isREG(16 | i32e));
        }

        void cmovg(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 15);
        }

        void cmovg(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 15, op.isREG(16 | i32e));
        }

        void cmovge(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 13);
        }

        void cmovge(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 13, op.isREG(16 | i32e));
        }

        void cmovl(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 12);
        }

        void cmovl(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 12, op.isREG(16 | i32e));
        }

        void cmovle(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 14);
        }

        void cmovle(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 14, op.isREG(16 | i32e));
        }

        void cmovna(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 6);
        }
        ...
        void cmovnae(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 2);
        }
        ...
        void cmovnb(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 3);
        }
        ...
        void cmovnbe(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 7);
        }
        ...
        void cmovnc(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 3);
        }
        ...
        void cmovne(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 5);
        }

        void cmovne(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 5, op.isREG(16 | i32e));
        }

        void cmovng(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 14);
        }
        ...
        void cmovnge(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 12);
        }
        ...
        void cmovnl(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 13);
        }
        ...
        void cmovnle(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 15);
        }
        ...
        void cmovno(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 1);
        }

        void cmovno(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 1, op.isREG(16 | i32e));
        }

        void cmovnp(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 11);
        }

        void cmovnp(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 11, op.isREG(16 | i32e));
        }

        void cmovns(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 9);
        }

        void cmovns(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 9, op.isREG(16 | i32e));
        }

        void cmovnz(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 5);
        }
        ...
        void cmovo(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 0);
        }

        void cmovo(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 0, op.isREG(16 | i32e));
        }

        void cmovp(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 10);
        }

        void cmovp(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 10, op.isREG(16 | i32e));
        }

        void cmovpe(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 10);
        }
        ...
        void cmovpo(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 11);
        }
        ...
        void cmovs(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 8);
        }

        void cmovs(Reg reg, Operand op)
        {
            opRO(reg, op, T_0F, 0x40 | 8, op.isREG(16 | i32e));
        }

        void cmovz(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1, 0x40 | 4);
        }
        ...
        void cmp(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x38, 7);
        }
        ...
        void cmppd(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F | T_66, 0xC2, &isXMM_XMMorMEM, imm8);
        }

        void cmpps(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F, 0xC2, &isXMM_XMMorMEM, imm8);
        }
        ...
        void cmpsd(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F | T_F2, 0xC2, &isXMM_XMMorMEM, imm8);
        }

        void cmpss(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F | T_F3, 0xC2, &isXMM_XMMorMEM, imm8);
        }
        ...
        {
            opRO(reg, op, T_0F, 0xB0 | (reg.isBit(8) ? 0 : 1), op.getBit() == reg.getBit());
        }

        void cmpxchg8b(Address addr)
        {
            opMR(addr, Reg32(1), T_0F, 0xC7);
        }

        void comisd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F, 0x2F, &isXMM_XMMorMEM);
        }

        void comiss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x2F, &isXMM_XMMorMEM);
        }
        ...
        void ctesta(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 7);
        }

        void ctesta(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 7);
        }

        void ctestae(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 3);
        }

        void ctestae(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 3);
        }

        void ctestb(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 2);
        }

        void ctestb(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 2);
        }

        void ctestbe(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 6);
        }

        void ctestbe(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 6);
        }

        void ctestc(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 2);
        }

        void ctestc(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 2);
        }

        void cteste(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 4);
        }

        void cteste(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 4);
        }

        void ctestf(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 11);
        }

        void ctestf(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 11);
        }

        void ctestg(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 15);
        }

        void ctestg(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 15);
        }

        void ctestge(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 13);
        }

        void ctestge(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 13);
        }

        void ctestl(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 12);
        }

        void ctestl(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 12);
        }

        void ctestle(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 14);
        }

        void ctestle(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 14);
        }

        void ctestna(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 6);
        }

        void ctestna(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 6);
        }

        void ctestnae(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 2);
        }

        void ctestnae(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 2);
        }

        void ctestnb(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 3);
        }

        void ctestnb(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 3);
        }

        void ctestnbe(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 7);
        }

        void ctestnbe(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 7);
        }

        void ctestnc(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 3);
        }

        void ctestnc(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 3);
        }

        void ctestne(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 5);
        }

        void ctestne(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 5);
        }

        void ctestng(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 14);
        }

        void ctestng(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 14);
        }

        void ctestnge(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 12);
        }

        void ctestnge(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 12);
        }

        void ctestnl(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 13);
        }

        void ctestnl(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 13);
        }

        void ctestnle(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 15);
        }

        void ctestnle(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 15);
        }

        void ctestno(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 1);
        }

        void ctestno(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 1);
        }

        void ctestns(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 9);
        }

        void ctestns(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 9);
        }

        void ctestnz(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 5);
        }

        void ctestnz(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 5);
        }

        void ctesto(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 0);
        }

        void ctesto(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 0);
        }

        void ctests(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 8);
        }

        void ctests(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 8);
        }

        void ctestt(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 10);
        }

        void ctestt(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 10);
        }

        void ctestz(Operand op, Reg r, int dfv = 0)
        {
            opCcmp(op, r, dfv, 0x84, 4);
        }

        void ctestz(Operand op, int imm, int dfv = 0)
        {
            opTesti(op, imm, dfv, 4);
        }
        ...
        {
            opSSE(xmm, op, T_F3 | T_0F, 0xE6, &isXMM_XMMorMEM);
        }

        void cvtdq2ps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5B, &isXMM_XMMorMEM);
        }

        void cvtpd2dq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F2 | T_0F, 0xE6, &isXMM_XMMorMEM);
        }

        void cvtpd2pi(Reg reg, Operand op)
        {
            opSSE(reg, op, T_66 | T_0F, 0x2D, &isMMX_XMMorMEM);
        }

        void cvtpd2ps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F, 0x5A, &isXMM_XMMorMEM);
        }

        void cvtpi2pd(Reg reg, Operand op)
        {
            opSSE(reg, op, T_66 | T_0F, 0x2A, &isXMM_MMXorMEM);
        }

        void cvtpi2ps(Reg reg, Operand op)
        {
            opSSE(reg, op, T_0F, 0x2A, &isXMM_MMXorMEM);
        }

        void cvtps2dq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F, 0x5B, &isXMM_XMMorMEM);
        }

        void cvtps2pd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5A, &isXMM_XMMorMEM);
        }

        void cvtps2pi(Reg reg, Operand op)
        {
            opSSE(reg, op, T_0F, 0x2D, &isMMX_XMMorMEM);
        }

        void cvtsd2si(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F2 | T_0F, 0x2D, &isREG32_XMMorMEM);
        }

        void cvtsd2ss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F2 | T_0F, 0x5A, &isXMM_XMMorMEM);
        }

        void cvtsi2sd(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F2 | T_0F, 0x2A, &isXMM_REG32orMEM);
        }

        void cvtsi2ss(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F3 | T_0F, 0x2A, &isXMM_REG32orMEM);
        }

        void cvtss2sd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F3 | T_0F, 0x5A, &isXMM_XMMorMEM);
        }

        void cvtss2si(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F3 | T_0F, 0x2D, &isREG32_XMMorMEM);
        }

        void cvttpd2dq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F, 0xE6, &isXMM_XMMorMEM);
        }

        void cvttpd2pi(Reg reg, Operand op)
        {
            opSSE(reg, op, T_66 | T_0F, 0x2C, &isMMX_XMMorMEM);
        }

        void cvttps2dq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F3 | T_0F, 0x5B, &isXMM_XMMorMEM);
        }

        void cvttps2pi(Reg reg, Operand op)
        {
            opSSE(reg, op, T_0F, 0x2C, &isMMX_XMMorMEM);
        }

        void cvttsd2si(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F2 | T_0F, 0x2C, &isREG32_XMMorMEM);
        }

        void cvttss2si(Reg reg, Operand op)
        {
            opSSE(reg, op, T_F3 | T_0F, 0x2C, &isREG32_XMMorMEM);
        }
        ...
        void div(Operand op)
        {
            opRext(op, 0, 6, T_APX | T_NF | T_CODE1_IF1, 0xF6);
        }

        void divpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x5E, &isXMM_XMMorMEM);
        }

        void divps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5E, &isXMM_XMMorMEM);
        }

        void divsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x5E, &isXMM_XMMorMEM);
        }

        void divss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x5E, &isXMM_XMMorMEM);
        }

        void dppd(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x41, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void dpps(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x40, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }
        ...
        void extractps(Operand op, Xmm xmm, uint8_t imm)
        {
        ...
        {
            opMR(addr, Reg32(4), 0, 0xDF);
        }

        void fbstp(Address addr)
        {
            opMR(addr, Reg32(6), 0, 0xDF);
        }
        ...
        {
            opMR(addr, Reg32(5), 0, 0xD9);
        }

        void fldenv(Address addr)
        {
            opMR(addr, Reg32(4), 0, 0xD9);
        }
        ...
        {
            opMR(addr, Reg32(6), 0, 0xDD);
        }

        void fnstcw(Address addr)
        {
            opMR(addr, Reg32(7), 0, 0xD9);
        }

        void fnstenv(Address addr)
        {
            opMR(addr, Reg32(6), 0, 0xD9);
        }

        void fnstsw(Address addr)
        {
            opMR(addr, Reg32(7), 0, 0xDD);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0xCF, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void gf2p8affineqb(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0xCE, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void gf2p8mulb(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0xCF, &isXMM_XMMorMEM);
        }

        void haddpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F | T_YMM, 0x7C, &isXMM_XMMorMEM);
        }

        void haddps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F2 | T_0F | T_YMM, 0x7C, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F | T_YMM, 0x7D, &isXMM_XMMorMEM);
        }

        void hsubps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F2 | T_0F | T_YMM, 0x7D, &isXMM_XMMorMEM);
        }

        void idiv(Operand op)
        {
            opRext(op, 0, 7, T_APX | T_NF | T_CODE1_IF1, 0xF6);
        }

        void imul(Operand op)
        {
            opRext(op, 0, 5, T_APX | T_NF | T_CODE1_IF1, 0xF6);
        }

        void imul(Reg d, Reg reg, Operand op)
        {
            opROO(d, op, reg, T_APX | T_ND1 | T_NF, 0xAF);
        }
        ...
        void inc(Reg d, Operand op)
        {
            opIncDec(d, op, 0);
        }

        void insertps(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x21, &isXMM_XMMorMEM, imm);
        }
        ...
        {
            ja(to!string(label), type);
        }

        void ja(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x77, 0x87, 0x0F);
        }

        void ja(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x77, 0x87, 0x0F);
        }

        void jae(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jae(to!string(label), type);
        }

        void jae(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x73, 0x83, 0x0F);
        }

        void jae(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x73, 0x83, 0x0F);
        }

        void jb(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jb(to!string(label), type);
        }

        void jb(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x72, 0x82, 0x0F);
        }

        void jb(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x72, 0x82, 0x0F);
        }

        void jbe(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jbe(to!string(label), type);
        }

        void jbe(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x76, 0x86, 0x0F);
        }

        void jbe(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x76, 0x86, 0x0F);
        }

        void jc(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x72, 0x82, 0x0F);
        }
        ...
        void jc(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x72, 0x82, 0x0F);
        }

        void je(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            je(to!string(label), type);
        }

        void je(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x74, 0x84, 0x0F);
        }

        void je(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x74, 0x84, 0x0F);
        }

        void jg(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jg(to!string(label), type);
        }

        void jg(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7F, 0x8F, 0x0F);
        }

        void jg(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7F, 0x8F, 0x0F);
        }

        void jge(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jge(to!string(label), type);
        }

        void jge(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7D, 0x8D, 0x0F);
        }

        void jge(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7D, 0x8D, 0x0F);
        }

        void jl(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jl(to!string(label), type);
        }

        void jl(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7C, 0x8C, 0x0F);
        }

        void jl(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7C, 0x8C, 0x0F);
        }

        void jle(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jle(to!string(label), type);
        }

        void jle(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7E, 0x8E, 0x0F);
        }

        void jle(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7E, 0x8E, 0x0F);
        }

        void jna(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x76, 0x86, 0x0F);
        }
        ...
        void jna(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x76, 0x86, 0x0F);
        }

        void jnae(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x72, 0x82, 0x0F);
        }
        ...
        void jnae(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x72, 0x82, 0x0F);
        }

        void jnb(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x73, 0x83, 0x0F);
        }
        ...
        void jnb(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x73, 0x83, 0x0F);
        }

        void jnbe(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x77, 0x87, 0x0F);
        }
        ...
        void jnbe(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x77, 0x87, 0x0F);
        }

        void jnc(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x73, 0x83, 0x0F);
        }
        ...
        void jnc(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x73, 0x83, 0x0F);
        }

        void jne(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jne(to!string(label), type);
        }

        void jne(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x75, 0x85, 0x0F);
        }

        void jne(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x75, 0x85, 0x0F);
        }

        void jng(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7E, 0x8E, 0x0F);
        }
        ...
        void jng(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7E, 0x8E, 0x0F);
        }

        void jnge(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7C, 0x8C, 0x0F);
        }
        ...
        void jnge(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7C, 0x8C, 0x0F);
        }

        void jnl(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7D, 0x8D, 0x0F);
        }
        ...
        void jnl(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7D, 0x8D, 0x0F);
        }

        void jnle(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7F, 0x8F, 0x0F);
        }
        ...
        void jnle(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7F, 0x8F, 0x0F);
        }

        void jno(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jno(to!string(label), type);
        }

        void jno(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x71, 0x81, 0x0F);
        }

        void jno(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x71, 0x81, 0x0F);
        }

        void jnp(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jnp(to!string(label), type);
        }

        void jnp(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7B, 0x8B, 0x0F);
        }

        void jnp(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7B, 0x8B, 0x0F);
        }

        void jns(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jns(to!string(label), type);
        }

        void jns(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x79, 0x89, 0x0F);
        }

        void jns(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x79, 0x89, 0x0F);
        }

        void jnz(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x75, 0x85, 0x0F);
        }
        ...
        void jnz(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x75, 0x85, 0x0F);
        }

        void jo(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jo(to!string(label), type);
        }

        void jo(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x70, 0x80, 0x0F);
        }

        void jo(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x70, 0x80, 0x0F);
        }

        void jp(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            jp(to!string(label), type);
        }

        void jp(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x7A, 0x8A, 0x0F);
        }

        void jp(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7A, 0x8A, 0x0F);
        }

        void jpe(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7A, 0x8A, 0x0F);
        }
        ...
        void jpe(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7A, 0x8A, 0x0F);
        }

        void jpo(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7B, 0x8B, 0x0F);
        }
        ...
        void jpo(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x7B, 0x8B, 0x0F);
        }

        void js(ref Label label, LabelType type = T_AUTO)
        {
        ...
        {
            js(to!string(label), type);
        }

        void js(const void* addr)
        {
            opJmpAbs(addr, T_NEAR, 0x78, 0x88, 0x0F);
        }

        void js(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x78, 0x88, 0x0F);
        }

        void jz(ref Label label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x74, 0x84, 0x0F);
        }
        ...
        void jz(string label, LabelType type = T_AUTO)
        {
            opJmp(label, type, 0x74, 0x84, 0x0F);
        }
        ...
        {
            opSSE(xmm, addr, T_F2 | T_0F, 0xF0);
        }

        void ldmxcsr(Address addr)
        {
            opMR(addr, Reg32(2), T_0F, 0xAE);
        }
        ...
        {
            opLoadSeg(addr, reg, T_0F, 0xB4);
        }

        void lgs(Reg reg, Address addr)
        {
            opLoadSeg(addr, reg, T_0F, 0xB5);
        }
        ...
        void loop(ref Label label)
        {
            opJmp(label, T_SHORT, 0xE2, 0, 0);
        }

        void loop(const char* label)
        {
            loop(to!string(label));
        }
        ...
        void loope(ref Label label)
        {
            opJmp(label, T_SHORT, 0xE1, 0, 0);
        }

        void loope(const char* label)
        {
            loope(to!string(label));
        }
        ...
        {
            opSSE(reg1, reg2, T_66 | T_0F, 0xF7);
        }

        void maskmovq(Mmx reg1, Mmx reg2)
        {
            opSSE(reg1, reg2, T_0F, 0xF7);
        }

        void maxpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x5F, &isXMM_XMMorMEM);
        }

        void maxps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5F, &isXMM_XMMorMEM);
        }

        void maxsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x5F, &isXMM_XMMorMEM);
        }

        void maxss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x5F, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_0F | T_66, 0x5D, &isXMM_XMMorMEM);
        }

        void minps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5D, &isXMM_XMMorMEM);
        }

        void minsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x5D, &isXMM_XMMorMEM);
        }

        void minss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x5D, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, addr, T_0F | T_66, 0x29);
        }

        void movapd(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x28, T_0F, T_66);
        }

        void movaps(Address addr, Xmm xmm)
        {
            opSSE(xmm, addr, T_0F | T_NONE, 0x29);
        }

        void movaps(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x28, T_0F, T_NONE);
        }

        void movbe(Address addr, Reg reg)
        {
            opMR(addr, reg, T_0F38, 0xF1, T_APX, 0x61);
        }

        void movbe(Reg reg, Address addr)
        {
            opMR(addr, reg, T_0F38, 0xF0, T_APX, 0x60);
        }

        void movd(Mmx mmx, Operand op)
        {
            if (!(op.isMEM() || op.isREG(32)))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            }
            if (mmx.isXMM())
                db(0x66);
            opSSE(mmx, op, T_0F | T_ALLOW_DIFF_SIZE, 0x6E);
        }

        void movd(Operand op, Mmx mmx)
        {
            if (!(op.isMEM() || op.isREG(32)))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            ...
            if (mmx.isXMM())
                db(0x66);
            opSSE(mmx, op, T_0F | T_ALLOW_DIFF_SIZE, 0x7E);
        }
        ...
        void movddup(Xmm xmm, Operand op)
        {
            opSSE(
                xmm,
                op,
                T_DUP | T_F2 | T_0F | T_EW1 | T_YMM | T_EVEX | T_ER_X | T_ER_Y | T_ER_Z,
                0x12,
                &isXMM_XMMorMEM, NONE
            );
        }

        void movdir64b(Reg reg, Address addr)
        {
            opMR(addr, reg.cvt32(), T_66 | T_0F38, 0xF8, T_APX | T_66);
        }

        void movdiri(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F38, 0xF9, T_APX);
        }

        void movdq2q(Mmx mmx, Xmm xmm)
        {
            opSSE(mmx, xmm, T_F2 | T_0F, 0xD6);
        }

        void movdqa(Address addr, Xmm xmm)
        {
            opSSE(xmm, addr, T_0F | T_66, 0x7F);
        }

        void movdqa(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x6F, T_0F, T_66);
        }

        void movdqu(Address addr, Xmm xmm)
        {
            opSSE(xmm, addr, T_0F | T_F3, 0x7F);
        }

        void movdqu(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x6F, T_0F, T_F3);
        }

        void movhlps(Xmm reg1, Xmm reg2)
        {
            opSSE(reg1, reg2, T_0F, 0x12);
        }

        void movhpd(Operand op1, Operand op2)
        {
            opMovXMM(op1, op2, T_66 | T_0F, 0x16);
        }

        void movhps(Operand op1, Operand op2)
        {
            opMovXMM(op1, op2, T_0F, 0x16);
        }

        void movlhps(Xmm reg1, Xmm reg2)
        {
            opSSE(reg1, reg2, T_0F, 0x16);
        }

        void movlpd(Operand op1, Operand op2)
        {
            opMovXMM(op1, op2, T_66 | T_0F, 0x12);
        }

        void movlps(Operand op1, Operand op2)
        {
            opMovXMM(op1, op2, T_0F, 0x12);
        }
        ...
        void movntdq(Address addr, Xmm reg)
        {
            ...
            opSSE(Reg16(reg.getIdx()), addr, T_0F, 0xE7);
        }

        void movntdqa(Xmm xmm, Address addr)
        {
            opSSE(xmm, addr, T_66 | T_0F38, 0x2A);
        }

        void movnti(Address addr, Reg32e reg)
        {
            opMR(addr, reg, T_0F, 0xC3);
        }
        ...
        void movntpd(Address addr, Xmm reg)
        {
            ...
            opSSE(Reg16(reg.getIdx()), addr, T_0F, 0x2B);
        }

        void movntps(Address addr, Xmm xmm)
        {
            opSSE(Xmm(xmm.getIdx()), addr, T_0F, 0x2B);
        }
        ...
        {
            if (!op.isMEM() && mmx.getKind() != op.getKind())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            }
            if (mmx.isXMM())
                db(0xF3);
            opSSE(mmx, op, T_0F | T_ALLOW_DIFF_SIZE, mmx.isXMM() ? 0x7E : 0x6F);
        }

        void movq2dq(Xmm xmm, Mmx mmx)
        {
            opSSE(xmm, mmx, T_F3 | T_0F, 0xD6);
        }
        ...
        {
            opSSE(xmm, addr, T_0F | T_F2, 0x11);
        }

        void movsd(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x10, T_0F, T_F2);
        }

        void movshdup(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F3 | T_0F | T_EW0 | T_YMM | T_EVEX, 0x16, &isXMM_XMMorMEM, NONE);
        }

        void movsldup(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_F3 | T_0F | T_EW0 | T_YMM | T_EVEX, 0x12, &isXMM_XMMorMEM, NONE);
        }

        void movss(Address addr, Xmm xmm)
        {
            opSSE(xmm, addr, T_0F | T_F3, 0x11);
        }

        void movss(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x10, T_0F, T_F3);
        }
        ...
        {
            opSSE(xmm, addr, T_0F | T_66, 0x11);
        }

        void movupd(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x10, T_0F, T_66);
        }

        void movups(Address addr, Xmm xmm)
        {
            opSSE(xmm, addr, T_0F | T_NONE, 0x11);
        }

        void movups(Xmm xmm, Operand op)
        {
            opMMX(xmm, op, 0x10, T_0F, T_NONE);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x42, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void mul(Operand op)
        {
            opRext(op, 0, 4, T_APX | T_NF | T_CODE1_IF1, 0xF6);
        }

        void mulpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x59, &isXMM_XMMorMEM);
        }

        void mulps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x59, &isXMM_XMMorMEM);
        }

        void mulsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x59, &isXMM_XMMorMEM);
        }

        void mulss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x59, &isXMM_XMMorMEM);
        }

        void mulx(Reg32e r1, Reg32e r2, Operand op)
        {
            opRRO(r1, r2, op, T_APX | T_F2 | T_0F38, 0xf6);
        }
        ...
        void neg(Operand op)
        {
            opRext(op, 0, 3, T_APX | T_NF | T_CODE1_IF1, 0xF6);
        }

        void neg(Reg d, Operand op)
        {
            opROO(d, op, Reg(3, Kind.REG, d.getBit()), T_APX | T_NF | T_CODE1_IF1 | T_ND1, 0xF6);
        }

        void not_(Operand op)
        {
            opRext(op, 0, 2, T_APX | T_CODE1_IF1, 0xF6);
        }

        void not_(Reg d, Operand op)
        {
            opROO(d, op, Reg(2, Kind.REG, d.getBit()), T_APX | T_CODE1_IF1 | T_ND1, 0xF6);
        }

        void or_(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x08, 1);
        }

        void or_(Operand op1, Operand op2)
        {
            opRO_MR(op1, op2, 0x08);
        }

        void or_(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NF | T_CODE1_IF1, 1);
        }

        void or_(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NF | T_CODE1_IF1, 0x08);
        }

        void orpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x56, &isXMM_XMMorMEM);
        }

        void orps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x56, &isXMM_XMMorMEM);
        }
        ...
        {
            opMMX(mmx, op, 0x1C, T_0F38, T_66);
        }

        void pabsd(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x1E, T_0F38, T_66);
        }

        void pabsw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x1D, T_0F38, T_66);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x2B, &isXMM_XMMorMEM);
        }
        ...
        {
            opMMX(mmx, op, 0x0F, T_0F3A, T_66, cast(uint8_t) imm);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x10, &isXMM_XMMorMEM, NONE);
        }

        void pblendw(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x0E, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void pclmulhqhqdq(Xmm xmm, Operand op)
        {
        ...
        void pclmullqhqdq(Xmm xmm, Operand op)
        {
        ...
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x44, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x29, &isXMM_XMMorMEM);
        }
        ...
        void pcmpestri(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x61, &isXMM_XMMorMEM, imm);
        }

        void pcmpestrm(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x60, &isXMM_XMMorMEM, imm);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x37, &isXMM_XMMorMEM);
        }
        ...
        void pcmpistri(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x63, &isXMM_XMMorMEM, imm);
        }

        void pcmpistrm(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x62, &isXMM_XMMorMEM, imm);
        }

        void pdep(Reg32e r1, Reg32e r2, Operand op)
        {
            opRRO(r1, r2, op, T_APX | T_F2 | T_0F38, 0xf5);
        }

        void pext(Reg32e r1, Reg32e r2, Operand op)
        {
            opRRO(r1, r2, op, T_APX | T_F3 | T_0F38, 0xf5);
        }

        void pextrb(Operand op, Xmm xmm, uint8_t imm)
        {
            opExt(op, xmm, 0x14, imm);
        }

        void pextrd(Operand op, Xmm xmm, uint8_t imm)
        {
            opExt(op, xmm, 0x16, imm);
        }

        void pextrw(Operand op, Mmx xmm, uint8_t imm)
        {
        ...
        {
            opMMX(mmx, op, 0x02, T_0F38, T_66);
        }

        void phaddsw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x03, T_0F38, T_66);
        }

        void phaddw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x01, T_0F38, T_66);
        }

        void phminposuw(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x41, &isXMM_XMMorMEM, NONE);
        }

        void phsubd(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x06, T_0F38, T_66);
        }

        void phsubsw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x07, T_0F38, T_66);
        }

        void phsubw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x05, T_0F38, T_66);
        }

        void pinsrb(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x20, &isXMM_REG32orMEM, imm);
        }

        void pinsrd(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x22, &isXMM_REG32orMEM, imm);
        }
        ...
        void pinsrw(Mmx mmx, Operand op, int imm)
        {
            ...
            opSSE(mmx, op, T_0F | (mmx.isXMM() ? T_66 : T_NONE), 0xC4, null, imm);
        }

        void pmaddubsw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x04, T_0F38, T_66);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3C, &isXMM_XMMorMEM);
        }

        void pmaxsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3D, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3F, &isXMM_XMMorMEM);
        }

        void pmaxuw(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3E, &isXMM_XMMorMEM);
        }

        void pminsb(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x38, &isXMM_XMMorMEM);
        }

        void pminsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x39, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3B, &isXMM_XMMorMEM);
        }

        void pminuw(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x3A, &isXMM_XMMorMEM);
        }
        ...
        void pmovmskb(Reg32e reg, Mmx mmx)
        {
            if (mmx.isXMM())
                db(0x66);
            opSSE(reg, mmx, T_0F, 0xD7);
        }

        void pmovsxbd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N4 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x21, &isXMM_XMMorMEM, NONE);
        }

        void pmovsxbq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N2 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x22, &isXMM_XMMorMEM, NONE);
        }

        void pmovsxbw(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x20, &isXMM_XMMorMEM, NONE);
        }

        void pmovsxdq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_EW0 | T_YMM | T_EVEX, 0x25, &isXMM_XMMorMEM, NONE);
        }

        void pmovsxwd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x23, &isXMM_XMMorMEM, NONE);
        }

        void pmovsxwq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N4 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x24, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxbd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N4 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x31, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxbq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N2 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x32, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxbw(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x30, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxdq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_EW0 | T_YMM | T_EVEX, 0x35, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxwd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N8 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x33, &isXMM_XMMorMEM, NONE);
        }

        void pmovzxwq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_N4 | T_N_VL | T_66 | T_0F38 | T_YMM | T_EVEX, 0x34, &isXMM_XMMorMEM, NONE);
        }

        void pmuldq(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x28, &isXMM_XMMorMEM);
        }

        void pmulhrsw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x0B, T_0F38, T_66);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38, 0x40, &isXMM_XMMorMEM);
        }
        ...
        void prefetchit0(Address addr)
        {
            opMR(addr, Reg32(7), T_0F, 0x18);
        }

        void prefetchit1(Address addr)
        {
            opMR(addr, Reg32(6), T_0F, 0x18);
        }

        void prefetchnta(Address addr)
        {
            opMR(addr, Reg32(0), T_0F, 0x18);
        }

        void prefetcht0(Address addr)
        {
            opMR(addr, Reg32(1), T_0F, 0x18);
        }

        void prefetcht1(Address addr)
        {
            opMR(addr, Reg32(2), T_0F, 0x18);
        }

        void prefetcht2(Address addr)
        {
            opMR(addr, Reg32(3), T_0F, 0x18);
        }

        void prefetchw(Address addr)
        {
            opMR(addr, Reg32(1), T_0F, 0x0D);
        }

        void prefetchwt1(Address addr)
        {
            opMR(addr, Reg32(2), T_0F, 0x0D);
        }
        ...
        {
            opMMX(mmx, op, 0x00, T_0F38, T_66);
        }

        void pshufd(Mmx mmx, Operand op, uint8_t imm8)
        {
            opMMX(mmx, op, 0x70, T_0F, T_66, imm8);
        }

        void pshufhw(Mmx mmx, Operand op, uint8_t imm8)
        {
            opMMX(mmx, op, 0x70, T_0F, T_F3, imm8);
        }

        void pshuflw(Mmx mmx, Operand op, uint8_t imm8)
        {
            opMMX(mmx, op, 0x70, T_0F, T_F2, imm8);
        }

        void pshufw(Mmx mmx, Operand op, uint8_t imm8)
        {
            opMMX(mmx, op, 0x70, T_0F, T_NONE, imm8);
        }

        void psignb(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x08, T_0F38, T_66);
        }

        void psignd(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x0A, T_0F38, T_66);
        }

        void psignw(Mmx mmx, Operand op)
        {
            opMMX(mmx, op, 0x09, T_0F38, T_66);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F38 | T_YMM, 0x17, &isXMM_XMMorMEM, NONE);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F, 0x6D, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F, 0x6C, &isXMM_XMMorMEM);
        }
        ...
        void rcl(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 2, d);
        }

        void rcl(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 2, d);
        }
        ...
        {
            opSSE(xmm, op, T_0F, 0x53, &isXMM_XMMorMEM);
        }

        void rcpss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x53, &isXMM_XMMorMEM);
        }
        ...
        void rcr(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 3, d);
        }

        void rcr(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 3, d);
        }
        ...
        void rol(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 8, d);
        }

        void rol(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 8, d);
        }
        ...
        void ror(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 9, d);
        }

        void ror(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 9, d);
        }

        void rorx(Reg32e r, Operand op, uint8_t imm)
        {
            opRRO(r, Reg32e(0, r.getBit()), op, T_0F3A | T_F2 | T_APX, 0xF0, imm);
        }

        void roundpd(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A | T_YMM, 0x09, &isXMM_XMMorMEM, imm);
        }

        void roundps(Xmm xmm, Operand op, uint8_t imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A | T_YMM, 0x08, &isXMM_XMMorMEM, imm);
        }

        void roundsd(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x0B, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void roundss(Xmm xmm, Operand op, int imm)
        {
            opSSE(xmm, op, T_66 | T_0F3A, 0x0A, &isXMM_XMMorMEM, cast(uint8_t) imm);
        }

        void rsqrtps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x52, &isXMM_XMMorMEM);
        }

        void rsqrtss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x52, &isXMM_XMMorMEM);
        }
        ...
        void sal(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 12, d);
        }

        void sal(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 12, d);
        }
        ...
        void sar(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 15, d);
        }

        void sar(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 15, d);
        }
        ...
        {
            opRRO(r1, r2, op, T_APX | T_F3 | T_0F38, 0xf7);
        }

        void sbb(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x18, 3);
        }
        ...
        {
            opRO_MR(op1, op2, 0x18);
        }

        void sbb(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NONE, 3);
        }

        void sbb(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NONE, 0x18);
        }
        ...
        void sha1msg1(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xC9, T_MUST_EVEX, 0xD9);
        }

        void sha1msg2(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xCA, T_MUST_EVEX, 0xDA);
        }

        void sha1nexte(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xC8, T_MUST_EVEX, 0xD8);
        }

        void sha1rnds4(Xmm x, Operand op, uint8_t imm)
        {
            opSSE_APX(x, op, T_0F3A, 0xCC, T_MUST_EVEX, 0xD4, imm);
        }

        void sha256msg1(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xCC, T_MUST_EVEX, 0xDC);
        }

        void sha256msg2(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xCD, T_MUST_EVEX, 0xDD);
        }

        void sha256rnds2(Xmm x, Operand op)
        {
            opSSE_APX(x, op, T_0F38, 0xCB, T_MUST_EVEX, 0xDB);
        }
        ...
        void shl(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 12, d);
        }

        void shl(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 12, d);
        }
        ...
        {
            opShxd(Reg(), op, reg, 0, 0xA4, 0x24, _cl);
        }

        void shld(Operand op, Reg reg, uint8_t imm)
        {
            opShxd(Reg(), op, reg, imm, 0xA4, 0x24);
        }

        void shld(Reg d, Operand op, Reg reg, Reg8 _cl)
        {
            opShxd(d, op, reg, 0, 0xA4, 0x24, _cl);
        }

        void shld(Reg d, Operand op, Reg reg, uint8_t imm)
        {
            opShxd(d, op, reg, imm, 0xA4, 0x24);
        }

        void shlx(Reg32e r1, Operand op, Reg32e r2)
        {
            opRRO(r1, r2, op, T_APX | T_66 | T_0F38, 0xf7);
        }
        ...
        void shr(Reg d, Operand op, Reg8 _cl)
        {
            opShift(op, _cl, 13, d);
        }

        void shr(Reg d, Operand op, int imm)
        {
            opShift(op, imm, 13, d);
        }
        ...
        {
            opShxd(Reg(), op, reg, 0, 0xAC, 0x2C, _cl);
        }

        void shrd(Operand op, Reg reg, uint8_t imm)
        {
            opShxd(Reg(), op, reg, imm, 0xAC, 0x2C);
        }

        void shrd(Reg d, Operand op, Reg reg, Reg8 _cl)
        {
            opShxd(d, op, reg, 0, 0xAC, 0x2C, _cl);
        }

        void shrd(Reg d, Operand op, Reg reg, uint8_t imm)
        {
            opShxd(d, op, reg, imm, 0xAC, 0x2C);
        }

        void shrx(Reg32e r1, Operand op, Reg32e r2)
        {
            opRRO(r1, r2, op, T_APX | T_F2 | T_0F38, 0xf7);
        }

        void shufpd(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F | T_66, 0xC6, &isXMM_XMMorMEM, imm8);
        }

        void shufps(Xmm xmm, Operand op, uint8_t imm8)
        {
            opSSE(xmm, op, T_0F, 0xC6, &isXMM_XMMorMEM, imm8);
        }

        void sqrtpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x51, &isXMM_XMMorMEM);
        }

        void sqrtps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x51, &isXMM_XMMorMEM);
        }

        void sqrtsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x51, &isXMM_XMMorMEM);
        }

        void sqrtss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x51, &isXMM_XMMorMEM);
        }
        ...
        void sub(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x28, 5);
        }
        ...
        {
            opRO_MR(op1, op2, 0x28);
        }

        void sub(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NF | T_CODE1_IF1, 5);
        }

        void sub(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NF | T_CODE1_IF1, 0x28);
        }

        void subpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x5C, &isXMM_XMMorMEM);
        }

        void subps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x5C, &isXMM_XMMorMEM);
        }

        void subsd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F2, 0x5C, &isXMM_XMMorMEM);
        }

        void subss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_F3, 0x5C, &isXMM_XMMorMEM);
        }
        ...
        {
            if (opROO(Reg(), op, reg, T_APX | T_NF, 0xF4))
                return;
            opCnt(reg, op, 0xBC);
        }
        ...
        {
            opSSE(xmm, op, T_66 | T_0F, 0x2E, &isXMM_XMMorMEM);
        }

        void ucomiss(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x2E, &isXMM_XMMorMEM);
        }
        ...
        {
            opSSE(xmm, op, T_0F | T_66, 0x15, &isXMM_XMMorMEM);
        }

        void unpckhps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x15, &isXMM_XMMorMEM);
        }

        void unpcklpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x14, &isXMM_XMMorMEM);
        }

        void unpcklps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x14, &isXMM_XMMorMEM);
        }

        void vaddpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x58);
        }

        void vaddps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x58);
        }

        void vaddsd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F2 | T_EW1 | T_EVEX | T_ER_X | T_N8, 0x58);
        }

        void vaddss(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_EW0 | T_EVEX | T_ER_X | T_N4, 0x58);
        }

        void vaddsubpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F | T_YMM, 0xD0);
        }

        void vaddsubps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_F2 | T_0F | T_YMM, 0xD0);
        }

        void vaesdec(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDE);
        }

        void vaesdeclast(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDF);
        }

        void vaesenc(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F38 | T_YMM | T_EVEX, 0xDC);
        }

        void vaesenclast(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
        ...
        void vaeskeygenassist(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A, 0xDF, imm);
        }

        void vandnpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x55);
        }

        void vandnps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x55);
        }

        void vandpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x54);
        }

        void vandps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x54);
        }

        void vbcstnebf162ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_F3 | T_0F38 | T_W0 | T_YMM | T_B16, 0xB1);
        }

        void vbcstnesh2ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_66 | T_0F38 | T_W0 | T_YMM | T_B16, 0xB1);
        }

        void vblendpd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_W0 | T_YMM, 0x0D, imm);
        }

        void vblendps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vcmppd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F | T_YMM, 0xC2, imm);
        }

        void vcmpps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_0F | T_YMM, 0xC2, imm);
        }

        void vcmpsd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_F2 | T_0F, 0xC2, imm);
        }

        void vcmpss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        {
            checkCvt1(x, op);
            opVex(x, null, op, T_0F | T_F3 | T_YMM | T_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL, 0xE6);
        }
        ...
        void vcvtneebf162ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_F3 | T_0F38 | T_W0 | T_YMM, 0xB0);
        }

        void vcvtneeph2ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_66 | T_0F38 | T_W0 | T_YMM, 0xB0);
        }

        void vcvtneobf162ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_F2 | T_0F38 | T_W0 | T_YMM, 0xB0);
        }

        void vcvtneoph2ps(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_0F38 | T_W0 | T_YMM, 0xB0);
        }

        void vcvtneps2bf16(Xmm x, Operand op, PreferredEncoding encoding = DefaultEncoding)
        {
            opCvt2(
                x,
                op,
                T_F3 | T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_B32 | orEvexIf(encoding, 0, T_MUST_EVEX, 0),
                0x72
            );
        }
        ...
        {
            checkCvt1(x, op);
            opVex(x, null, op, T_0F38 | T_66 | T_W0 | T_EVEX | T_EW0 | T_N8 | T_N_VL | T_SAE_Y, 0x13);
        }
        ...
        {
            checkCvt1(x, op);
            opVex(x, null, op, T_0F | T_YMM | T_EVEX | T_EW0 | T_B32 | T_N8 | T_N_VL | T_SAE_Y, 0x5A);
        }

        void vcvtps2ph(Operand op, Xmm x, uint8_t imm)
        {
            checkCvt1(x, op);
            opVex(x, null, op, T_0F3A | T_66 | T_W0 | T_EVEX | T_EW0 | T_N8 | T_N_VL | T_SAE_Y | T_M_K, 0x1D, imm);
        }
        ...
        {
            opCvt2(x, op, T_66 | T_0F | T_YMM | T_EVEX | T_EW1 | T_B64 | T_SAE_Z, 0xE6);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x98);
        }

        void vfmadd132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x98);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xA8);
        }

        void vfmadd213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA8);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xB8);
        }

        void vfmadd231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB8);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x96);
        }

        void vfmaddsub132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x96);
        }

        void vfmaddsub213pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xA6);
        }

        void vfmaddsub213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA6);
        }

        void vfmaddsub231pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xB6);
        }

        void vfmaddsub231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB6);
        }

        void vfmsub132pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x9A);
        }

        void vfmsub132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9A);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xAA);
        }

        void vfmsub213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAA);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xBA);
        }

        void vfmsub231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBA);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x97);
        }

        void vfmsubadd132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x97);
        }

        void vfmsubadd213pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xA7);
        }

        void vfmsubadd213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xA7);
        }

        void vfmsubadd231pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xB7);
        }

        void vfmsubadd231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xB7);
        }

        void vfnmadd132pd(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x9C);
        }

        void vfnmadd132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9C);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xAC);
        }

        void vfnmadd213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAC);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xBC);
        }

        void vfnmadd231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBC);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x9E);
        }

        void vfnmsub132ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x9E);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xAE);
        }

        void vfnmsub213ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xAE);
        }
        ...
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W1 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0xBE);
        }

        void vfnmsub231ps(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0xBE);
        }
        ...
        void vgf2p8affineinvqb(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_W1 | T_EW1 | T_YMM | T_EVEX | T_SAE_Z | T_B64, 0xCF, imm);
        }

        void vgf2p8affineqb(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vhaddpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F | T_YMM, 0x7C);
        }

        void vhaddps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_F2 | T_0F | T_YMM, 0x7C);
        }

        void vhsubpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_66 | T_0F | T_YMM, 0x7D);
        }

        void vhsubps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
        ...
        void vmaxpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_SAE_Z | T_B64, 0x5F);
        }

        void vmaxps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_SAE_Z | T_B32, 0x5F);
        }

        void vmaxsd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F2 | T_EW1 | T_EVEX | T_SAE_X | T_N8, 0x5F);
        }

        void vmaxss(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_EW0 | T_EVEX | T_SAE_X | T_N4, 0x5F);
        }

        void vminpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_SAE_Z | T_B64, 0x5D);
        }

        void vminps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_SAE_Z | T_B32, 0x5D);
        }

        void vminsd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F2 | T_EW1 | T_EVEX | T_SAE_X | T_N8, 0x5D);
        }

        void vminss(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F3 | T_EW0 | T_EVEX | T_SAE_X | T_N4, 0x5D);
        }
        ...
        {
            opAVX_X_X_XM(x, xm0, addr, T_N8 | T_66 | T_0F | T_EW1 | T_EVEX, 0x17);
        }
        ...
        {
            opAVX_X_X_XM(x, xm0, addr, T_N8 | T_0F | T_EW0 | T_EVEX, 0x17);
        }
        ...
        void vmovlhps(Xmm x1, Xmm x2, Operand op = Operand())
        {
            if (!op.isNone() && !op.isXMM())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        {
            opAVX_X_X_XM(x, xm0, addr, T_N8 | T_66 | T_0F | T_EW1 | T_EVEX, 0x13);
        }

        void vmovlpd(Xmm x, Operand op1, Operand op2 = Operand())
        {
            if (!op2.isNone() && !op2.isMEM())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        {
            opAVX_X_X_XM(x, xm0, addr, T_N8 | T_0F | T_EW0 | T_EVEX, 0x13);
        }

        void vmovlps(Xmm x, Operand op1, Operand op2 = Operand())
        {
            if (!op2.isNone() && !op2.isMEM())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vmovmskpd(Reg r, Xmm x)
        {
            ...
            opAVX_X_X_XM(
                x.isXMM() ? Xmm(r.getIdx()) : Ymm(r.getIdx()), cvtIdx0(x), x, T_0F | T_66 | T_W0 | T_YMM, 0x50
            );
        }
        ...
        {
            opVex(x, null, addr, T_0F | T_66 | T_YMM | T_EVEX | T_EW0, 0xE7);
        }

        void vmovntdqa(Xmm x, Address addr)
        {
            opVex(x, null, addr, T_0F38 | T_66 | T_YMM | T_EVEX | T_EW0, 0x2A);
        }

        void vmovntpd(Address addr, Xmm x)
        {
            opVex(x, null, addr, T_0F | T_66 | T_YMM | T_EVEX | T_EW1, 0x2B);
        }

        void vmovntps(Address addr, Xmm x)
        {
            opVex(x, null, addr, T_0F | T_YMM | T_EVEX | T_EW0, 0x2B);
        }

        void vmovq(Address addr, Xmm x)
        {
            opAVX_X_X_XM(x, xm0, addr, T_0F | T_66 | T_EVEX | T_EW1 | T_N8, x.getIdx() < 16 ? 0xD6
                    : 0x7E);
        }
        ...
        void vmovsd(Xmm x1, Xmm x2, Operand op = Operand())
        {
            if (!op.isNone() && !op.isXMM())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vmovss(Xmm x1, Xmm x2, Operand op = Operand())
        {
            if (!op.isNone() && !op.isXMM())
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vpalignr(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpblendd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpblendw(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpclmulhqhqdq(Xmm x1, Xmm x2, Operand op)
        {
            vpclmulqdq(x1, x2, op, 0x11);
        }

        void vpclmulhqlqdq(Xmm x1, Xmm x2, Operand op)
        {
            vpclmulqdq(x1, x2, op, 0x01);
        }

        void vpclmullqhqdq(Xmm x1, Xmm x2, Operand op)
        {
            vpclmulqdq(x1, x2, op, 0x10);
        }

        void vpclmullqlqdq(Xmm x1, Xmm x2, Operand op)
        {
            vpclmulqdq(x1, x2, op, 0x00);
        }

        void vpclmulqdq(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpcmpestri(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A, 0x61, imm);
        }

        void vpcmpestrm(Xmm xm, Operand op, uint8_t imm)
        {
        ...
        void vpcmpistri(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A, 0x63, imm);
        }

        void vpcmpistrm(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A, 0x62, imm);
        }

        void vpdpbusd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
        {
            opEncoding(x1, x2, op, T_66 | T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_B32, 0x50, encoding);
        }

        void vpdpbusds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
        {
            opEncoding(x1, x2, op, T_66 | T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_B32, 0x51, encoding);
        }

        void vpdpwssd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
        {
            opEncoding(x1, x2, op, T_66 | T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_B32, 0x52, encoding);
        }

        void vpdpwssds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
        {
            opEncoding(x1, x2, op, T_66 | T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_B32, 0x53, encoding);
        }

        void vperm2f128(Ymm y1, Ymm y2, Operand op, uint8_t imm)
        {
        ...
        void vperm2i128(Ymm y1, Ymm y2, Operand op, uint8_t imm)
        {
            if (!(y1.isYMM() && y2.isYMM() && op.isYMEM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vpermilpd(Xmm xm, Operand op, uint8_t imm)
        {
        ...
        void vpermilps(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_EVEX | T_B32, 0x04, imm);
        }

        void vpermpd(Ymm y, Operand op, uint8_t imm)
        {
        ...
        void vpermq(Ymm y, Operand op, uint8_t imm)
        {
        ...
        void vpextrb(Operand op, Xmm x, uint8_t imm)
        {
            if (!((op.isREG(8 | 16 | i32e) || op.isMEM()) && x.isXMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            }
            opVex(x, null, op, T_0F3A | T_66 | T_EVEX | T_N1, 0x14, imm);
        }

        void vpextrd(Operand op, Xmm x, uint8_t imm)
        {
            if (!((op.isREG(32) || op.isMEM()) && x.isXMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            }
            opVex(x, null, op, T_0F3A | T_66 | T_W0 | T_EVEX | T_EW0 | T_N4, 0x16, imm);
        }

        void vpextrq(Operand op, Xmm x, uint8_t imm)
        {
            if (!((op.isREG(64) || op.isMEM()) && x.isXMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            }
            opVex(x, null, op, T_0F3A | T_66 | T_W1 | T_EVEX | T_EW1 | T_N8, 0x16, imm);
        }

        void vpextrw(Operand op, Xmm x, uint8_t imm)
        {
            ...
            {
                opVex(x, null, op, T_0F3A | T_66 | T_EVEX | T_N2, 0x15, imm);
            }
        }
        ...
        void vpinsrb(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpinsrd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            if (!(x1.isXMM() && x2.isXMM() && (op.isREG(32) || op.isMEM())))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vpinsrq(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vpinsrw(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            if (!(x1.isXMM() && x2.isXMM() && (op.isREG(32) || op.isMEM())))
            {
                mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
        ...
        void vpshufd(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F | T_EW0 | T_YMM | T_EVEX | T_B32, 0x70, imm);
        }

        void vpshufhw(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_F3 | T_0F | T_YMM | T_EVEX, 0x70, imm);
        }

        void vpshuflw(Xmm xm, Operand op, uint8_t imm)
        {
        ...
        void vroundpd(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A | T_YMM, 0x09, imm);
        }

        void vroundps(Xmm xm, Operand op, uint8_t imm)
        {
            opAVX_X_XM_IMM(xm, op, T_66 | T_0F3A | T_YMM, 0x08, imm);
        }

        void vroundsd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_W0, 0x0B, imm);
        }

        void vroundss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
        ...
        void vsha512msg1(Ymm y, Xmm x)
        {
            if (!(y.isYMM() && x.isXMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_PARAMETER));
            }
            opVex(y, null, x, T_F2 | T_0F38 | T_W0 | T_YMM, 0xCC);
        }

        void vsha512msg2(Ymm y1, Ymm y2)
        {
            if (!(y1.isYMM() && y2.isYMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_PARAMETER));
            }
            opVex(y1, null, y2, T_F2 | T_0F38 | T_W0 | T_YMM, 0xCD);
        }

        void vsha512rnds2(Ymm y1, Ymm y2, Xmm x)
        {
            if (!(y1.isYMM() && y2.isYMM() && x.isXMM()))
            {
                mixin(XBYAK_THROW(ERR.BAD_PARAMETER));
            }
            opVex(y1, y2, x, T_F2 | T_0F38 | T_W0 | T_YMM, 0xCB);
        }

        void vshufpd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F | T_EW1 | T_YMM | T_EVEX | T_B64, 0xC6, imm);
        }

        void vshufps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_0F | T_EW0 | T_YMM | T_EVEX | T_B32, 0xC6, imm);
        }

        void vsm3msg1(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_0F38 | T_W0 | T_EW0 | T_EVEX, 0xDA);
        }

        void vsm3msg2(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_W0 | T_EW0 | T_EVEX, 0xDA);
        }

        void vsm3rnds2(Xmm x1, Xmm x2, Operand op, uint8_t imm)
        {
            opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_W0 | T_EW0 | T_EVEX, 0xDE, imm);
        }

        void vsm4key4(Xmm x1, Xmm x2, Operand op)
        {
            opAVX_X_X_XM(x1, x2, op, T_F3 | T_0F38 | T_W0 | T_EW0 | T_EVEX, 0xDA);
        }

        void vsm4rnds4(Xmm x1, Xmm x2, Operand op)
        {
        ...
        void vsubpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x5C);
        }

        void vsubps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_EW0 | T_YMM | T_EVEX | T_ER_Z | T_B32, 0x5C);
        }

        void vsubsd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_F2 | T_EW1 | T_EVEX | T_ER_X | T_N8, 0x5C);
        }

        void vsubss(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
        ...
        void vxorpd(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
            opAVX_X_X_XM(xmm, op1, op2, T_0F | T_66 | T_EW1 | T_YMM | T_EVEX | T_ER_Z | T_B64, 0x57);
        }

        void vxorps(Xmm xmm, Operand op1, Operand op2 = Operand())
        {
        ...
        void xor_(Operand op, uint32_t imm)
        {
            opOI(op, imm, 0x30, 6);
        }

        void xor_(Operand op1, Operand op2)
        {
            opRO_MR(op1, op2, 0x30);
        }

        void xor_(Reg d, Operand op, uint32_t imm)
        {
            opROI(d, op, imm, T_NF | T_CODE1_IF1, 6);
        }

        void xor_(Reg d, Operand op1, Operand op2)
        {
            opROO(d, op1, op2, T_NF | T_CODE1_IF1, 0x30);
        }

        void xorpd(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F | T_66, 0x57, &isXMM_XMMorMEM);
        }

        void xorps(Xmm xmm, Operand op)
        {
            opSSE(xmm, op, T_0F, 0x57, &isXMM_XMMorMEM);
        }
        ...
        version (XBYAK_ENABLE_OMITTED_OPERAND)
        {
            void vblendpd(Xmm x, Operand op, uint8_t imm)
            {
                vblendpd(x, x, op, imm);
            }

            void vblendps(Xmm x, Operand op, uint8_t imm)
            {
                vblendps(x, x, op, imm);
            }
            ...
            void vcmppd(Xmm x, Operand op, uint8_t imm)
            {
                vcmppd(x, x, op, imm);
            }

            void vcmpps(Xmm x, Operand op, uint8_t imm)
            {
                vcmpps(x, x, op, imm);
            }

            void vcmpsd(Xmm x, Operand op, uint8_t imm)
            {
                vcmpsd(x, x, op, imm);
            }

            void vcmpss(Xmm x, Operand op, uint8_t imm)
            {
                vcmpss(x, x, op, imm);
            }
            ...
            void vdppd(Xmm x, Operand op, uint8_t imm)
            {
                vdppd(x, x, op, imm);
            }

            void vdpps(Xmm x, Operand op, uint8_t imm)
            {
                vdpps(x, x, op, imm);
            }

            void vinsertps(Xmm x, Operand op, uint8_t imm)
            {
                vinsertps(x, x, op, imm);
            }

            void vmpsadbw(Xmm x, Operand op, uint8_t imm)
            {
                vmpsadbw(x, x, op, imm);
            }
            ...
            void vpalignr(Xmm x, Operand op, uint8_t imm)
            {
                vpalignr(x, x, op, imm);
            }
            ...
            void vpblendd(Xmm x, Operand op, uint8_t imm)
            {
                vpblendd(x, x, op, imm);
            }
            ...
            void vpblendw(Xmm x, Operand op, uint8_t imm)
            {
                vpblendw(x, x, op, imm);
            }

            void vpclmulqdq(Xmm x, Operand op, uint8_t imm)
            {
            ...
            void vpinsrb(Xmm x, Operand op, uint8_t imm)
            {
                vpinsrb(x, x, op, imm);
            }

            void vpinsrd(Xmm x, Operand op, uint8_t imm)
            {
                vpinsrd(x, x, op, imm);
            }

            void vpinsrq(Xmm x, Operand op, uint8_t imm)
            {
                vpinsrq(x, x, op, imm);
            }

            void vpinsrw(Xmm x, Operand op, uint8_t imm)
            {
                vpinsrw(x, x, op, imm);
            }
            ...
            void vpslld(Xmm x, uint8_t imm)
            {
                vpslld(x, x, imm);
            }

            void vpslldq(Xmm x, uint8_t imm)
            {
                vpslldq(x, x, imm);
            }
            ...
            void vpsrld(Xmm x, uint8_t imm)
            {
                vpsrld(x, x, imm);
            }

            void vpsrldq(Xmm x, uint8_t imm)
            {
                vpsrldq(x, x, imm);
            }
            ...
            void vroundsd(Xmm x, Operand op, uint8_t imm)
            {
                vroundsd(x, x, op, imm);
            }

            void vroundss(Xmm x, Operand op, uint8_t imm)
            {
                vroundss(x, x, op, imm);
            }
            ...
            void vshufpd(Xmm x, Operand op, uint8_t imm)
            {
                vshufpd(x, x, op, imm);
            }

            void vshufps(Xmm x, Operand op, uint8_t imm)
            {
                vshufps(x, x, op, imm);
            }
        ...
        version (XBYAK64)
        {
            ...
            {
                opMR(addr, Reg64(1), T_0F, 0xC7);
            }

            void fxrstor64(Address addr)
            {
                opMR(addr, Reg64(1), T_0F, 0xAE);
            }
            ...
            void movsxd(Reg64 reg, Operand op)
            {
                ...
                opRO(reg, op, T_ALLOW_DIFF_SIZE, 0x63);
            }

            void pextrq(Operand op, Xmm xmm, uint8_t imm)
            {
                if (!op.isREG(64) && !op.isMEM())
                {
                    mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
                }
                opSSE(Reg64(xmm.getIdx()), op, T_66 | T_0F3A, 0x16, null, imm);
            }

            void pinsrq(Xmm xmm, Operand op, uint8_t imm)
            {
                if (!op.isREG(64) && !op.isMEM())
                {
                    mixin(XBYAK_THROW(ERR.BAD_COMBINATION));
            ...
            void senduipi(Reg64 r)
            {
                opRR(Reg32(6), r.cvt32(), T_F3 | T_0F, 0xC7);
            }
            ...
            void push2(Reg64 r1, Reg64 r2)
            {
                opROO(r1, r2, Reg64(6), T_APX | T_ND1 | T_W0, 0xFF);
            }

            void push2p(Reg64 r1, Reg64 r2)
            {
                opROO(r1, r2, Reg64(6), T_APX | T_ND1 | T_W1, 0xFF);
            }

            void pop2(Reg64 r1, Reg64 r2)
            {
                opROO(r1, r2, Reg64(0), T_APX | T_ND1 | T_W0, 0x8F);
            }

            void pop2p(Reg64 r1, Reg64 r2)
            {
                opROO(r1, r2, Reg64(0), T_APX | T_ND1 | T_W1, 0x8F);
            }

            void cmpbexadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE6);
            }

            void cmpbxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE2);
            }

            void cmplexadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xEE);
            }

            void cmplxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xEC);
            }

            void cmpnbexadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE7);
            }

            void cmpnbxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE3);
            }

            void cmpnlexadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xEF);
            }

            void cmpnlxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xED);
            }

            void cmpnoxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE1);
            }

            void cmpnpxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xEB);
            }

            void cmpnsxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE9);
            }

            void cmpnzxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE5);
            }

            void cmpoxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE0);
            }

            void cmppxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xEA);
            }

            void cmpsxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE8);
            }

            void cmpzxadd(Address addr, Reg32e r1, Reg32e r2)
            {
                opRRO(r1, r2, addr, T_APX | T_66 | T_0F38, 0xE4);
            }

            void aesdec128kl(Xmm x, Address addr)
            {
                opSSE_APX(x, addr, T_F3 | T_0F38, 0xDD, T_F3 | T_MUST_EVEX, 0xDD);
            }

            void aesdec256kl(Xmm x, Address addr)
            {
                opSSE_APX(x, addr, T_F3 | T_0F38, 0xDF, T_F3 | T_MUST_EVEX, 0xDF);
            }

            void aesdecwide128kl(Address addr)
            {
                opSSE_APX(xmm1, addr, T_F3 | T_0F38, 0xD8, T_F3 | T_MUST_EVEX, 0xD8);
            }

            void aesdecwide256kl(Address addr)
            {
                opSSE_APX(xmm3, addr, T_F3 | T_0F38, 0xD8, T_F3 | T_MUST_EVEX, 0xD8);
            }

            void aesenc128kl(Xmm x, Address addr)
            {
                opSSE_APX(x, addr, T_F3 | T_0F38, 0xDC, T_F3 | T_MUST_EVEX, 0xDC);
            }

            void aesenc256kl(Xmm x, Address addr)
            {
                opSSE_APX(x, addr, T_F3 | T_0F38, 0xDE, T_F3 | T_MUST_EVEX, 0xDE);
            }

            void aesencwide128kl(Address addr)
            {
                opSSE_APX(xmm0, addr, T_F3 | T_0F38, 0xD8, T_F3 | T_MUST_EVEX, 0xD8);
            }

            void aesencwide256kl(Address addr)
            {
                opSSE_APX(xmm2, addr, T_F3 | T_0F38, 0xD8, T_F3 | T_MUST_EVEX, 0xD8);
            }

            void encodekey128(Reg32 r1, Reg32 r2)
            {
                opEncodeKey(r1, r2, 0xFA, 0xDA);
            }

            void encodekey256(Reg32 r1, Reg32 r2)
            {
                opEncodeKey(r1, r2, 0xFB, 0xDB);
            }

            void rdfsbase(Reg32e r)
            {
                opRR(eax, r, T_F3 | T_0F | T_ALLOW_DIFF_SIZE, 0xAE);
            }

            void rdgsbase(Reg32e r)
            {
                opRR(ecx, r, T_F3 | T_0F | T_ALLOW_DIFF_SIZE, 0xAE);
            }

            void wrfsbase(Reg32e r)
            {
                opRR(edx, r, T_F3 | T_0F | T_ALLOW_DIFF_SIZE, 0xAE);
            }

            void wrgsbase(Reg32e r)
            {
                opRR(ebx, r, T_F3 | T_0F | T_ALLOW_DIFF_SIZE, 0xAE);
            }

            void ldtilecfg(Address addr)
            {
                if (opROO(Reg(), addr, tmm0, T_APX | T_0F38 | T_W0, 0x49))
                    return;
                opVex(tmm0, tmm0, addr, T_0F38 | T_W0, 0x49);
            }

            void sttilecfg(Address addr)
            {
                if (opROO(Reg(), addr, tmm0, T_APX | T_66 | T_0F38 | T_W0, 0x49))
                    return;
                opVex(tmm0, tmm0, addr, T_66 | T_0F38 | T_W0, 0x49);
            }

            void tileloadd(Tmm tm, Address addr)
            {
                opAMX(tm, addr, T_F2 | T_0F38 | T_W0, 0x4B);
            }

            void tileloaddt1(Tmm tm, Address addr)
            {
                opAMX(tm, addr, T_66 | T_0F38 | T_W0, 0x4B);
            }
            ...
            void tilestored(Address addr, Tmm tm)
            {
                if (opROO(Reg(), addr, tm, T_APX | T_F3 | T_0F38 | T_W0, 0x4B))
                    return;
                opVex(tm, tmm0, addr, T_F3 | T_0F38 | T_W0, 0x4B);
            }

            void tilezero(Tmm Tmm)
            {
                opVex(Tmm, tmm0, tmm0, T_F2 | T_0F38 | T_W0, 0x49);
            }

            void tdpbssd(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F2 | T_0F38 | T_W0, 0x5e);
            }

            void tdpbsud(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F3 | T_0F38 | T_W0, 0x5e);
            }

            void tdpbusd(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_66 | T_0F38 | T_W0, 0x5e);
            }

            void tdpbuud(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_0F38 | T_W0, 0x5e);
            }

            void tdpfp16ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F2 | T_0F38 | T_W0, 0x5c);
            }

            void tdpbf16ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F3 | T_0F38 | T_W0, 0x5c);
            }

            void tileloaddrs(Tmm tm, Address addr)
            {
                opAMX(tm, addr, T_F2 | T_0F38 | T_W0, 0x4A);
            }

            void tileloaddrst1(Tmm tm, Address addr)
            {
                opAMX(tm, addr, T_66 | T_0F38 | T_W0, 0x4A);
            }

            void tdpbf8ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_MAP5 | T_W0, 0xFD);
            }

            void tdpbhf8ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F2 | T_MAP5 | T_W0, 0xFD);
            }

            void tdphbf8ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_F3 | T_MAP5 | T_W0, 0xFD);
            }

            void tdphf8ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_66 | T_MAP5 | T_W0, 0xFD);
            }

            void tmmultf32ps(Tmm x1, Tmm x2, Tmm x3)
            {
                opVex(x1, x3, x2, T_66 | T_0F38 | T_W0, 0x48);
            }
        }
        else
        {
            ...
            {
                opLoadSeg(addr, reg, T_NONE, 0xC5);
            }

            void les(Reg reg, Address addr)
            {
                opLoadSeg(addr, reg, T_NONE, 0xC4);
            }
        }
        ...
        else
        {
            ...
            void and(Operand op, uint32_t imm)
            {
                and_(op, imm);
            }

            void and(Reg d, Operand op, uint32_t imm)
            {
                and_(d, op, imm);
            }

            void and(Reg d, Operand op1, Operand op2)
            {
                and_(d, op1, op2);
            }
            ...
            void or(Operand op, uint32_t imm)
            {
                or_(op, imm);
            }

            void or(Reg d, Operand op, uint32_t imm)
            {
                or_(d, op, imm);
            }

            void or(Reg d, Operand op1, Operand op2)
            {
                or_(d, op1, op2);
            }
            ...
            void xor(Operand op, uint32_t imm)
            {
                xor_(op, imm);
            }

            void xor(Reg d, Operand op, uint32_t imm)
            {
                xor_(d, op, imm);
            }

            void xor(Reg d, Operand op1, Operand op2)
            {
                xor_(d, op1, op2);
            }
        ...
        else
        {
            ...
            {
                opKmov(k, addr, true, 32);
            }

            void kmovd(Opmask k, Operand op)
            {
                opKmov(k, op, false, 32);
            }
            ...
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W0, 0x44);
            }

            void knotd(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W1, 0x44);
            }

            void knotq(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W1, 0x44);
            }

            void knotw(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W0, 0x44);
            }
            ...
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W0, 0x98);
            }

            void kortestd(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W1, 0x98);
            }

            void kortestq(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W1, 0x98);
            }

            void kortestw(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W0, 0x98);
            }
            ...
            void kshiftlb(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W0, 0x32, imm);
            }

            void kshiftld(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W0, 0x33, imm);
            }

            void kshiftlq(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W1, 0x33, imm);
            }

            void kshiftlw(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W1, 0x32, imm);
            }

            void kshiftrb(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W0, 0x30, imm);
            }

            void kshiftrd(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W0, 0x31, imm);
            }

            void kshiftrq(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W1, 0x31, imm);
            }

            void kshiftrw(Opmask r1, Opmask r2, uint8_t imm)
            {
                opVex(r1, null, r2, T_66 | T_0F3A | T_W1, 0x30, imm);
            }

            void ktestb(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W0, 0x99);
            }

            void ktestd(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_66 | T_W1, 0x99);
            }

            void ktestq(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W1, 0x99);
            }

            void ktestw(Opmask r1, Opmask r2)
            {
                opVex(r1, null, r2, T_0F | T_W0, 0x99);
            }
            ...
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x58);
            }
            ...
            void vaddsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_F3 | T_EW0 | T_MUST_EVEX | T_ER_X | T_N2, 0x58);
            }

            void valignd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x03, imm);
            }

            void valignq(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vcmpbf16(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opVex(k, x, op, T_MUST_EVEX | T_F2 | T_0F3A | T_EW0 | T_YMM | T_B16, 0xC2, imm);
            }

            void vcmpeq_ospd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 16);
            }

            void vcmpeq_osps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 16);
            }

            void vcmpeq_ossd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 16);
            }

            void vcmpeq_osss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 16);
            }

            void vcmpeq_uqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 8);
            }

            void vcmpeq_uqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 8);
            }

            void vcmpeq_uqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 8);
            }

            void vcmpeq_uqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 8);
            }

            void vcmpeq_uspd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 24);
            }

            void vcmpeq_usps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 24);
            }

            void vcmpeq_ussd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 24);
            }

            void vcmpeq_usss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 24);
            }

            void vcmpeqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 0);
            }

            void vcmpeqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 0);
            }

            void vcmpeqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 0);
            }

            void vcmpeqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 0);
            }

            void vcmpfalse_ospd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 27);
            }

            void vcmpfalse_osps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 27);
            }

            void vcmpfalse_ossd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 27);
            }

            void vcmpfalse_osss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 27);
            }

            void vcmpfalsepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 11);
            }

            void vcmpfalseps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 11);
            }

            void vcmpfalsesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 11);
            }

            void vcmpfalsess(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 11);
            }

            void vcmpge_oqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 29);
            }

            void vcmpge_oqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 29);
            }

            void vcmpge_oqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 29);
            }

            void vcmpge_oqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 29);
            }

            void vcmpgepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 13);
            }

            void vcmpgeps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 13);
            }

            void vcmpgesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 13);
            }

            void vcmpgess(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 13);
            }

            void vcmpgt_oqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 30);
            }

            void vcmpgt_oqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 30);
            }

            void vcmpgt_oqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 30);
            }

            void vcmpgt_oqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 30);
            }

            void vcmpgtpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 14);
            }

            void vcmpgtps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 14);
            }

            void vcmpgtsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 14);
            }

            void vcmpgtss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 14);
            }

            void vcmple_oqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 18);
            }

            void vcmple_oqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 18);
            }

            void vcmple_oqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 18);
            }

            void vcmple_oqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 18);
            }

            void vcmplepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 2);
            }

            void vcmpleps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 2);
            }

            void vcmplesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 2);
            }

            void vcmpless(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 2);
            }

            void vcmplt_oqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 17);
            }

            void vcmplt_oqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 17);
            }

            void vcmplt_oqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 17);
            }

            void vcmplt_oqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 17);
            }

            void vcmpltpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 1);
            }

            void vcmpltps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 1);
            }

            void vcmpltsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 1);
            }

            void vcmpltss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 1);
            }

            void vcmpneq_oqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 12);
            }

            void vcmpneq_oqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 12);
            }

            void vcmpneq_oqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 12);
            }

            void vcmpneq_oqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 12);
            }

            void vcmpneq_ospd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 28);
            }

            void vcmpneq_osps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 28);
            }

            void vcmpneq_ossd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 28);
            }

            void vcmpneq_osss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 28);
            }

            void vcmpneq_uspd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 20);
            }

            void vcmpneq_usps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 20);
            }

            void vcmpneq_ussd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 20);
            }

            void vcmpneq_usss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 20);
            }

            void vcmpneqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 4);
            }

            void vcmpneqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 4);
            }

            void vcmpneqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 4);
            }

            void vcmpneqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 4);
            }

            void vcmpnge_uqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 25);
            }

            void vcmpnge_uqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 25);
            }

            void vcmpnge_uqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 25);
            }

            void vcmpnge_uqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 25);
            }

            void vcmpngepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 9);
            }

            void vcmpngeps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 9);
            }

            void vcmpngesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 9);
            }

            void vcmpngess(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 9);
            }

            void vcmpngt_uqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 26);
            }

            void vcmpngt_uqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 26);
            }

            void vcmpngt_uqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 26);
            }

            void vcmpngt_uqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 26);
            }

            void vcmpngtpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 10);
            }

            void vcmpngtps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 10);
            }

            void vcmpngtsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 10);
            }

            void vcmpngtss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 10);
            }

            void vcmpnle_uqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 22);
            }

            void vcmpnle_uqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 22);
            }

            void vcmpnle_uqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 22);
            }

            void vcmpnle_uqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 22);
            }

            void vcmpnlepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 6);
            }

            void vcmpnleps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 6);
            }

            void vcmpnlesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 6);
            }

            void vcmpnless(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 6);
            }

            void vcmpnlt_uqpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 21);
            }

            void vcmpnlt_uqps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 21);
            }

            void vcmpnlt_uqsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 21);
            }

            void vcmpnlt_uqss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 21);
            }

            void vcmpnltpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 5);
            }

            void vcmpnltps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 5);
            }

            void vcmpnltsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 5);
            }

            void vcmpnltss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 5);
            }

            void vcmpord_spd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 23);
            }

            void vcmpord_sps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 23);
            }

            void vcmpord_ssd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 23);
            }

            void vcmpord_sss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 23);
            }

            void vcmpordpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 7);
            }

            void vcmpordps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 7);
            }

            void vcmpordsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 7);
            }

            void vcmpordss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 7);
            }

            void vcmppd(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_66 | T_0F | T_EW1 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B64, 0xC2, imm);
            }

            void vcmpph(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B16, 0xC2, imm);
            }

            void vcmpps(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_0F | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0xC2, imm);
            }

            void vcmpsd(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_N8 | T_F2 | T_0F | T_EW1 | T_SAE_Z | T_MUST_EVEX, 0xC2, imm);
            }

            void vcmpsh(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_N2 | T_F3 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0xC2, imm);
            }

            void vcmpss(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_N4 | T_F3 | T_0F | T_EW0 | T_SAE_Z | T_MUST_EVEX, 0xC2, imm);
            }

            void vcmptrue_uspd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 31);
            }

            void vcmptrue_usps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 31);
            }

            void vcmptrue_ussd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 31);
            }

            void vcmptrue_usss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 31);
            }

            void vcmptruepd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 15);
            }

            void vcmptrueps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 15);
            }

            void vcmptruesd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 15);
            }

            void vcmptruess(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 15);
            }

            void vcmpunord_spd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 19);
            }

            void vcmpunord_sps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 19);
            }

            void vcmpunord_ssd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 19);
            }

            void vcmpunord_sss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 19);
            }

            void vcmpunordpd(Opmask k, Xmm x, Operand op)
            {
                vcmppd(k, x, op, 3);
            }

            void vcmpunordps(Opmask k, Xmm x, Operand op)
            {
                vcmpps(k, x, op, 3);
            }

            void vcmpunordsd(Opmask k, Xmm x, Operand op)
            {
                vcmpsd(k, x, op, 3);
            }

            void vcmpunordss(Opmask k, Xmm x, Operand op)
            {
                vcmpss(k, x, op, 3);
            }

            void vcomisbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N2 | T_66 | T_MAP5 | T_EW0 | T_MUST_EVEX, 0x2F);
            }

            void vcomish(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N2 | T_MAP5 | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2F);
            }
            ...
            void vcomxsd(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N8 | T_F2 | T_0F | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x2F);
            }

            void vcomxsh(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N2 | T_F3 | T_MAP5 | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2F);
            }

            void vcomxss(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N4 | T_F3 | T_0F | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2F);
            }

            void vcvt2ph2bf8(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N1 | T_F2 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvt2ph2bf8s(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N1 | T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvt2ph2hf8(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N1 | T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x18);
            }

            void vcvt2ph2hf8s(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N1 | T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x1B);
            }

            void vcvt2ps2phx(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F38 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B32, 0x67);
            }

            void vcvtbf162ibs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x69);
            }

            void vcvtbf162iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x6B);
            }

            void vcvtbiasph2bf8(Xmm x1, Xmm x2, Operand op)
            {
                opCvt6(x1, x2, op, T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvtbiasph2bf8s(Xmm x1, Xmm x2, Operand op)
            {
                opCvt6(x1, x2, op, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvtbiasph2hf8(Xmm x1, Xmm x2, Operand op)
            {
                opCvt6(x1, x2, op, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x18);
            }

            void vcvtbiasph2hf8s(Xmm x1, Xmm x2, Operand op)
            {
                opCvt6(x1, x2, op, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x1B);
            }

            void vcvtdq2ph(Xmm x, Operand op)
            {
                checkCvt4(x, op);
                opCvt(x, op, T_N16 | T_N_VL | T_MAP5 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B32, 0x5B);
            }

            void vcvthf82ph(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_MUST_EVEX | T_F2 | T_MAP5 | T_EW0 | T_YMM | T_N1, 0x1E);
            }
            ...
            void vcvtpd2ph(Xmm x, Operand op)
            {
            ...
            void vcvtph2bf8(Xmm x, Operand op)
            {
                opCvt2(x, op, T_F3 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvtph2bf8s(Xmm x, Operand op)
            {
                opCvt2(x, op, T_F3 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x74);
            }

            void vcvtph2dq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_MUST_EVEX | T_B16, 0x5B);
            }

            void vcvtph2hf8(Xmm x, Operand op)
            {
                opCvt2(x, op, T_F3 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x18);
            }

            void vcvtph2hf8s(Xmm x, Operand op)
            {
                opCvt2(x, op, T_F3 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x1B);
            }

            void vcvtph2ibs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B16, 0x69);
            }

            void vcvtph2iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B16, 0x6B);
            }
            ...
            void vcvtph2pd(Xmm x, Operand op)
            {
                ...
                opVex(x, null, op, T_N4 | T_N_VL | T_MAP5 | T_EW0 | T_YMM | T_SAE_X | T_MUST_EVEX | T_B16, 0x5A);
            }

            void vcvtph2psx(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_MAP6 | T_EW0 | T_YMM | T_SAE_Y | T_MUST_EVEX | T_B16, 0x13);
            }
            ...
            void vcvtph2qq(Xmm x, Operand op)
            {
                ...
                opVex(x, null, op, T_N4 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_X | T_MUST_EVEX | T_B16, 0x7B);
            }

            void vcvtph2udq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_MUST_EVEX | T_B16, 0x79);
            }
            ...
            void vcvtph2uqq(Xmm x, Operand op)
            {
                ...
                opVex(x, null, op, T_N4 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_X | T_MUST_EVEX | T_B16, 0x79);
            }

            void vcvtph2uw(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x7D);
            }

            void vcvtph2w(Xmm x, Operand op)
            {
            ...
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B32, 0x69);
            }

            void vcvtps2iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B32, 0x6B);
            }

            void vcvtps2phx(Xmm x, Operand op)
            {
                checkCvt4(x, op);
            ...
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_0F | T_EW0 | T_YMM | T_ER_Y | T_MUST_EVEX | T_B32, 0x7B);
            }
            ...
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_0F | T_EW0 | T_YMM | T_ER_Y | T_MUST_EVEX | T_B32, 0x79);
            }
            ...
            void vcvtqq2ph(Xmm x, Operand op)
            {
            ...
            void vcvtsd2sh(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vcvtsh2ss(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vcvtss2sh(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vcvttbf162ibs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x68);
            }

            void vcvttbf162iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x6A);
            }

            void vcvttpd2dqs(Xmm x, Operand op)
            {
                opCvt2(x, op, T_MAP5 | T_EW1 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B64, 0x6D);
            }
            ...
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW1 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B64, 0x6D);
            }
            ...
            void vcvttpd2udqs(Xmm x, Operand op)
            {
                opCvt2(x, op, T_MAP5 | T_EW1 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B64, 0x6C);
            }
            ...
            void vcvttpd2uqqs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW1 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B64, 0x6C);
            }

            void vcvttph2dq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_F3 | T_MAP5 | T_EW0 | T_YMM | T_SAE_Y | T_MUST_EVEX | T_B16, 0x5B);
            }

            void vcvttph2ibs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B16, 0x68);
            }

            void vcvttph2iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B16, 0x6A);
            }
            ...
            void vcvttph2qq(Xmm x, Operand op)
            {
                ...
                opVex(x, null, op, T_N4 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_SAE_X | T_MUST_EVEX | T_B16, 0x7A);
            }

            void vcvttph2udq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_MAP5 | T_EW0 | T_YMM | T_SAE_Y | T_MUST_EVEX | T_B16, 0x78);
            }
            ...
            void vcvttph2uqq(Xmm x, Operand op)
            {
                ...
                opVex(x, null, op, T_N4 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_SAE_X | T_MUST_EVEX | T_B16, 0x78);
            }

            void vcvttph2uw(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B16, 0x7C);
            }

            void vcvttph2w(Xmm x, Operand op)
            {
            ...
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B32, 0x6D);
            }

            void vcvttps2ibs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B32, 0x68);
            }

            void vcvttps2iubs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_B32, 0x6A);
            }

            void vcvttps2qq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_0F | T_EW0 | T_YMM | T_SAE_Y | T_MUST_EVEX | T_B32, 0x7A);
            }

            void vcvttps2qqs(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(
                    x,
                    null,
                    op,
                    T_N8 | T_N_VL | T_66 | T_MAP5 | T_EW0 | T_YMM | T_SAE_X | T_SAE_Y | T_MUST_EVEX | T_B32,
                    0x6D
                );
            }
            ...
            void vcvttps2udqs(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B32, 0x6C);
            }

            void vcvttps2uqq(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_66 | T_0F | T_EW0 | T_YMM | T_SAE_Y | T_MUST_EVEX | T_B32, 0x78);
            }

            void vcvttps2uqqs(Xmm x, Operand op)
            {
                checkCvt1(x, op);
                opVex(
                    x,
            ...
            {
                uint64_t type =
                    (T_N8 | T_F2 | T_0F | T_SAE_X | T_MUST_EVEX) |
                    (r.isREG(64) ? T_EW1 : T_EW0);
            ...
            {
                uint64_t type =
                    (T_N4 | T_F3 | T_0F | T_SAE_X | T_MUST_EVEX) |
                    (r.isREG(64) ? T_EW1 : T_EW0);
                opVex(r, xm0, op, type, 0x78);
            }
            ...
            {
                checkCvt1(x, op);
                opVex(x, null, op, T_N8 | T_N_VL | T_F3 | T_0F | T_EW0 | T_YMM | T_MUST_EVEX | T_B32, 0x7A);
            }

            void vcvtudq2ph(Xmm x, Operand op)
            {
                checkCvt4(x, op);
            ...
            void vcvtuqq2ph(Xmm x, Operand op)
            {
            ...
            void vcvtw2ph(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_F3 | T_MAP5 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x7D);
            }

            void vdbpsadbw(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x42, imm);
            }

            void vdivbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x5E);
            }

            void vdivph(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_ER_Z | T_B16, 0x5E);
            }

            void vdivsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
            ...
            {
                opAVX_X_X_XM(x1, x2, op, T_0F38 | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x52);
            }
            ...
            void vextractf32x4(Operand op, Ymm r, uint8_t imm)
            {
                ...
                opVex(r, null, op, T_N16 | T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x19, imm);
            }
            ...
            void vextractf32x8(Operand op, Zmm r, uint8_t imm)
            {
                ...
                opVex(r, null, op, T_N32 | T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x1B, imm);
            }
            ...
            void vextracti32x4(Operand op, Ymm r, uint8_t imm)
            {
                ...
                opVex(r, null, op, T_N16 | T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x39, imm);
            }
            ...
            void vextracti32x8(Operand op, Zmm r, uint8_t imm)
            {
                ...
                opVex(r, null, op, T_N32 | T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x3B, imm);
            }
            ...
            void vfixupimmpd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW1 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B64, 0x54, imm);
            }

            void vfixupimmps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x54, imm);
            }

            void vfixupimmsd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N8 | T_66 | T_0F3A | T_EW1 | T_SAE_Z | T_MUST_EVEX, 0x55, imm);
            }

            void vfixupimmss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F3A | T_EW0 | T_SAE_Z | T_MUST_EVEX, 0x55, imm);
            }

            void vfmadd132bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x98);
            }

            void vfmadd132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x98);
            }

            void vfmadd132sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0x99);
            }

            void vfmadd213bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xA8);
            }

            void vfmadd213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xA8);
            }

            void vfmadd213sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xA9);
            }

            void vfmadd231bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xB8);
            }

            void vfmadd231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xB8);
            }

            void vfmadd231sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xB9);
            }

            void vfmaddcph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_F3 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B32, 0x56);
            }

            void vfmaddsub132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x96);
            }

            void vfmaddsub213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xA6);
            }

            void vfmaddsub231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xB6);
            }

            void vfmsub132bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x9A);
            }

            void vfmsub132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x9A);
            }

            void vfmsub132sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0x9B);
            }

            void vfmsub213bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xAA);
            }

            void vfmsub213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xAA);
            }

            void vfmsub213sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xAB);
            }

            void vfmsub231bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xBA);
            }

            void vfmsub231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xBA);
            }

            void vfmsub231sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xBB);
            }

            void vfmsubadd132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x97);
            }

            void vfmsubadd213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xA7);
            }

            void vfmsubadd231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xB7);
            }

            void vfmulcph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_F3 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B32, 0xD6);
            }

            void vfnmadd132bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x9C);
            }

            void vfnmadd132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x9C);
            }

            void vfnmadd132sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0x9D);
            }

            void vfnmadd213bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xAC);
            }

            void vfnmadd213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xAC);
            }

            void vfnmadd213sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xAD);
            }

            void vfnmadd231bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xBC);
            }

            void vfnmadd231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xBC);
            }

            void vfnmadd231sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xBD);
            }

            void vfnmsub132bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x9E);
            }

            void vfnmsub132ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x9E);
            }

            void vfnmsub132sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0x9F);
            }

            void vfnmsub213bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xAE);
            }

            void vfnmsub213ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xAE);
            }

            void vfnmsub213sh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_ER_X | T_MUST_EVEX, 0xAF);
            }

            void vfnmsub231bf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0xBE);
            }

            void vfnmsub231ph(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0xBE);
            }

            void vfnmsub231sh(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vfpclasspd(Opmask k, Operand op, uint8_t imm)
            {
            ...
            void vfpclassph(Opmask k, Operand op, uint8_t imm)
            {
                if (!op.isBit(128 | 256 | 512))
                {
                    mixin(XBYAK_THROW(ERR.BAD_MEM_SIZE));
                }
                opVex(k.changeBit(op.getBit()), null, op, T_0F3A | T_MUST_EVEX | T_YMM | T_EW0 | T_B16, 0x66, imm);
            }

            void vfpclassps(Opmask k, Operand op, uint8_t imm)
            {
                if (!op.isBit(128 | 256 | 512))
                {
                    mixin(XBYAK_THROW(ERR.BAD_MEM_SIZE));
            ...
            void vfpclasssd(Opmask k, Operand op, uint8_t imm)
            {
                if (!op.isXMEM())
                {
                    mixin(XBYAK_THROW(ERR.BAD_MEM_SIZE));
                }
                opVex(k, null, op, T_66 | T_0F3A | T_MUST_EVEX | T_EW1 | T_N8, 0x67, imm);
            }

            void vfpclasssh(Opmask k, Operand op, uint8_t imm)
            {
                if (!op.isXMEM())
                {
                    mixin(XBYAK_THROW(ERR.BAD_MEM_SIZE));
                }
                opVex(k, null, op, T_0F3A | T_MUST_EVEX | T_EW0 | T_N2, 0x67, imm);
            }

            void vfpclassss(Opmask k, Operand op, uint8_t imm)
            {
                if (!op.isXMEM())
                {
                    mixin(XBYAK_THROW(ERR.BAD_MEM_SIZE));
                }
                opVex(k, null, op, T_66 | T_0F3A | T_MUST_EVEX | T_EW0 | T_N4, 0x67, imm);
            }
            ...
            void vgetexpbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x42);
            }
            ...
            void vgetexpph(Xmm x, Operand op)
            {
            ...
            void vgetexpsh(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vgetmantbf16(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x26, imm);
            }

            void vgetmantpd(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW1 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B64, 0x26, imm);
            }

            void vgetmantph(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B16, 0x26, imm);
            }

            void vgetmantps(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x26, imm);
            }

            void vgetmantsd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N8 | T_66 | T_0F3A | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x27, imm);
            }

            void vgetmantsh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x27, imm);
            }

            void vgetmantss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vmaxbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x5F);
            }

            void vmaxph(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_SAE_Z | T_B16, 0x5F);
            }

            void vmaxsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_F3 | T_EW0 | T_MUST_EVEX | T_SAE_X | T_N2, 0x5F);
            }

            void vminbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x5D);
            }

            void vminmaxbf16(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_F2 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x52, imm);
            }

            void vminmaxpd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(
                    x1, x2, op, T_66 | T_0F3A | T_EW1 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B64, 0x52, imm
                );
            }

            void vminmaxph(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_0F3A | T_EW0 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B16, 0x52, imm);
            }

            void vminmaxps(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(
                    x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Y | T_SAE_Z | T_MUST_EVEX | T_B32, 0x52, imm
                );
            }

            void vminmaxsd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N8 | T_66 | T_0F3A | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x53, imm);
            }

            void vminmaxsh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x53, imm);
            }

            void vminmaxss(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x53, imm);
            }

            void vminph(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_SAE_Z | T_B16, 0x5D);
            }

            void vminsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_F3 | T_EW0 | T_MUST_EVEX | T_SAE_X | T_N2, 0x5D);
            }

            void vmovdqa32(Address addr, Xmm x)
            {
                opAVX_X_XM_IMM(
                    x, addr, T_66 | T_0F | T_EW0 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K, 0x7F
                );
            }
            ...
            {
                opAVX_X_XM_IMM(
                    x, addr, T_66 | T_0F | T_EW1 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K, 0x7F
                );
            }
            ...
            {
                opAVX_X_XM_IMM(
                    x, addr, T_F2 | T_0F | T_EW1 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K, 0x7F
                );
            }
            ...
            void vmovdqu32(Address addr, Xmm x)
            {
                opAVX_X_XM_IMM(
                    x,
                    addr,
                    T_F3 | T_0F | T_EW0 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K,
                    0x7F
                );
            }
            ...
            void vmovdqu64(Address addr, Xmm x)
            {
                opAVX_X_XM_IMM(
                    x,
                    addr,
                    T_F3 | T_0F | T_EW1 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K,
                    0x7F
                );
            }
            ...
            void vmovdqu8(Address addr, Xmm x)
            {
                opAVX_X_XM_IMM(
                    x,
                    addr,
                    T_F2 | T_0F | T_EW0 | T_YMM | T_ER_X | T_ER_Y | T_ER_Z | T_MUST_EVEX | T_M_K,
                    0x7F
                );
            }
            ...
            void vmovsh(Xmm x1, Xmm x2, Xmm x3)
            {
                opAVX_X_X_XM(x1, x2, x3, T_N2 | T_F3 | T_MAP5 | T_EW0 | T_MUST_EVEX, 0x10);
            }

            void vmpsadbw(Xmm x1, Xmm x2, Operand op, uint8_t imm, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(
                    x1,
                    x2,
                    ...
                    encoding,
                    imm,
                    T_66 | T_W0 | T_YMM, T_F3 | T_0F3A | T_EW0 | T_B32,
                    1
                );
            }

            void vmulbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x59);
            }

            void vmulph(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_ER_Z | T_B16, 0x59);
            }

            void vmulsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
            ...
            {
                opVex(x, null, r, T_66 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX, 0x7A);
            }

            void vpbroadcastd(Xmm x, Reg32 r)
            {
                opVex(x, null, r, T_66 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX, 0x7C);
            }

            void vpbroadcastmb2q(Xmm x, Opmask k)
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_YMM | T_MUST_EVEX | T_EW1, 0x2A);
            }

            void vpbroadcastmw2d(Xmm x, Opmask k)
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_YMM | T_MUST_EVEX | T_EW0, 0x3A);
            }

            void vpbroadcastw(Xmm x, Reg16 r)
            {
                opVex(x, null, r, T_66 | T_0F38 | T_EW0 | T_YMM | T_MUST_EVEX, 0x7B);
            }

            void vpcmpb(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
                opAVX_K_X_XM(k, x, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX, 0x3F, imm);
            }

            void vpcmpd(Opmask k, Xmm x, Operand op, uint8_t imm)
            {
            ...
            void vpcompressb(Operand op, Xmm x)
            {
            ...
            void vpcompressw(Operand op, Xmm x)
            {
            ...
            {
                opEncoding(x1, x2, op, T_F2 | T_0F38 | T_YMM, 0x50, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpbssds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_F2 | T_0F38 | T_YMM, 0x51, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpbsud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_F3 | T_0F38 | T_YMM, 0x50, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpbsuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_F3 | T_0F38 | T_YMM, 0x51, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpbuud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_0F38 | T_YMM, 0x50, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpbuuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_0F38 | T_YMM, 0x51, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwsud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_F3 | T_0F38 | T_YMM, 0xD2, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwsuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_F3 | T_0F38 | T_YMM, 0xD3, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwusd(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_66 | T_0F38 | T_YMM, 0xD2, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwusds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_66 | T_0F38 | T_YMM, 0xD3, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwuud(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_0F38 | T_YMM, 0xD2, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }

            void vpdpwuuds(Xmm x1, Xmm x2, Operand op, PreferredEncoding encoding = DefaultEncoding)
            {
                opEncoding(x1, x2, op, T_0F38 | T_YMM, 0xD3, encoding, NONE, T_W0, T_EW0 | T_B32, 1);
            }
            ...
            {
                opVex(k, null, x, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW0, 0x29);
            }

            void vpmovd2m(Opmask k, Xmm x)
            {
                opVex(k, null, x, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW0, 0x39);
            }
            ...
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW0, 0x28);
            }

            void vpmovm2d(Xmm x, Opmask k)
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW0, 0x38);
            }

            void vpmovm2q(Xmm x, Opmask k)
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW1, 0x38);
            }

            void vpmovm2w(Xmm x, Opmask k)
            {
                opVex(x, null, k, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW1, 0x28);
            }

            void vpmovq2m(Opmask k, Xmm x)
            {
                opVex(k, null, x, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW1, 0x39);
            }
            ...
            {
                opVex(k, null, x, T_F3 | T_0F38 | T_MUST_EVEX | T_YMM | T_EW1, 0x29);
            }
            ...
            void vpshldd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x71, imm);
            }

            void vpshldq(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vpshrdw(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vpternlogd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX | T_B32, 0x25, imm);
            }

            void vpternlogq(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vrcpbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x4C);
            }

            void vrcpph(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x4C);
            }

            void vrcpsh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_MUST_EVEX, 0x4D);
            }

            void vreducebf16(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x56, imm);
            }

            void vreducepd(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW1 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B64, 0x56, imm);
            }

            void vreduceph(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B16, 0x56, imm);
            }

            void vreduceps(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x56, imm);
            }

            void vreducesd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N8 | T_66 | T_0F3A | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x57, imm);
            }

            void vreducesh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x57, imm);
            }

            void vreducess(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N4 | T_66 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x57, imm);
            }

            void vrndscalebf16(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_F2 | T_0F3A | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x08, imm);
            }

            void vrndscalepd(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW1 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B64, 0x09, imm);
            }

            void vrndscaleph(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B16, 0x08, imm);
            }

            void vrndscaleps(Xmm x, Operand op, uint8_t imm)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_0F3A | T_EW0 | T_YMM | T_SAE_Z | T_MUST_EVEX | T_B32, 0x08, imm);
            }

            void vrndscalesd(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N8 | T_66 | T_0F3A | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x0B, imm);
            }

            void vrndscalesh(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_0F3A | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x0A, imm);
            }

            void vrndscaless(Xmm x1, Xmm x2, Operand op, uint8_t imm)
            {
            ...
            void vrsqrtbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x4E);
            }

            void vrsqrtph(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x4E);
            }

            void vrsqrtsh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_66 | T_MAP6 | T_EW0 | T_MUST_EVEX, 0x4F);
            }

            void vscalefbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x2C);
            }

            void vscalefbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_MAP6 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x2C);
            }
            ...
            void vscalefph(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vscalefsh(Xmm x1, Xmm x2, Operand op)
            {
            ...
            void vshuff32x4(Ymm y1, Ymm y2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW0 | T_B32, 0x23, imm);
            }

            void vshuff64x2(Ymm y1, Ymm y2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW1 | T_B64, 0x23, imm);
            }

            void vshufi32x4(Ymm y1, Ymm y2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW0 | T_B32, 0x43, imm);
            }

            void vshufi64x2(Ymm y1, Ymm y2, Operand op, uint8_t imm)
            {
                opAVX_X_X_XM(y1, y2, op, T_66 | T_0F3A | T_YMM | T_MUST_EVEX | T_EW1 | T_B64, 0x43, imm);
            }

            void vsqrtbf16(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x51);
            }

            void vsqrtph(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_MAP5 | T_EW0 | T_YMM | T_ER_Z | T_MUST_EVEX | T_B16, 0x51);
            }

            void vsqrtsh(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_N2 | T_F3 | T_MAP5 | T_EW0 | T_ER_X | T_MUST_EVEX, 0x51);
            }

            void vsubbf16(Xmm x1, Xmm x2, Operand op)
            {
                opAVX_X_X_XM(x1, x2, op, T_66 | T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_B16, 0x5C);
            }

            void vsubph(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_EW0 | T_YMM | T_MUST_EVEX | T_ER_Z | T_B16, 0x5C);
            }

            void vsubsh(Xmm xmm, Operand op1, Operand op2 = Operand())
            {
                opAVX_X_X_XM(xmm, op1, op2, T_MAP5 | T_F3 | T_EW0 | T_MUST_EVEX | T_ER_X | T_N2, 0x5C);
            }

            void vucomish(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N2 | T_MAP5 | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2E);
            }

            void vucomxsd(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N8 | T_F2 | T_0F | T_EW1 | T_SAE_X | T_MUST_EVEX, 0x2E);
            }

            void vucomxsh(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N2 | T_F3 | T_MAP5 | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2E);
            }

            void vucomxss(Xmm x, Operand op)
            {
                opAVX_X_XM_IMM(x, op, T_N4 | T_F3 | T_0F | T_EW0 | T_SAE_X | T_MUST_EVEX, 0x2E);
            }
            ...
            version (XBYAK64)
            {
                ...
                {
                    opVex(x, null, r, T_66 | T_0F38 | T_EW1 | T_YMM | T_MUST_EVEX, 0x7C);
                }
            }
        }
