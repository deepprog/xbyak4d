...
T local_max_(T)(T x, T y)
{
    return x >= y ? x : y;
}

T local_min_(T)(T x, T y)
{
    return x < y ? x : y;
}
            ...
            if (data[0] >= 0x8000001D)
            {
                ...
                )
                {
                    ...
                    */
                    ...
                    if (cacheType == 0x2)
                        continue; // Skip instruction cache.
            ...
            */
            for (int i = 0; dataCacheLevels_ < maxNumberCacheLevels; i++)
            {
                ...
                if (cacheType == DATA_CACHE || cacheType == UNIFIED_CACHE)
                {
                    ...
                    if (logical_cores != 0)
                    { // true only if leaf 0xB is supported and valid
...
public:
    ...
    uint32_t getNumCores(CpuTopologyLevel level) const
    {
        ...
        default:
            mixin(XBYAK_THROW_RET(ERR.X2APIC_IS_NOT_SUPPORTED, "0"));
        }
    }
    ...
    uint32_t getCoresSharingDataCache(uint32_t i) const
    {
        if (i >= dataCacheLevels_)
            mixin(XBYAK_THROW_RET(ERR.BAD_PARAMETER, "0"));
    ...
    */
    ...
    this()
    {
        ...
        if (maxNum >= 7)
        {
            ...
            if (maxNumSubLeaves >= 1)
            {
                ...
                if (*EAX & (1U << 23))
                    type_ |= tAVX_IFMA;
                if (*EDX & (1U << 4))
                    type_ |= tAVX_VNNI_INT8;
                if (*EDX & (1U << 5))
                    type_ |= tAVX_NE_CONVERT;
                if (*EDX & (1U << 10))
                    type_ |= tAVX_VNNI_INT16;
                if (*EDX & (1U << 14))
                    type_ |= tPREFETCHITI;
                if (*EDX & (1U << 19))
                    type_ |= tAVX10;
        ...
        if (maxNum >= 0x19)
        {
            ...
            if (*EBX & (1U << 2))
                type_ |= tWIDE_KL;
            if (type_ & (tKEYLOCKER | tAESKLE | tWIDE_KL))
                type_ |= tKEYLOCKER_WIDE;
        }
    ...
    int getAVX10version() const
    {
        return avx10version_;
    }
}
    ...
    version (XBYAK64)
    {
        ...
        */
            this(CodeGenerator code, int pNum, int tNum = 0, int stackSizeByte = 0, bool makeEpilog = true)
            {
                ...
                if (pNum < 0 || pNum > 4)
                    mixin(XBYAK_THROW(ERR.BAD_PNUM));
                const int allRegNum = pNum + tNum_ + (useRcx_ ? 1 : 0) + (useRdx_ ? 1 : 0);
                if (tNum_ < 0 || allRegNum > maxRegNum)
                    mixin(XBYAK_THROW(ERR.BAD_TNUM));
                Reg64 _rsp = code.rsp;
